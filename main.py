...
# Telegram-kommandon enligt spec, PnL, CSV-export, symbols/timeframe, FastAPI keep-alive för Render.
...
# --- FastAPI för Render (uvicorn main:app) ---
...
def parse_symbols_text(args: List[str]) -> List[str]:
    if not args: return []
    return parse_symbols_arg(" ".join(args))
...
async def symbols_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    new_list = parse_symbols_text(context.args)
    if not new_list:
...
async def reset_pnl_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Sätt ankare = nuvarande råa totals (utan tidigare ankare)
