# main.py — PTB i FastAPI:s loop, keep-alive för Render Free
# Kommandon: /status /set_ai /start_mock /start_live /engine_start /engine_stop
#            /symbols /timeframe /export_csv /pnl /reset_pnl /help
# ORB: färgskifte => nytt ORB, 1 trade/ORB, SL på ORB-linjen, trailing per candle

import os, csv, json, time, asyncio, traceback
...
# ======= Bas-konfig =======
BOT_NAME = "Mp ORBbot"
TELEGRAM_TOKEN = "8079688612:AAGM-6vTQ6R_ZSdfnQWD0LCqcmS7_zk46Us"  # hårdkodad
SERVICE_URL = os.getenv("SERVICE_URL", "https://mporbbot.onrender.com/")  # keep-alive ping
...
def fetch_kucoin_candles(symbol:str, start_ts:int, end_ts:int, interval:str)->List[dict]:
    ...
    out=[]
    for row in reversed(data):  # kucoin returnerar senaste först
...
class ORBWindow:
    def __init__(self, start_ts:int, high:float, low:float, direction:str):
        self.start_ts=start_ts; self.high=high; self.low=low; self.direction=direction
        self.id=f"{start_ts}"
...
CONFIG={"ai_mode":"neutral","mock_mode":True,"is_trading":False,
        "min_orb_pct":min_orb_pct_for_mode("neutral"),"last_heartbeat":now_ts(),"chat_id":None}
...
def log_trade(side,symbol,entry,exitp,qty,fee,pnl,note):
    path=MOCK_LOG if CONFIG["mock_mode"] else REAL_LOG
    log_trade_row(path,[fmt_ts(now_ts()),symbol,side,f"{entry:.8f}",f"{exitp:.8f}",
                        f"{qty:.8f}",f"{fee:.6f}",f"{pnl:.6f}",
                        "mock" if CONFIG["mock_mode"] else "live",
                        CONFIG["ai_mode"], STATE[symbol].orb.id if STATE[symbol].orb else "", note])

def load_pnl_anchor()->Dict[str,float]:
    if not os.path.exists(PNL_ANCHOR_FILE): return {"mock":0.0,"live":0.0,"date":today_str()}
    try:
        with open(PNL_ANCHOR_FILE,"r",encoding="utf-8") as f: data=json.load(f)
        if data.get("date")!=today_str(): return {"mock":0.0,"live":0.0,"date":today_str()}
        return data
    except: return {"mock":0.0,"live":0.0,"date":today_str()}

def save_pnl_anchor(mock_total:float, live_total:float):
    with open(PNL_ANCHOR_FILE,"w",encoding="utf-8") as f: json.dump({"mock":mock_total,"live":live_total,"date":today_str()},f)
...
def read_pnl_for_today(paths:List[str])->Dict[str,float]:
    ...
    anc=load_pnl_anchor()
    totals["mock"]-=anc.get("mock",0.0); totals["live"]-=anc.get("live",0.0)
    return totals

# ======= Trading =======
def update_trailing_stop(symbol:str, prev:dict):
    st=STATE[symbol]
    if not st.in_position or not TRAIL_ON_EACH_CANDLE: return
    if st.side=="long":
        st.stop=max(st.stop or -1e18, prev["low"]-STOP_OFFSET)
    else:
        st.stop=min(st.stop or  1e18, prev["high"]+STOP_OFFSET)
...
def trading_tick(interval:str)->List[str]:
    notes=[]
...
async def keepalive_job(context: ContextTypes.DEFAULT_TYPE):
    try:
        SESSION.get(SERVICE_URL, timeout=5)
    except Exception:
        pass

# ======= Bygg PTB-app (handlers) =======
def build_application()->Application:
    ...
    app_tg.add_handler(CallbackQueryHandler(confirm_cb, pattern="^(confirm_start_mock|cancel_start_mock|confirm_start_live|cancel_start_live)$"))
...
# ======= Kör PTB i FastAPI:s loop utan run_polling =======
async def telegram_boot():
    global TG_APP
    ...
    while True:
        try:
            ...
            print("[startup] PTB initialize/start …")
            await TG_APP.initialize()
            await TG_APP.start()

            # schema-jobb
            TG_APP.job_queue.run_repeating(
                lambda c: [c.bot.send_message(chat_id=CONFIG["chat_id"], text=n) for n in trading_tick(INTERVAL)] if CONFIG["chat_id"] else None,
                interval=10, first=5
            )
            TG_APP.job_queue.run_repeating(heartbeat_job, interval=HEARTBEAT_SEC, first=5)
            TG_APP.job_queue.run_repeating(keepalive_job, interval=240, first=15)  # ping var 4:e min

            print("[startup] updater.start_polling() …")
            await TG_APP.updater.start_polling(allowed_updates=Update.ALL_TYPES)

            # håll igång (updater kör i bakgrunden)
            await asyncio.Event().wait()
        except Exception as e:
            print("[fatal] telegram loop kraschade:", e)
            traceback.print_exc()
            try:
                if TG_APP:
                ...
                pass
            await asyncio.sleep(5)  # auto-retry
        finally:
            TG_APP = None
...
@app.get("/")
def root():
    return {"status":"Mp ORBbot running","timeframe":INTERVAL,"symbols":SYMBOLS,
            "mode":"mock" if CONFIG["mock_mode"] else "live","ai_mode":CONFIG["ai_mode"]}

@app.on_event("startup")
async def on_startup():
    print("[render] uvicorn startup → starta telegram (utan run_polling) + keepalive")
    asyncio.create_task(telegram_boot())
