...
# FastAPI för Render (uvicorn main:app), Telegram-polling i bakgrundstråd
...
# Hårdkodad token (enligt önskan)
TELEGRAM_TOKEN = "8079688612:AAGM-6vTQ6R_ZSdfnQWD0LCqcmS7_zk46Us"

# Standard-symboler & timeframe (kan bytas via /symbols /timeframe)
...
# FastAPI-app (Render startar oss via uvicorn main:app)
...
# ===========================
#   BYGG & STARTA TELEGRAM
# ===========================
def build_telegram_app():
    app_tg = ApplicationBuilder().token(TELEGRAM_TOKEN).build()

    # 🔧 Fix: vissa miljöer ger None → skapa JobQueue manuellt
    if app_tg.job_queue is None:
        print("[startup] JobQueue missing → creating manually")
        jq = JobQueue()
        jq.set_application(app_tg)
        app_tg.job_queue = jq
