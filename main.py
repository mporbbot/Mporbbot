...
# main_v44.py
# ------------------------------------------------------------
# MP Bot – v44 (hybrid: TP-gate -> trailing)
# - Reply-keyboard only
# - Mock-handel med avgifter per sida
# - Grid/DCA + TP (hybrid kan ersätta fast TP)
# - Long & short
...
# - Trailing + HYBRID: aktivera trailing först efter liten vinst och lås min-vinst
...
HYB_LOCK_PCT = float(os.getenv("HYB_LOCK_PCT", "0.10"))        # säkra minst +0.10% från snitt när trailing aktiveras
...
class Position:
    ...
    lock_price: float = 0.0      # minsta vinstnivå när hybrid aktiverat
...
class EngineState:
    ...
    risk_cfg: Dict[str, float] = field(default_factory=lambda: dict(
        ...
        hyb_on=HYB_ON,             # HYBRID på/av
        hyb_start=HYB_START_PCT,   # procent för att starta trailing
        hyb_lock=HYB_LOCK_PCT,     # lås minsta vinst
    ))
                ...
                for sym in STATE.symbols:
                    ...
                    if trail_on and (st.pos.trailing_active or not hyb_on):
                        ...
                        if st.pos.side == "LONG":
                            ...
                            stop_level = trail_level
                            if st.pos.trailing_active and st.pos.lock_price > 0:
                                # för SHORT vill vi inte ha stop ovanför låsnivån
...
async def cmd_risk(update: Update, _):
    ...
    msg = (
        ...
        f"  trail={'ON' if r.get('trail_on') else 'OFF'} (min={r.get('trail_min',0):.2f}% x{r.get('trail_mult',1):.2f})\n"
        f"  hybrid={'ON' if r.get('hyb_on') else 'OFF'} (start={r.get('hyb_start',0):.2f}% lock={r.get('hyb_lock',0):.2f}%)\n"
        "Ex: /risk set trail_on on  | /risk set hyb_on on  | /risk set hyb_start 0.40  | /risk set hyb_lock 0.15"
    )
    await tg_app.bot.send_message(STATE.chat_id, msg, reply_markup=reply_kb())
...
async def root():
    total = sum(STATE.per_sym[s].realized_pnl_net for s in STATE.symbols)
    return f"MP Bot v44 OK | engine_on={STATE.engine_on} | thr={STATE.entry_threshold:.2f} | pnl_total={total:+.4f}"
