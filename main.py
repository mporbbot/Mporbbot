...
# - Telegram + FastAPI (Webhook, DigitalOcean-kompatibel)
#
# LÄGEN:
...
#   KUCOIN_API_KEY_VERSION  (2 eller 3 – matchar KuCoin)
#   KUCOIN_BASE_URL         (default https://api.kucoin.com)
#
...
def _env_strip(name: str, default: str = "") -> str:
    """Läs env-variabel och trimma bort osynliga mellanslag runtom."""
    v = os.getenv(name, default)
    return v.strip() if v is not None else default


# Telegram / webhook
BOT_TOKEN = _env_strip("TELEGRAM_BOT_TOKEN") or _env_strip("BOT_TOKEN")
if not BOT_TOKEN:
    raise RuntimeError("Saknar TELEGRAM_BOT_TOKEN/BOT_TOKEN i env.")

WEBHOOK_BASE = _env_strip("WEBHOOK_BASE").rstrip("/")

# Symboler / tidsupplösningar
DEFAULT_SYMBOLS = (
    _env_strip("SYMBOLS", "BTC-USDT,ETH-USDT,ADA-USDT,LINK-USDT,XRP-USDT")
    .replace(" ", "")
).split(",")
DEFAULT_TFS = _env_strip("TIMEFRAMES", "1m,3m,5m").replace(" ", "").split(",")

# Standardinställningar
MOCK_SIZE_USDT = float(_env_strip("MOCK_SIZE_USDT", "50"))
FEE_PER_SIDE = float(_env_strip("FEE_PER_SIDE", "0.001"))
MAX_OPEN_POS = int(_env_strip("MAX_POS", "4"))

TP_PCT = float(_env_strip("TP_PCT", "0.30"))  # take-profit i %
SL_PCT = float(_env_strip("SL_PCT", "0.50"))  # stop-loss i %
TRAIL_START_PCT = float(_env_strip("TRAIL_START_PCT", "2.0"))
TRAIL_PCT = float(_env_strip("TRAIL_PCT", "2.5"))

ENTRY_THRESHOLD = float(_env_strip("ENTRY_THRESHOLD", "0.55"))  # momentum-score
ALLOW_SHORTS_DEFAULT = (
    _env_strip("ALLOW_SHORTS", "false").lower() in ("1", "true", "on", "yes")
)

# Mean Reversion / regime defaults
MR_DEV_PCT = float(_env_strip("MR_DEV_PCT", "1.20"))  # avvikelse från EMA20 i %
TREND_SLOPE_MIN = float(_env_strip("TREND_SLOPE_MIN", "0.20"))  # min lutning EMA20 (%)
RANGE_ATR_MAX = float(_env_strip("RANGE_ATR_MAX", "0.80"))  # max ATR% för range

# Loss-guard defaults
LOSS_GUARD_ON_DEFAULT = True
LOSS_GUARD_N_DEFAULT = int(_env_strip("LOSS_GUARD_N", "2"))
LOSS_GUARD_PAUSE_MIN_DEFAULT = int(_env_strip("LOSS_GUARD_PAUSE_MIN", "15"))

# -----------------------------
# KuCoin private API (för LIVE)
# -----------------------------

KUCOIN_API_KEY = _env_strip("KUCOIN_API_KEY")
KUCOIN_API_SECRET = _env_strip("KUCOIN_API_SECRET")
KUCOIN_API_PASSPHRASE = _env_strip("KUCOIN_API_PASSPHRASE")
KUCOIN_API_KEY_VERSION = _env_strip("KUCOIN_API_KEY_VERSION", "2")
KUCOIN_BASE_URL = _env_strip("KUCOIN_BASE_URL", "https://api.kucoin.com")
...
def kucoin_creds_ok() -> bool:
    # Debug – visa längder, inte värden
    print(
        f"[KUCOIN DEBUG] key_len={len(KUCOIN_API_KEY)}, "
        f"secret_len={len(KUCOIN_API_SECRET)}, "
        f"passphrase_len={len(KUCOIN_API_PASSPHRASE)}, "
        f"version={KUCOIN_API_KEY_VERSION!r}"
    )
...
async def kucoin_place_market_order(symbol: str, side: str, amount: float):
    ...
    amount:
        ...
        - SELL: mängd bas-valuta (size), t.ex. BTC
    """
    side_l = side.lower()
    body = {
...
def open_position(
    sym: str, side: str, price: float, st: SymState, regime: str, reason: str
) -> None:
...
app = FastAPI(title="MP Bot v54", version="1.0.0")
