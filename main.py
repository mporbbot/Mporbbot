# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# MP Bot – v56 (Hybrid Momentum + Mean Reversion, MOCK + LIVE)
# ------------------------------------------------------------
...
# - Telegram + FastAPI (DigitalOcean-kompatibel)
#
# LÄGEN:
...
#
# Telegram:
#   /start, /status, /engine_on, /engine_off
#   /timeframe, /threshold
#   /risk (+ knappar för size, TP, SL, max_pos)
#   /pnl, /export_csv
#   /export_skatt (Skatteverks-CSV från real_trade_log.csv)
#   /export_all   (ZIP med alla loggar + Skatte-CSV)
#   /mode (MOCK/LIVE + bekräftelse)
#   /close_all, /reset_pnl
#   /test_buy (litet live test-köp BTC-USDT 5 USDT)
#
# Miljövariabler (DigitalOcean):
#   TELEGRAM_BOT_TOKEN  eller BOT_TOKEN
#   WEBHOOK_BASE        (https://din-app.ondigitalocean.app)
#
#   KUCOIN_API_KEY
#   KUCOIN_API_SECRET
#   KUCOIN_API_PASSPHRASE
#   KUCOIN_API_KEY_VERSION  (t.ex. "3" för v3-nyckel)
...
#   MAX_POS            (default 4)
#
#   QTY_DECIMALS       (antal decimaler för size, default 4)
#
# ------------------------------------------------------------
...
# Hur många decimaler vi använder för size (qty)
...
async def kucoin_private_request(method: str, path: str, body: Optional[dict] = None):
    ...
    logger.info(
        "[KUCOIN DEBUG] key_len=%s, secret_len=%s, passphrase_len=%s, version='%s'",
        len(KUCOIN_API_KEY),
        len(KUCOIN_API_SECRET),
        len(KUCOIN_API_PASSPHRASE),
...
async def kucoin_place_market_order(symbol: str, side: str, amount: float):
    ...
    if side_l == "buy":
        ...
        body["size"] = f"{amount:.{QTY_DECIMALS}f}"
...
def open_position(sym: str, side: str, price: float, st: SymState,
    ...
    # räkna qty i basvalutan, runda till QTY_DECIMALS
...
def close_position(sym: str, price: float, st: SymState, reason: str) -> float:
    ...
    _log_trade({
        ...
        "side": pos.side,
        "price": round(price, 6),
        "qty": round(pos.qty, QTY_DECIMALS),
        "gross": round(gross, 6),
        "fee_in": round(fee_in, 6),
        "fee_out": round(fee_out, 6),
        "net": round(net, 6),
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "🤖 MP Bot v56 – Hybrid Momentum + Mean Reversion\n"
...
async def cmd_risk(update: Update, _):
    ...
    # Annars: visa status + snabbknappar
    kb = InlineKeyboardMarkup(
        [
            ...
            # rad 2: TP
            [
                InlineKeyboardButton("TP 0.3%", callback_data="risk_tp_0_3"),
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        f"Aktuellt:\n"
        f"• Size: {STATE.mock_size:.1f} USDT\n"
        f"• TP: {STATE.tp_pct:.2f}% | SL: {STATE.sl_pct:.2f}%\n"
        f"• max_pos: {STATE.max_pos}\n\n"
        "Välj snabb-inställning eller använd /risk set key value.",
        reply_markup=kb,
    )
...
async def cmd_export_csv(update: Update, _):
    ...
    await tg_app.bot.send_document(
        ...
        caption="Export CSV (intern logg)",
    )

async def cmd_export_skatt(update: Update, _):
    """
    Skatteverks-CSV baserad på real_trade_log.csv (endast LIVE-trades).
    """
    STATE.chat_id = update.effective_chat.id
    fname = "real_trade_log.csv"
    if not os.path.exists(fname):
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Ingen real_trade_log.csv hittades ännu (inga LIVE-trades loggade).",
            reply_markup=reply_kb(),
        )
        return
    ...
    rows_out = [
        [
            ...
            "fee_out",
            "net",
            "info",       # extra info (regime, strategi, mode=live)
        ]
    ]
    ...
    await tg_app.bot.send_document(
        STATE.chat_id,
        document=io.BytesIO(buf.getvalue().encode("utf-8")),
        filename="skatt_real_trades.csv",
        caption="Skatteverket-export (LIVE-trades)",
    )

async def cmd_export_all(update: Update, _):
    """
    Skicka ZIP med:
      - real_trade_log.csv (LIVE)
      - mock_trade_log.csv (MOCK)
      - skatt_real_trades.csv (genererad på flygande fot)
    """
    STATE.chat_id = update.effective_chat.id

    files_to_add = []
    if os.path.exists("real_trade_log.csv"):
        files_to_add.append("real_trade_log.csv")
    if os.path.exists("mock_trade_log.csv"):
        files_to_add.append("mock_trade_log.csv")

    # Generera Skatte-CSV i minnet om real finns
    skatt_bytes = None
    if os.path.exists("real_trade_log.csv"):
        rows_out = [
            [
        ...
        with open("real_trade_log.csv", "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for r in reader:
                rows_out.append(
                    [
        ...
        buf = io.StringIO()
        csv.writer(buf).writerows(rows_out)
        skatt_bytes = buf.getvalue().encode("utf-8")

    if not files_to_add and not skatt_bytes:
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Inga loggfiler hittades ännu (mock_trade_log.csv / real_trade_log.csv).",
            reply_markup=reply_kb(),
        )
        return
    ...
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if skatt_bytes:
            zf.writestr("skatt_real_trades.csv", skatt_bytes)

    zip_buf.seek(0)
    await tg_app.bot.send_document(
        STATE.chat_id,
        document=zip_buf,
        filename="mpbot_logs.zip",
        caption="Alla loggar (LIVE, MOCK + Skatteverket-export)",
    )
...
async def on_button(update: Update, _):
    ...
    if data.startswith("risk_tp_"):
        ...
        if data == "risk_tp_0_3":
            STATE.tp_pct = 0.30
        elif data == "risk_tp_0_5":
            STATE.tp_pct = 0.50
        elif data == "risk_tp_1_0":
            STATE.tp_pct = 1.00
        await query.edit_message_text(
            f"TP uppdaterad till {STATE.tp_pct:.2f}%."
        )
        return
    ...
    if data.startswith("risk_sl_"):
        ...
        if data == "risk_sl_0_5":
            STATE.sl_pct = 0.50
        elif data == "risk_sl_1_0":
            STATE.sl_pct = 1.00
        elif data == "risk_sl_2_0":
            STATE.sl_pct = 2.00
        await query.edit_message_text(
            f"SL uppdaterad till {STATE.sl_pct:.2f}%."
        )
        return

    # Risk max_pos snabbval
    if data.startswith("risk_maxpos_"):
        await query.answer()
        try:
            mp = int(data.split("_")[-1])
        except Exception:
            mp = STATE.max_pos
        STATE.max_pos = mp
        await query.edit_message_text(
            f"max_pos uppdaterad till {STATE.max_pos}."
        )
        return
...
tg_app.add_handler(CommandHandler("export_skatt", cmd_export_skatt))
tg_app.add_handler(CommandHandler("export_all", cmd_export_all))
...
async def on_startup():
    logger.info("Startup: init Telegram-app")
    await tg_app.initialize()
    if WEBHOOK_BASE:
        url = f"{WEBHOOK_BASE}/webhook/{BOT_TOKEN}"
        logger.info("Sätter webhook till %s", url)
