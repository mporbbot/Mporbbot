# main.py
# Mp ORBbot — v2.3 (robust env-token, FastAPI health, Telegram polling)
# Kör som Render Web Service + /health, och läser TELEGRAM-token robust.
...
def read_token() -> Tuple[Optional[str], Optional[str]]:
    ...
    Försök i ordning:
      ...
      2) BOT_TOKEN
      3) TOKEN
      4) .env (om finns) och repetera 1-3
    Returnerar (token, key_used)
    """
    candidates = ["TELEGRAM_BOT_TOKEN", "BOT_TOKEN", "TOKEN"]
    ...
    # Försök .env
    try:
        from dotenv import load_dotenv  # valfri, bara fallback
        if load_dotenv():
            for k in candidates:
                v = os.getenv(k)
                if v and v.strip():
...
class KuCoinPublic:
    ...
    def get_kline(symbol: str, granularity_sec: int, start_ts: Optional[int]=None, end_ts: Optional[int]=None) -> List[List]:
        ...
        params = {"type": KuCoinPublic._gran_to_str(granularity_sec), "symbol": symbol}
...
class Trade:
    ...
    trail_floor_price: Optional[float] = None
    trail_ceiling_price: Optional[float] = None

    def current_rr(self, price: float) -> float:
        return pct_change(self.entry_price, price) if self.side == "LONG" else pct_change(price, self.entry_price)
    ...
    def _apply_dca(self, price: float):
        ...
        total = self.qty * self.entry_price + add_qty * price
        self.qty += add_qty
        self.size_usdt += add_usdt
        self.entry_price = total / self.qty
...
def detect_orb_windows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(1, n):
        ...
        if prev_red_to_green:
            orb_high[i] = h[i]; orb_low[i] = l[i]; orb_side[i] = "LONG"; armed = True; side = "LONG"
        elif prev_green_to_red:
            orb_high[i] = h[i]; orb_low[i] = l[i]; orb_side[i] = "SHORT"; armed = True; side = "SHORT"
        else:
            if armed:
                orb_high[i] = orb_high[i-1]; orb_low[i] = orb_low[i-1]; orb_side[i] = side
            else:
                orb_side[i] = ""
    df = df.copy()
    df["orb_high"], df["orb_low"], df["orb_side"] = orb_high, orb_low, orb_side
    return df
        ...
        for i in range(1, len(df)):
            ...
            if open_trade:
                ...
                if closed:
                    ...
                    pnl = gross - fees
                    trades.append({
                        "entry_time": open_trade.entry_time, "exit_time": row["time"], "side": open_trade.side,
                        "entry": float(open_trade.entry_price), "exit": float(exit_price), "pnl": float(pnl),
                        "reason": reason, "legs": open_trade.dca_legs_done + 1
                    })
                    open_trade = None

            if open_trade is None and isinstance(row["orb_high"], float) and not math.isnan(row["orb_high"]):
                side = row["orb_side"]; trend = row["trend"]
                if (side == "LONG" and trend != 1) or (side == "SHORT" and trend != -1):
                    continue
                ...
                if trigger:
                    ...
                    open_trade = Trade(symbol=symbol, side=side, entry_price=entry_price, size_usdt=MOCK_TRADE_SIZE_USDT,
                                       qty=qty, entry_time=row["time"], fee_rate=DEFAULT_FEE)

        pnl_total = sum(t["pnl"] for t in trades)
        wins = sum(1 for t in trades if t["pnl"] > 0)
        losses = sum(1 for t in trades if t["pnl"] <= 0)
        return {"symbol": symbol, "trades": trades, "summary": {"n_trades": len(trades), "wins": wins, "losses": losses, "pnl_total": pnl_total}}
...
class MpORBbot:
    ...
    async def cmd_mock_trade(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if price is None:
            ...
            return
        side = "LONG"
        trade = Trade(symbol=symbol, side=side, entry_price=price, size_usdt=MOCK_TRADE_SIZE_USDT,
                      qty=MOCK_TRADE_SIZE_USDT / price, entry_time=now_ts(), fee_rate=DEFAULT_FEE)
        exit_price = price * 1.006  # simulera +0.6%
        closed, _, _ = trade.check_exits_and_manage(exit_price, now_ts())
        TradeLogger().log(trade, exit_price, "MOCK", True, now_ts())
        ...
        for sym in symbols:
            try:
                ...
                with open(out_path, "w", newline="", encoding="utf-8") as f:
                    ...
                    for t in res["trades"]:
                        w.writerow([t["entry_time"], t["exit_time"], t["side"], round(t["entry"],8), round(t["exit"],8), round(t["pnl"],6), t["reason"], t["legs"]])
...
def env_keys_present() -> List[str]:
    present = []
    for k in ["TELEGRAM_BOT_TOKEN", "BOT_TOKEN", "TOKEN"]:
        v = os.getenv(k)
        if v and v.strip():
            present.append(k)
    return present
...
@app.get("/health")
async def health():
    # visa inte token-värde, bara vilka nycklar som finns
    return {"status": "ok", "botReady": bool(env_keys_present()), "envKeys": env_keys_present()}

# ================== STARTA BOT I BAKGRUND ==================
_application = None

async def run_bot():
    token, key_used = read_token()
    if not token:
        logger.error("Ingen Telegram-token i miljön. Sätt TELEGRAM_BOT_TOKEN (eller BOT_TOKEN/TOKEN) i Render → Environment.")
        return
    logger.info(f"Telegram-token hittad i env-nyckel: {key_used}")

    bot = MpORBbot()
    application = ApplicationBuilder().token(token).build()
    application.add_handler(CommandHandler("start", bot.cmd_start))
    application.add_handler(CommandHandler("stop", bot.cmd_stop))
    application.add_handler(CommandHandler("status", bot.cmd_status))
    application.add_handler(CommandHandler("set_ai", bot.cmd_set_ai))
    application.add_handler(CommandHandler("help", bot.cmd_help))
    application.add_handler(CommandHandler("export_csv", bot.cmd_export_csv))
    application.add_handler(CommandHandler("mock_trade", bot.cmd_mock_trade))
    application.add_handler(CommandHandler("backtest", bot.cmd_backtest))
