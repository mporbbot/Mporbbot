...
async def kucoin_place_market_order(symbol: str, side: str, amount: float):
    ...
    amount:
      ...
      - För SELL: bas-kvantitet (size)

    Returnerar (ok: bool, err: str)

    Dessutom:
      - vid BUY: fyller LAST_LIVE_ENTRY_INFO[symbol]
      - vid SELL: fyller LAST_LIVE_EXIT_INFO[symbol]
      med summerade fills (size, funds, fee, avg_price).
    """
    if not kucoin_creds_ok():
        return False, "Inga KuCoin-creds satta."
    ...
    if not order_id:
        return False, "OrderId saknas i KuCoin-svar."
...
def open_position(
    ...
    """
    Öppna position.
    - MOCK: qty = STATE.mock_size / price
    - LIVE: använder riktiga fills från LAST_LIVE_ENTRY_INFO[sym]
    """
    if STATE.trade_mode == "live":
        info = LAST_LIVE_ENTRY_INFO.pop(sym, None)
        if info:
            entry_price = float(info.get("avg_price") or price)
            qty = float(info.get("size", 0.0) or 0.0)
            usd_in = float(info.get("funds", 0.0) or 0.0)
            fee_in = float(info.get("fee", 0.0) or 0.0)
...
def close_position(
    ...
    - LIVE: använder verkliga funds/fee från LAST_LIVE_EXIT_INFO[sym]
    - MOCK: modell, men använder pos.usd_in/fee_in.
    """
    if not st.pos:
        return 0.0
    ...
    if STATE.trade_mode == "live":
        info = LAST_LIVE_EXIT_INFO.pop(sym, None)
        if info:
            usd_in = pos.usd_in
            fee_in = pos.fee_in
            usd_out = float(info.get("funds", 0.0) or 0.0)
            fee_out = float(info.get("fee", 0.0) or 0.0)
