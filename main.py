# main.py
# Mp ORBbot – Telegram i huvudtrådens event loop (Render/uvicorn-kompatibelt)
# Kommandon: /status /set_ai /start_mock /start_live /engine_start /engine_stop
#            /symbols /timeframe /export_csv /pnl /reset_pnl /help
# ORB: färgskifte => nytt ORB, 1 trade/ORB, initial SL på ORB-linjen, trailing SL per candle
...
# ======= FastAPI (Render håller processen vid liv) =======
...
def build_application()->Application:
    ...
    app_tg.job_queue.run_repeating(lambda c: [c.bot.send_message(chat_id=CONFIG["chat_id"], text=n) for n in trading_tick(INTERVAL)] if CONFIG["chat_id"] else None, interval=10, first=5)
    app_tg.job_queue.run_repeating(heartbeat_job, interval=HEARTBEAT_SEC, first=5)
    return app_tg

# ======= Kör Telegram i huvudloop + auto-reconnect =======
async def telegram_runner():
    ...
    while True:
        try:
            ...
            print("[startup] Telegram polling startas …")
            # stop_signals=None → inga signalhandlers (bra i uvicorn-loop)
            await application.run_polling(stop_signals=None, allowed_updates=Update.ALL_TYPES)
        except Exception as e:
            print("[fatal] polling kraschade:", e)
            traceback.print_exc()
            await asyncio.sleep(5)   # auto-reconnect efter 5s
        else:
            # run_polling returnerade normalt → börja om (säkerhetsbälte)
...
async def on_startup():
    print("[render] uvicorn startup → starta telegram runner i bakgrunden")
    asyncio.create_task(telegram_runner())
