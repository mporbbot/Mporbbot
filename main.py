...
def main_menu_keyboard() -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton("/tp"), KeyboardButton("/sl")],  # ✅ NEW
...
def tp_buttons(current: float) -> InlineKeyboardMarkup:
    options = [0.60, 0.70, 0.80, 0.90, 1.00]
    row = []
    for x in options:
...
def sl_buttons(current: float) -> InlineKeyboardMarkup:
    options = [0.20, 0.25, 0.30, 0.35, 0.40]
    row = []
    for x in options:
        label = f"{x:.2f}%" + (" ✅" if abs(current - x) < 1e-9 else "")
...
async def open_long(app: Application, symbol: str, reason: str):
    ...
    if CFG.trade_mode == "mock":
        entry_price = mock_entry_price(lvl)
    else:
        entry_price = float(lvl.get("price", 0.0)) or float(lvl.get("ask", 0.0)) or float(lvl.get("bid", 0.0))
    ...
    await notify_trade(app, f"🟢 ENTRY ({tag}) {symbol} @ {entry_price:.6f} | TP={tp:.6f} SL={sl:.6f} | stake={funds:.2f}{extra} | {reason}")
...
async def close_long(app: Application, symbol: str, reason: str):
    ...
    if CFG.trade_mode == "mock":
        exit_price = mock_exit_price(lvl)
    else:
        exit_price = float(lvl.get("price", 0.0)) or float(lvl.get("bid", 0.0)) or float(lvl.get("ask", 0.0))
...
async def evaluate_symbol(app: Application, symbol: str):
    ...
    if symbol in POS:
        lvl = await asyncio.to_thread(KC.get_level1, symbol)
        if CFG.trade_mode == "mock":
            px = mock_exit_price(lvl)
        else:
            px = float(lvl.get("price", 0.0)) or float(lvl.get("bid", 0.0)) or float(lvl.get("ask", 0.0))
        if px >= POS[symbol].tp_price:
            await close_long(app, symbol, reason="TP")
        elif px <= POS[symbol].sl_price:
    ...
    if pullback_ok and bullish_ok:
        ...
        await open_long(app, symbol, reason=reason)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Mock realism: spread via bid/ask + slippage.\n",
...
async def cmd_sl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        v = parse_float(args[0])
        if v is None:
            await update.message.reply_text("Ogiltigt. Ex: /sl 0.25")
            return
...
def build_app() -> Application:
    ...
    app.add_handler(CommandHandler("tp", cmd_tp))  # ✅ NEW
    app.add_handler(CommandHandler("sl", cmd_sl))  # ✅ NEW
