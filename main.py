...
# main_v43.py
# ------------------------------------------------------------
# MP Bot – v43 (trailing stop + threshold control)
# - Reply-keyboard only
# - Mock-handel med avgifter per sida
# - Grid/DCA + TP (kan ersättas av trailing)
# - Long & short
...
# - Trailing stop som följer vinster (PÅ/AV + parametrar)
...
MAX_OPEN_POS = int(os.getenv("MAX_POS", "2"))  # försiktigare än 5

# Grid/DCA (mer konservativt)
GRID_MAX_SAFETY = int(os.getenv("GRID_MAX_SAFETY", "2"))             # var 3
GRID_STEP_MIN_PCT = float(os.getenv("GRID_STEP_MIN_PCT", "0.40"))    # var 0.15
GRID_STEP_MULT = float(os.getenv("GRID_STEP_MULT", "1.2"))           # var 0.5
GRID_SIZE_MULT = float(os.getenv("GRID_SIZE_MULT", "1.2"))           # var 1.5
GRID_TP_PCT = float(os.getenv("GRID_TP_PCT", "0.20"))                # var 0.25
TP_SAFETY_BONUS = float(os.getenv("TP_SAFETY_BONUS", "0.03"))        # var 0.05

# Risk
DD_STOP_PCT = float(os.getenv("DD_STOP_PCT", "2.0"))                 # var 3.0

# Entry tröskel
ENTRY_THRESHOLD = float(os.getenv("ENTRY_THRESHOLD", "0.80"))        # var 0.35

# Trailing (låter vinster rinna)
TRAIL_ON = (os.getenv("TRAIL_ON", "true").lower() in ("1", "true", "on", "yes"))
TRAIL_ATR_MULT = float(os.getenv("TRAIL_ATR_MULT", "2.5"))           # multiplicerar ATR%
TRAIL_MIN_PCT = float(os.getenv("TRAIL_MIN_PCT", "0.30"))            # minsta trailing i %

# AI Model fil – i arbetskatalogen
...
class EngineState:
    ...
    risk_cfg: Dict[str, float] = field(default_factory=lambda: dict(
        ...
        allow_shorts=True,
        trail_on=TRAIL_ON,           # NYTT
        trail_mult=TRAIL_ATR_MULT,   # NYTT
        trail_min=TRAIL_MIN_PCT,     # NYTT
    ))
...
def _enter_leg(sym: str, side: str, price: float, usd_size: float, st: SymState) -> TradeLeg:
    ...
    # sätt initialt TP (används om trailing är AV)
        ...
        try:
            if STATE.engine_on:
                ...
                for sym in STATE.symbols:
                    ...
                    if trail_on:
                        # Trailing exit
                        if (st.pos.side == "LONG" and price <= trail_level) or \
                           (st.pos.side == "SHORT" and price >= trail_level):
                            ...
                            if STATE.chat_id:
                                ...
                                await app.bot.send_message(
                                    STATE.chat_id,
                                    f"🏁 TRAIL EXIT {sym} @ {price:.4f} | Net: {net:+.4f} USDT {mark} | trail={trail_w:.2f}%"
                                )
                            if STATE.ai_on:
                    ...
                    if did_exit:
                        continue  # hoppa vidare till nästa symbol
...
def status_text() -> str:
    ...
    for s in STATE.symbols:
        ...
        if st.pos:
            pos_lines.append(
                f"{s}: {st.pos.side} avg {st.pos.avg_price:.4f} → TP {st.pos.target_price:.4f} | legs {st.pos.safety_count}"
            )
    grid = STATE.grid_cfg
    risk = STATE.risk_cfg
    grid_line = (
        f"Grid: max_safety={grid['max_safety']} "
        f"step_mult={grid['step_mult']} "
        f"step_min%={grid['step_min']} "
        f"size_mult={grid['size_mult']} "
        f"tp%={grid['tp']} (+{grid['tp_bonus']}%/safety)"
    )
    risk_line = (
        f"Risk: dd={risk['dd']}% | max_pos={risk['max_pos']} | shorts={'ON' if risk['allow_shorts'] else 'OFF'} | "
        f"trail={'ON' if risk.get('trail_on') else 'OFF'} (min={risk.get('trail_min',0):.2f}% x{risk.get('trail_mult',1):.2f})"
    )
    lines = [
...
async def cmd_risk(update: Update, _):
    ...
    if len(toks) == 4 and toks[0] == "/risk" and toks[1] == "set":
        ...
        if key in STATE.risk_cfg:
            try:
                ...
                elif key in ("allow_shorts", "trail_on"):
    ...
    msg = (
        ...
        "Ex: /risk set dd 2.5   | /risk set max_pos 3   | /risk set allow_shorts on\n"
...
# registrera handlers (kompakt och robust mot citatteckenstrul)
for name, func in [
    ('start', cmd_start),
    ('status', cmd_status),
...
async def root():
    total = sum(STATE.per_sym[s].realized_pnl_net for s in STATE.symbols)
    return f"MP Bot v43 OK | engine_on={STATE.engine_on} | thr={STATE.entry_threshold:.2f} | pnl_total={total:+.4f}"
