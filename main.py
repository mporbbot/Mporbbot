...
# KuCoin API (behövs bara för live trading; marknadsdata funkar utan)
...
class SymbolState:
    pos_open: bool = False
    direction: Optional[str] = None  # 'long' or 'short' (vi kör long i denna version)
...
# KuCoin clients (globala så vi kan reconnecta)
ku_rest = KuClient(KU_KEY, KU_SECRET, KU_PASSPHRASE)  # funkar för public calls även utan nycklar
...
def ai_filter_ok(symbol: str, price: float) -> bool:
    # enkel placeholder: olika trösklar beroende på läge
    if engine.ai_mode == "aggressiv":
        return True
    if engine.ai_mode == "försiktig":
        # blockera om priset inte rört sig sista minuten (dummy)
        st = sym_state[symbol]
...
def trail_update(symbol: str, price: float):
    ...
    if not (engine.trail_enable and st.pos_open and st.entry_price):
        return
    # Lås vinst enligt regler
    up = (price - st.entry_price) / st.entry_price
    if up >= TRAIL_ARM_PCT:
        # höj stop med step, men minst min_lock
...
async def maybe_open_orb(symbol: str, price: float, ctx: Optional[ContextTypes.DEFAULT_TYPE]=None):
    ...
    if not st.orb_window_active or st.orb_high is None or st.orb_low is None:
        return
    # enkel ORB: bryter över ORB-high => long
    if price > st.orb_high and ai_filter_ok(symbol, price):
        st.pos_open = True
        st.direction = "long"
        st.entry_price = price
        st.stop_price = st.orb_low  # initial SL på ORB-låget
        st.trades_today += 1
        await log_trade(MOCK_LOG if engine.mock_mode else REAL_LOG,
...
# =========== TELEGRAM ===========
# Vi antar en single-user setup; vi sparar chat_id när /start används
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/backtest <symbol> <period> [fee]  ex: /backtest btcusdt 3d 0.001\n"
        "/export_csv – exportera senaste backtest/mock CSV (filer sparas lokalt)\n"
        "/orb_on | /orb_off – slå på/av ORB för alla valda symboler\n"
        "/mock_trade <SYMBOL> – simulerar en trade manuellt\n"
    )
...
async def cmd_export_csv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # I Render kan filuppladdning vara storleksbegränsad; vi svarar med bekräftelse och filnamn
    await update.message.reply_text("CSV-filer finns lokalt i samma katalog:\n- mock_trade_log.csv\n- real_trade_log.csv")

async def cmd_backtest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Förenklad placeholder – hämtar inte historik här för att hålla main.py kompakt
    await update.message.reply_text("Backtest-körning (placeholder). Använd /mock_trade för snabbtest.\nVill du ha full backtestloader från KuCoin API här också, säg till så lägger jag in den.")
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "JA":
        ...
        await update.message.reply_text("⚠️ LIVE-läge begärt. Se till att KUCOIN API-nycklar finns satt. (Annars faller den tillbaka till mock för trades.)")
    else:
...
async def _handle_tick(symbol: str, price: float):
    ...
    # ORB-fönster uppdateras alltid med senaste pris
    update_orb_window(symbol, price)

    # Hantera positioner om motor kör
    if engine.running:
        # Trailing
...
async def _subscribe_ws(loop):
    async def deal_msg(msg):
        # msg ex: {'topic':'/market/ticker:BTC-USDT','data':{'price':'...'}}
        if "data" in msg and "price" in msg["data"]:
            topic = msg.get("topic", "")
            # topic like '/market/ticker:BTC-USDT'
            if ":" in topic:
...
async def ws_manager():
    """Startar WS och reconnectar vid behov."""
    while True:
...
async def rest_poll_fallback():
    """Om WS inte levererar – poll:a REST var 3s som fallback."""
    while True:
        if not ws_connected.is_set():
            try:
                for s in engine.symbols:
                    inst = s.replace("USDT","-USDT")
                    tick = ku_rest.get_ticker(inst)
...
async def market_watchdog(context: ContextTypes.DEFAULT_TYPE):
    """Körs var 10s: varnar/reconnectar om det är tyst i marknadsflödet."""
    now = now_ts()
    stale_symbols = []
    very_stale = False
    for s in engine.symbols:
    ...
    if very_stale and engine.running and engine_chat_id:
        engine.running = False
        await context.bot.send_message(chat_id=engine_chat_id,
            text="⛔ Marknadsdata saknas >5 min. Motorn pausad tills flödet återkommer. Kör /status och starta igen med 'JA' när du ser att ticks rullar.")

# =========== ENGINE LOOP (lätt) ===========
async def engine_loop():
    """Lättvikt – huvuddelen sker i tick-handlern. Här kan vi lägga periodic tasks."""
    while True:
        # exempel: dagsreset vid midnatt
        now = datetime.now(TZ)
