# main_v42.py
# ------------------------------------------------------------
# MP Bot – v42 (stabilitet + AI-sparning)
# - Reply-keyboard
# - Mock-handel (samma logik som v41)
# - AI-modell sparas till ./data/ai_model.pkl (mapp skapas)
# - Robust KuCoin-hämtning med timeout/retries
# - Watchdog som startar om engine-loop om den fastnar
# - Heartbeat till chat var 10:e minut (om engine ON)
# ------------------------------------------------------------
...
# ---- FIX 1: spara lokalt i repo + skapa mapp ----
...
def reply_kb() -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton("/kick")],  # manuell restart av engine
    ]
...
# -----------------------------
# DATA & INDICATORS (med retries)
# -----------------------------
async def _fetch_with_retries(url: str, params: dict, retries: int = 3, timeout_s: float = 6.0):
    delay = 0.6
    last_exc = None
...
async def get_klines(symbol: str, tf: str, limit: int = 60):
    """Returnerar [(ts_ms, open, high, low, close)] äldst->nyast."""
...
async def _engine_once(app: Application):
    """Ett varv. Lätt att tidsbegränsa från watchdog."""
    # öppna nya
    ...
    if len(open_syms) < STATE.risk_cfg["max_pos"]:
        for sym in STATE.symbols:
            ...
            if score > 0.35:
                ...
                if STATE.chat_id:
                    await app.bot.send_message(STATE.chat_id,
                        f"🔻 ENTRY {sym} SHORT @ {st.pos.legs[-1].price:.4f} | TP {st.pos.target_price:.4f}")
                open_syms.append(sym)
                if len(open_syms) >= STATE.risk_cfg["max_pos"]:
                    break
...
async def engine_loop(app: Application):
    """Huvudloop som uppdaterar heartbeat och fångar fel."""
    await asyncio.sleep(2)
    while True:
        try:
            if STATE.engine_on:
                # tidsbegränsa ett helt varv så inget kan fastna för evigt
                await asyncio.wait_for(_engine_once(app), timeout=25.0)
                STATE.last_loop_at = datetime.now(timezone.utc)
            await asyncio.sleep(2)
        except asyncio.TimeoutError:
            # ett varv låste sig – fortsätt nästa
            STATE.last_loop_at = datetime.now(timezone.utc)
        except Exception as e:
            if STATE.chat_id:
                try:
...
async def watchdog_loop(app: Application):
    """Startar om engine_loop om den verkar hänga."""
    while True:
        try:
            await asyncio.sleep(30)
            if not STATE.engine_on:
                continue
            stale_for = datetime.now(timezone.utc) - STATE.last_loop_at
            if stale_for > timedelta(minutes=2):
...
async def heartbeat_loop(app: Application):
    """Info var 10:e minut så man ser att allt lever."""
    while True:
        try:
...
def status_text() -> str:
    ...
    lines = [
        ...
        (f"Grid: max_safety={grid['max_safety']} step_mult={grid['step_mult']} "
         f"step_min%={grid['step_min']} size_mult={grid['size_mult']} tp%={grid['tp']} (+{grid['tp_bonus']}%/safety)"),
...
async def cmd_risk(update: Update, _):
    ...
    await tg_app.bot.send_message(STATE.chat_id,
        (f"Risk:\n  dd={r['dd']}%\n  max_pos={r['max_pos']}\n  shorts={'ON' if r['allow_shorts'] else 'OFF'}\n"
         "Ex: /risk set dd 2.0   | /risk set max_pos 8   | /risk set allow_shorts on"),
        reply_markup=reply_kb())
