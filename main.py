...
#   /risk              -> snabbknappar för size, TP, SL, max_pos
#
...
KUCOIN_API_KEY_VERSION = os.getenv("KUCOIN_API_KEY_VERSION", "3")  # du har V3
...
async def cmd_risk(update: Update, _):
    """
    /risk set ...  -> avancerat
    /risk          -> snabbknappar (size, TP, SL, max_pos)
    """
    STATE.chat_id = update.effective_chat.id
    toks = update.message.text.strip().split()
    ...
    kb = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton("TP 0.3%", callback_data="risk_tp_03"),
                InlineKeyboardButton("TP 0.5%", callback_data="risk_tp_05"),
                InlineKeyboardButton("TP 1.0%", callback_data="risk_tp_10"),
            ],
            [
                InlineKeyboardButton("SL 0.5%", callback_data="risk_sl_05"),
                InlineKeyboardButton("SL 1.0%", callback_data="risk_sl_10"),
                InlineKeyboardButton("SL 2.0%", callback_data="risk_sl_20"),
            ],
            [
...
async def cmd_mode(update: Update, _):
    ...
    if target == "live":
        ...
        # /mode live JA (fallback via text)
        if not kucoin_creds_ok():
            await tg_app.bot.send_message(
                STATE.chat_id,
...
async def on_button(update: Update, _):
    ...
    # Risk TP snabbval (risk_tp_03 => 0.3 osv)
    if data.startswith("risk_tp_"):
        await query.answer()
        raw = data.split("_")[-1]  # t.ex. "03"
        try:
            val = float(raw) / 10.0
        except Exception:  # noqa: BLE001
            val = STATE.tp_pct
    ...
    if data.startswith("risk_maxpos_"):
        ...
        STATE.max_pos = mp
        await query.edit_message_text(
            f"Max antal öppna positioner uppdaterat till {mp}."
        )
        return
...
async def on_startup():
    # enkel debug på längder (inte värden)
    if kucoin_creds_ok():
        print(
            "[KUCOIN DEBUG]",
        ...
        print("[KUCOIN DEBUG] credentials INCOMPLETE")
