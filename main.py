# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# MP Bot – v51 (Hybrid Momentum + Mean Reversion, MOCK only)
# ------------------------------------------------------------
...
# - Loss-guard: pausar nya entries efter X förlusttrades i rad
...
# Dessa är nu satta efter dina "bra" settings
MOCK_SIZE_USDT = float(os.getenv("MOCK_SIZE_USDT", "50"))
FEE_PER_SIDE = float(os.getenv("FEE_PER_SIDE", "0.001"))       # 0.10%

# Risk/Reward defaults
TP_PCT = float(os.getenv("TP_PCT", "0.3"))       # take-profit 0.30 %
SL_PCT = float(os.getenv("SL_PCT", "0.5"))       # stop-loss 0.50 %
...
# Loss-guard defaults
LOSS_GUARD_ON_DEFAULT = (os.getenv("LOSS_GUARD_ON", "true").lower()
                         in ("1", "true", "on", "yes"))
    ...
    while True:
        try:
            if STATE.engine_on:
                ...
                # Är loss-guard aktiv?
                paused = False
                if (STATE.loss_guard_on and STATE.loss_guard_until
                        and now < STATE.loss_guard_until):
                    paused = True
                ...
                for sym in STATE.symbols:
                    ...
                    if move_pct >= STATE.tp_pct and not pos.trailing:
                        ...
                        if STATE.loss_guard_on:
                            if net < 0:
                                STATE.loss_guard_streak += 1
                            else:
                                STATE.loss_guard_streak = 0
                        continue
                ...
                # Aktivera loss-guard om streak nått N
                if (STATE.loss_guard_on and
                    STATE.loss_guard_streak >= STATE.loss_guard_n and
                    (STATE.loss_guard_until is None or now >= STATE.loss_guard_until)):
                    STATE.loss_guard_until = now + timedelta(
                        minutes=STATE.loss_guard_pause_min
                    )
                    STATE.loss_guard_streak = 0
                    if STATE.chat_id:
                        t_str = STATE.loss_guard_until.strftime("%H:%M")
                        await app.bot.send_message(
                            STATE.chat_id,
                            f"🛑 Loss-guard aktiv – pausar nya entries till {t_str} (UTC)."
                        )
...
def status_text() -> str:
    ...
    regime_line = ("Regime: AUTO (trend + mean reversion)"
                   if STATE.regime_auto else "Regime: TREND only")
    mr_line = (f"MR: {'ON' if STATE.mr_on else 'OFF'} | "
               f"dev={STATE.mr_dev_pct:.2f}% | "
    ...
    if STATE.loss_guard_on:
        extra = ""
        now = datetime.now(timezone.utc)
        if STATE.loss_guard_until and STATE.loss_guard_until > now:
            extra = f" (PAUSED till {STATE.loss_guard_until.strftime('%H:%M')})"
        loss_line = (f"Loss-guard: ON | N={STATE.loss_guard_n} | "
                     f"pause={STATE.loss_guard_pause_min}m{extra}")
    else:
        loss_line = "Loss-guard: OFF"
    ...
    lines = [
        ...
        (
            "Risk: tp={:.2f}% | sl={:.2f}% | trail_start={:.2f}% | "
            "trail={:.2f}% | shorts={} | max_pos={}"
        ).format(
            STATE.tp_pct,
            STATE.sl_pct,
            STATE.trail_start_pct,
            STATE.trail_pct,
            "ON" if STATE.allow_shorts else "OFF",
            STATE.max_pos,
        ),
...
async def health():
    ...
    return {
        ...
        "loss_guard_pause_min": STATE.loss_guard_pause_min,
    }
