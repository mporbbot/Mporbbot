...
# Symboler du vill handla (GARI är BORTA)
SYMBOLS = [
    "BTC-USDT",
    "ETH-USDT",
    "ADA-USDT",
...
#########################################################
# ENKELT LIKVIDITETS-FILTER (pris & spread)
#########################################################

def passes_liquidity_filters(symbol):
    """
    Returnerar (ok, last_price, spread_pct)
    ok = False om:
      - pris < 0.01 USDT
      - spread > 1 %
    """
    try:
        ...
        best_bid = float(ticker.get("bestBid", 0) or 0)
        best_ask = float(ticker.get("bestAsk", 0) or 0)
        last_price = float(ticker.get("price", 0) or 0)

        if last_price <= 0:
            return False, last_price, None

        spread_pct = (
            (best_ask - best_bid) / last_price
            if best_ask > 0 and best_bid > 0 else 999
        )

        if last_price < 0.01:
            return False, last_price, spread_pct

        if spread_pct > 0.01:
            return False, last_price, spread_pct

        return True, last_price, spread_pct
    except Exception as e:
        print(f"[{symbol}] Fel i passes_liquidity_filters: {e}")
...
#########################################################
# TRADING-LOOP (en tråd per symbol)
#########################################################
...
def trading_loop(symbol):
    ...
    while True:
        ...
        try:
            ok, last_price, spread = passes_liquidity_filters(symbol)
            if not ok:
                LAST_STATUS = (
                    f"{symbol} | LIQ FILTER: pris={last_price:.6f}, "
                    f"spread={spread*100 if spread is not None else 'N/A'}%"
                )
                time.sleep(5)
                continue
...
def main_keyboard():
    return ReplyKeyboardMarkup([
        ...
        ["/pnl_today"],
        ["/risk 25", "/sl 1.0"],
        ["/threshold_10", "/threshold_20", "/threshold_30"],
        ["/testbuy ADA-USDT"],
...
def cmd_pnl_today(update, context: CallbackContext):
    update.message.reply_text(f"Dagens PnL: {round(daily_pnl, 4)} USDT")
...
def cmd_sl(update, context: CallbackContext):
    ...
    if len(parts) == 2:
        try:
            ...
            update.message.reply_text(f"Fallback-SL ändrad till {STOP_LOSS_PCT} %")
        except:
...
def cmd_risk(update, context: CallbackContext):
    ...
    if len(parts) == 2:
        try:
            ...
            update.message.reply_text(f"Threshold ändrad till {THRESHOLD}")
        except:
...
def cmd_testbuy(update, context: CallbackContext):
    try:
        ...
        if not ok:
            update.message.reply_text(
                f"{symbol} passerar inte likviditetsfilter (pris {last_price}, spread {spread})."
            )
            return
...
def cmd_export_mock(update, context: CallbackContext):
    if os.path.isfile(MOCK_LOG):
        ...
        update.message.reply_text("Ingen mock-logfil ännu.")
...
def cmd_export_live(update, context: CallbackContext):
    if os.path.isfile(LIVE_LOG):
        ...
        update.message.reply_text("Ingen live-logfil ännu.")
...
def main():
    ...
    dp.add_handler(CommandHandler("threshold_10", cmd_threshold_10))
    dp.add_handler(CommandHandler("threshold_20", cmd_threshold_20))
    dp.add_handler(CommandHandler("threshold_30", cmd_threshold_30))
