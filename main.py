...
#########################################################
# LIKVIDITETS-FILTER (pris & spread)
#########################################################
...
def passes_liquidity_filters(symbol):
    ...
    try:
        ...
        # Filter A: pris under 0.01 USDT
        if last_price < 0.01:
            return False, last_price, spread_pct

        # Filter B: spread större än 1 %
        if spread_pct > 0.01:
            return False, last_price, spread_pct
...
def trading_loop(symbol):
    ...
    while True:
        ...
        try:
            ...
            if not ok:
                LAST_STATUS = (
                    f"{symbol} | LIQUIDITY FILTER: pris={last_price:.6f}, "
...
def set_threshold(update, context: CallbackContext, value):
    ...
    THRESHOLD = value
    update.message.reply_text(
        f"⚙️ Threshold ändrad till {THRESHOLD}",
        parse_mode="Markdown"
    )
