# main.py
# Mp ORBbot - Strategy: Trend Break & Hold (5m), LONG only
# Keeps Telegram menus & CSV logging. Mock includes bid/ask + slippage. Live ready (keys required).
#
# Commands:
#   /engine_on /engine_off /status /pnl /reset_pnl /close_all
#   /threshold  (now used as RETEST band % around breakout level)
#   /stake      (buttons)
#   /tp         (buttons)
#   /sl         (buttons)
#   /coins      (buttons)
...
# /threshold now defines retest band around breakout level (in %)
RETEST_BAND_PCT_DEFAULT = 0.50

COOLDOWN_SECONDS_DEFAULT = 180
MAX_POSITIONS_DEFAULT = 5
STAKE_USDT_DEFAULT = 30.0

# Trailing (enabled after profit)
TRAIL_ACTIVATE_PCT = 0.40   # activate after +0.40%
TRAIL_DISTANCE_PCT = 0.25   # trail by 0.25%

# Pending retest expiration
RETEST_EXPIRE_SECONDS = 60 * 45  # 45 minutes

# MOCK realism defaults
MOCK_USE_BID_ASK_DEFAULT = True
MOCK_SLIPPAGE_PCT_DEFAULT = 0.02  # 0.02% extra friction on top of bid/ask
...
class BotConfig:
    ...
    entry_mode: str = "retest"  # break | retest

    # Used as retest band around breakout level
    threshold_pct: float = RETEST_BAND_PCT_DEFAULT
    ...
    mock_slippage_pct: float = MOCK_SLIPPAGE_PCT_DEFAULT  # percent
    ...
    def from_json(d: Dict[str, Any]) -> "BotConfig":
        ...
        cfg.sl_pct = float(clamp(float(cfg.sl_pct), 0.10, 2.00))
...
class KuCoinClient:
    ...
    def get_klines(self, symbol: str, ktype: str, start_ts: int, end_ts: int) -> List[Dict[str, float]]:
        """
        ktype examples: '1min', '5min', '15min'
        returns list sorted asc by time
        """
...
LAST_5M_T: Dict[str, int] = {}
COOLDOWN_UNTIL: Dict[str, int] = {}

# pending retest: symbol -> {"level": float, "set_ts": int, "expire_ts": int}
PENDING_RETEST: Dict[str, Dict[str, Any]] = {}
...
def threshold_buttons(current: float) -> InlineKeyboardMarkup:
    # used as RETEST band %
    options = [0.20, 0.30, 0.40, 0.50, 0.70, 1.00]
    row = []
    for x in options:
...
def tp_buttons(current: float) -> InlineKeyboardMarkup:
    options = [0.60, 0.70, 0.80, 0.90, 1.00, 1.20, 1.50]
    row = []
    for x in options:
        label = f"{x:.2f}%" + (" ✅" if abs(current - x) < 1e-9 else "")
...
def sl_buttons(current: float) -> InlineKeyboardMarkup:
    options = [0.25, 0.30, 0.35, 0.40, 0.45, 0.50]
    row = []
    for x in options:
        label = f"{x:.2f}%" + (" ✅" if abs(current - x) < 1e-9 else "")
...
def _choose_sl(entry_price: float, breakout_level: float) -> float:
    """
    SL = tighter of:
      - fixed percent SL from entry
      - under breakout level with small buffer
    Tighter means: higher price (closer to entry) for LONG.
    """
    sl_pct_price = entry_price * (1.0 - CFG.sl_pct / 100.0)
    sl_level_price = breakout_level * (1.0 - 0.10 / 100.0)  # 0.10% under level
    return max(sl_pct_price, sl_level_price)


async def open_long(app: Application, symbol: str, breakout_level: float, reason: str):
    ...
    if CFG.trade_mode == "mock":
        entry_price = mock_entry_price(lvl1)
    else:
        entry_price = float(lvl1.get("price", 0.0)) or float(lvl1.get("ask", 0.0)) or float(lvl1.get("bid", 0.0))

    qty = funds / entry_price if entry_price > 0 else 0.0
    tp = entry_price * (1.0 + CFG.tp_pct / 100.0)
    sl = _choose_sl(entry_price, breakout_level)
    ...
    # clear pending retest once in
    PENDING_RETEST.pop(symbol, None)

    tag = "LIVE" if (CFG.trade_mode == "live" and buy_id) else "MOCK"
    extra = ""
    if CFG.trade_mode == "mock":
        extra = f" | mock(bid/ask={CFG.mock_use_bid_ask}, slip={CFG.mock_slippage_pct:.2f}%)"
    await notify_trade(
        app,
        f"🟢 ENTRY ({tag}) {symbol} @ {entry_price:.6f} | lvl={breakout_level:.6f} | TP={tp:.6f} SL={sl:.6f} "
        f"| TF=5m | mode={CFG.entry_mode.upper()} | {reason}{extra}"
    )


async def close_long(app: Application, symbol: str, reason: str):
    ...
    if CFG.trade_mode == "mock":
        exit_price = mock_exit_price(lvl1)
    else:
        exit_price = float(lvl1.get("price", 0.0)) or float(lvl1.get("bid", 0.0)) or float(lvl1.get("ask", 0.0))

    POS.pop(symbol, None)

    gross = (exit_price - p.entry_price) * p.qty
    fee = calc_fees(p.entry_price, exit_price, p.qty, CFG.fee_rate)
    net = gross - fee
    ...
    if CFG.trade_mode == "mock":
        extra = f" | mock(bid/ask={CFG.mock_use_bid_ask}, slip={CFG.mock_slippage_pct:.2f}%)"
    await notify_trade(app, f"{emoji} EXIT {symbol} @ {exit_price:.6f} | Net {net:+.4f} USDT | {reason}{extra}")


async def manage_open_position(app: Application, symbol: str):
    """
    Manage TP/SL/Trailing on every loop (not only on new candle).
    """
    if symbol not in POS:
        return
    p = POS[symbol]
    ...
    lvl1 = await asyncio.to_thread(KC.get_level1, symbol)
    if CFG.trade_mode == "mock":
        px = mock_exit_price(lvl1)
    else:
        px = float(lvl1.get("price", 0.0)) or float(lvl1.get("bid", 0.0)) or float(lvl1.get("ask", 0.0))

    # update peak
    if px > p.peak_price:
        p.peak_price = px

    # activate trailing after +TRAIL_ACTIVATE_PCT
    if not p.trailing_active:
        if px >= p.entry_price * (1.0 + TRAIL_ACTIVATE_PCT / 100.0):
            p.trailing_active = True
            p.trailing_stop = p.peak_price * (1.0 - TRAIL_DISTANCE_PCT / 100.0)
    else:
        # trail up with peak
        new_stop = p.peak_price * (1.0 - TRAIL_DISTANCE_PCT / 100.0)
        if new_stop > p.trailing_stop:
            p.trailing_stop = new_stop

    # hard exits
    if px >= p.tp_price:
        await close_long(app, symbol, reason="TP")
        return
    if px <= p.sl_price:
        await close_long(app, symbol, reason="SL")
        return
    if p.trailing_active and p.trailing_stop > 0 and px <= p.trailing_stop:
        await close_long(app, symbol, reason="TRAIL")
        return
...
async def evaluate_symbol(app: Application, symbol: str):
    ...
    Strategy:
      - Signal timeframe: 5m
      - Trend filter: EMA50 > EMA200 and EMA50 slope up
      - Breakout: close over highest high of previous BREAKOUT_LOOKBACK candles
      - Entry:
          - break: entry immediately on breakout close
          - retest: set pending level; enter when price returns into band around breakout level
    """
    # 1) Always manage open positions each loop
    if symbol in POS:
        await manage_open_position(app, symbol)
        return

    if in_cooldown(symbol) or not can_open_new_position():
        return

    # 2) If pending retest exists, check retest trigger on live price (every loop)
    pend = PENDING_RETEST.get(symbol)
    if pend:
        now = int(time.time())
        if now >= int(pend["expire_ts"]):
            PENDING_RETEST.pop(symbol, None)
        else:
            lvl1 = await asyncio.to_thread(KC.get_level1, symbol)
            px = mock_entry_price(lvl1) if CFG.trade_mode == "mock" else float(lvl1.get("price", 0.0)) or float(lvl1.get("ask", 0.0)) or float(lvl1.get("bid", 0.0))
            level = float(pend["level"])
            band = CFG.threshold_pct / 100.0
            lo = level * (1.0 - band)
            hi = level * (1.0 + band)
            # "retest" means price comes back into the band around level
            if lo <= px <= hi:
                await open_long(app, symbol, breakout_level=level, reason=f"RETEST hit band [{lo:.6f}-{hi:.6f}]")
        return

    # 3) Fetch 5m candles to compute signal
    end = int(time.time())
    # Need enough candles for EMA200; 5m candles: ~300 candles = 25h
    start = end - (60 * 60 * 30)  # 30 hours
    candles = await asyncio.to_thread(KC.get_klines, symbol, TF_SIGNAL, start, end)
    if len(candles) < (EMA_SLOW + 30):
        return

    last_t = candles[-1]["t"]
    if LAST_5M_T.get(symbol) == last_t:
        return
    LAST_5M_T[symbol] = last_t
    ...
    # EMA50 slope: compare EMA50 now vs EMA50 6 candles ago (30 minutes)
    ema50_prev = ema_last(closes[:-6], EMA_FAST) if len(closes) > (EMA_FAST + 10) else None
    if ema50_prev is None:
        return

    trend_ok = (ema50 > ema200) and (ema50 > ema50_prev)
    if not trend_ok:
        return

    # Breakout level: highest high of previous N candles (exclude current candle)
    if len(highs) < BREAKOUT_LOOKBACK + 2:
        return
    level = max(highs[-(BREAKOUT_LOOKBACK + 1):-1])  # prev N candles
    last_close = closes[-1]

    # Breakout confirmation: close above level
    if not (last_close > level):
        return

    # Entry decision
    if CFG.entry_mode == "break":
        await open_long(app, symbol, breakout_level=level, reason=f"BREAK close>{level:.6f} (EMA50>EMA200 & slope up)")
    else:
        # retest mode: set pending retest
        now = int(time.time())
        PENDING_RETEST[symbol] = {
            "level": float(level),
            "set_ts": now,
            "expire_ts": now + RETEST_EXPIRE_SECONDS,
        }
        await notify_trade(app, f"🟡 PENDING RETEST {symbol} | lvl={level:.6f} | band=±{CFG.threshold_pct:.2f}% | expires {RETEST_EXPIRE_SECONDS//60}m")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Mp ORBbot är igång.\n"
        "Strategi: Trend Break & Hold (5m) LONG only.\n"
        "• EMA50>EMA200 + EMA50 slope up\n"
        "• Breakout: close över senaste local high\n"
        "• Entry mode: BREAK eller RETEST (nu: %s)\n"
        "• /threshold = retest-band (%% runt breakout level)\n" % CFG.entry_mode.upper(),
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        f"Strategy: Trend Break & Hold (TF=5m) | mode={CFG.entry_mode.upper()}",
        f"Trade mode: {CFG.trade_mode} (live keys: {'OK' if KC.has_keys() else 'missing'})",
        f"Retest band (/threshold): ±{CFG.threshold_pct:.2f}%",
        f"Stake per trade: {CFG.stake_usdt:.2f} USDT",
        f"TP/SL: {CFG.tp_pct:.2f}% / {CFG.sl_pct:.2f}%",
        f"Trailing: activate +{TRAIL_ACTIVATE_PCT:.2f}% | dist {TRAIL_DISTANCE_PCT:.2f}%",
        ...
        f"Pending retests: {list(PENDING_RETEST.keys()) if PENDING_RETEST else 'none'}",
...
async def cmd_close_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym in list(POS.keys()):
        try:
            await close_long(context.application, sym, reason="CLOSE_ALL")
        except Exception:
            pass
...
async def cmd_threshold(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if v is None:
            await update.message.reply_text("Ogiltigt värde. Ex: /threshold 0.5")
            return
        CFG.threshold_pct = float(round(clamp(v, 0.05, 2.0), 4))
        save_config()
        await update.message.reply_text(f"✅ Retest-band satt till: ±{CFG.threshold_pct:.2f}%")
        return
    await update.message.reply_text(
        f"Retest-band nu: ±{CFG.threshold_pct:.2f}%\nVälj:",
...
async def cmd_tp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        v = parse_float(args[0])
        if v is None:
            await update.message.reply_text("Ogiltigt. Ex: /tp 0.9")
            return
...
async def cmd_sl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        v = parse_float(args[0])
        if v is None:
            await update.message.reply_text("Ogiltigt. Ex: /sl 0.4")
            return
        CFG.sl_pct = float(round(clamp(v, 0.10, 2.00), 4))
        save_config()
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("thr:"):
        ...
        CFG.threshold_pct = float(round(clamp(v, 0.05, 2.0), 4))
        save_config()
        await q.edit_message_text(f"✅ Retest-band satt till: ±{CFG.threshold_pct:.2f}%")
        return
...
def main():
    ...
    print("Mp ORBbot running (Trend Break & Hold 5m)...")
