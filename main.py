...
#
# Core idea:
#   1) If spread >= min_spread: place LIMIT BUY at bid (postOnly).
#   2) When filled -> place LIMIT SELL at ask (postOnly) targeting profit.
#   3) If position goes too negative (panic stop) -> MARKET SELL to exit.
#   4) Inventory cap per coin prevents over-exposure.
#   5) Timeout / cancel-replace to avoid stale orders.
#
# Telegram controls (kept simple):
#   /threshold = MIN SPREAD % required to quote (e.g. 0.35)
#   /tp        = PROFIT TARGET % (per round trip, e.g. 0.15)
#   /sl        = PANIC STOP % (max loss from entry, e.g. 0.60)
#   /stake     = stake USDT per cycle (you can change anytime)
...
# Strategy defaults (spread market making)
MIN_SPREAD_PCT_DEFAULT = 0.35     # /threshold
PROFIT_TARGET_PCT_DEFAULT = 0.15  # /tp (net target-ish)
PANIC_STOP_PCT_DEFAULT = 0.60     # /sl (max loss from entry before market exit)

STAKE_USDT_DEFAULT = 30.0
MAX_POSITIONS_DEFAULT = 10  # allows multiple coins simultaneously
COOLDOWN_SECONDS_DEFAULT = 2

# Inventory cap per coin (USDT notional)
INVENTORY_CAP_USDT_DEFAULT = 60.0  # allows up to 2 x 30USDT cycles per coin

# Order mgmt
ORDER_TIMEOUT_SEC_DEFAULT = 20     # cancel if not filled
REQUOTE_SEC_DEFAULT = 5            # cancel/replace cadence for stale orders
...
class BotConfig:
    ...
    # Cap/timeout (kept internal defaults; can be edited in config.json anytime)
...
class KuCoinClient:
    ...
    def cancel_order(self, order_id: str) -> None:
        ...
        # 404/400 can happen if already done; ignore
...
def inventory_value_usdt(symbol: str, lvl: Dict[str, float], st: MMState) -> float:
    # approximate at bid
    bid = float(lvl.get("bid", 0.0))
    return max(0.0, st.inv_qty) * bid

# -----------------------------
# Live market order (panic)
# -----------------------------
def kucoin_market_sell(symbol: str, size: float, client: KuCoinClient) -> str:
    # KuCoin: market sell uses size
    path = "/api/v1/orders"
    payload = {
        "clientOid": str(ts_ms()),
        "side": "sell",
...
async def check_fill_sell(app: Application, st: MMState) -> bool:
    ...
    if (not is_active) and deal_size > 0:
        ...
        # PnL for the sold qty (assume we sell all inventory in this cycle)
        ...
        log_trade(
            ...
            net_pnl=net,
            reason="SPREAD_CYCLE",
            buy_order_id="",  # could store last buy id if you want
...
async def place_buy_if_ok(app: Application, st: MMState, lvl: Dict[str, float]):
    ...
    # Inventory cap check (per coin)
    cap = float(CFG.inventory_cap_usdt)
    if cap > 0:
        inv_val = inventory_value_usdt(st.symbol, lvl, st)
        if inv_val >= cap:
            return

    # size from stake and bid
    ...
    # live: place postOnly limit at bid
    try:
...
async def place_sell_if_have_inventory(app: Application, st: MMState, lvl: Dict[str, float]):
    ...
    # Panic stop: if price goes too far against us -> market sell
    panic = float(CFG.sl_pct) / 100.0
    if bid <= entry * (1.0 - panic):
        if CFG.trade_mode == "mock":
            # mock market exit at bid
            qty = st.inv_qty
            exit_p = bid
            gross = (exit_p - entry) * qty
            fee = calc_fees(entry, exit_p, qty, CFG.fee_rate)
            net = gross - fee
            ...
            log_trade(
                ...
                fee_paid=fee,
                net_pnl=net,
                reason="PANIC_EXIT_MOCK",
            )
            CFG.total_net_pnl += net
            CFG.trades += 1
            save_config()
            await notify(app, f"🟥 PANIC EXIT (MOCK) {st.symbol} qty={qty:.6f} @ {exit_p:.6f} | NET {net:+.4f} USDT")
        ...
        # We do not know exact fill price here without fills endpoint; approximate using bid for logging
        qty = st.inv_qty
        exit_p = bid
        gross = (exit_p - entry) * qty
        fee = calc_fees(entry, exit_p, qty, CFG.fee_rate)
    ...
    # We want to sell at ask when possible, but only if it meets target
    if ask < target_px:
        # not yet profitable enough; wait
        return

    # place sell at current ask (postOnly). If ask moves, we'll cancel/replace by timeout/requote logic.
    price = ask
    size = st.inv_qty
    ...
    if CFG.trade_mode == "mock":
        ...
        log_trade(
            ...
            fee_paid=fee,
            net_pnl=net,
            reason="SPREAD_CYCLE_MOCK",
        )
        CFG.total_net_pnl += net
        CFG.trades += 1
        save_config()

        await notify(app, f"🎯 MOCK SELL {st.symbol} qty={qty:.6f} @ ask={exit_p:.6f} | NET {net:+.4f} USDT")
        st.inv_qty = 0.0
        st.inv_entry_price = 0.0
        st.inv_entry_ts = 0
        st.cooldown_until = now_s() + CFG.cooldown_seconds
    ...
    try:
        ...
        st.sell_price = price
        st.sell_size = size
        await notify(app, f"🎯 SELL order {st.symbol} price={price:.6f} size={size:.6f} (target met)")
    except Exception as e:
...
async def requote_stale_orders(app: Application, st: MMState, lvl: Dict[str, float]):
    # cancel/replace if order is old and market moved
    # BUY: if bid changed and order older than requote_sec
    if st.buy_order_id and (now_s() - st.buy_ts) >= CFG.requote_sec:
        new_bid = float(lvl["bid"])
        # if price moved meaningfully (tiny threshold), re-quote
        if new_bid > 0 and abs(new_bid - st.buy_price) / st.buy_price > 0.0002:
            oid = st.buy_order_id
            st.buy_order_id = ""
            await asyncio.to_thread(KC.cancel_order, oid)
            await notify(app, f"🔁 Requote BUY {st.symbol} cancel {oid}")
            # place new buy at new bid
            try:
    ...
    # SELL: if ask changed and order older than requote_sec AND still meets target
    if st.sell_order_id and (now_s() - st.sell_ts) >= CFG.requote_sec and st.inv_qty > 0:
        new_ask = float(lvl["ask"])
        entry = st.inv_entry_price
        target = float(CFG.tp_pct) / 100.0
        if new_ask >= entry * (1.0 + target) and new_ask > 0:
...
async def evaluate_symbol(app: Application, symbol: str):
    ...
    # if inventory -> try sell or panic
...
async def engine_background_loop(app: Application):
    while True:
        try:
            if CFG.engine_on:
                # cap global positions roughly by counting coins with inv or pending orders
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Mp ORBbot är igång.\n"
        "Strategi: Inventory-first Spread Market Maker (limit på bid -> sell på ask).\n"
        "Styrning:\n"
        "• /threshold = min spread % för att lägga buy\n"
        "• /tp = target % (profit)\n"
        "• /sl = panic stop % (max minus -> stäng)\n"
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        f"Stake per cycle: {CFG.stake_usdt:.2f} USDT (du kan ändra via /stake)",
...
async def cmd_close_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # cancel orders + panic exit inventory (live) / close mock at bid
    ...
    for sym, st in list(MM.items()):
        ...
        if st.inv_qty > 0:
            ...
            if CFG.trade_mode == "mock":
                ...
                fee = calc_fees(entry, exit_p, qty, CFG.fee_rate)
                net = gross - fee
                log_trade(True, "KuCoin", sym, "MM_LONG", qty, CFG.stake_usdt, entry, exit_p, gross, fee, net, "FORCE_CLOSE_MOCK")
                CFG.total_net_pnl += net
                CFG.trades += 1
                save_config()
            else:
                if KC.has_keys():
                    try:
                        await asyncio.to_thread(kucoin_market_sell, sym, qty, KC)
                    except Exception:
                        pass
                # approximate log
                exit_p = bid
                ...
                fee = calc_fees(entry, exit_p, qty, CFG.fee_rate)
                net = gross - fee
                log_trade(False, "KuCoin", sym, "MM_LONG", qty, CFG.stake_usdt, entry, exit_p, gross, fee, net, "FORCE_CLOSE_LIVE_APPROX")
                CFG.total_net_pnl += net
                CFG.trades += 1
                save_config()
...
def main():
    ...
    print("Mp ORBbot running (Spread Market Maker, inventory-first)...")
