...
# Single file main.py (UPDATED) — PART 1/2
# =========================
...
# Risk / exits
DEFAULT_TP_PCT = 0.60              # optional TP (hard exit) if trailing not armed
DEFAULT_SL_PCT = 0.35

DEFAULT_TRAIL_ACTIVATE_PCT = 0.35  # arms trailing when move >= this %
DEFAULT_TRAIL_DIST_PCT = 0.20      # trailing stop distance in %
...
def parse_float_arg(context: ContextTypes.DEFAULT_TYPE, default: Optional[float] = None) -> Optional[float]:
    if not context.args:
        return default
    try:
        return float(str(context.args[0]).replace(",", "."))
    except Exception:
        return None

def parse_on_off_arg(context: ContextTypes.DEFAULT_TYPE) -> Optional[bool]:
    if not context.args:
        return None
    v = str(context.args[0]).strip().lower()
    if v in ("on", "1", "true", "yes", "ja"):
        return True
    if v in ("off", "0", "false", "no", "nej"):
        return False
    return None
...
class MeanReversionEngine:
    ...
    def update_positions(self) -> List[str]:
        ...
        for sym, pos in list(self.positions.items()):
            ...
            # Trailing arm when move >= trail_activate_pct
            ...
            if (not pos.trail_active) and move_pct >= self.settings.trail_activate_pct:
                ...
                msgs.append(
                    f"🟡 TRAIL ARMED {sym} | move={move_pct:.2f}% | trail_stop={pos.trail_stop:.6f}"
                )
                continue

            # Optional hard TP (only if trailing is NOT active)
            if (not pos.trail_active) and move_pct >= self.settings.tp_pct:
...
Den här delen innehåller allt som saknades/fixades:

✅ `/trail_activate X` och `/trail_dist Y` (nu funkar de)  
✅ `/mr_band X` och `/mr_exit X` (skriv valfritt värde – inte bara presets)  
✅ `/mr_trend on/off` (fixad och fungerar)  
✅ presets kvar men du kan också skriva egna värden  
✅ status visar rätt och hjälper dig debugga

---

## ✅ main.py (DEL 2/2) — klistra in DIREKT efter DEL 1/2
...
```python
    ...
    def status_text(self) -> str:
        ...
        return (
            ...
            f"<b>TP</b> (/tp): {fmt_pct(s.tp_pct)} (hard exit if trail not armed)\n"
            f"<b>SL</b> (/sl): {fmt_pct(s.sl_pct)}\n"
            f"<b>Trailing</b> (/trail_activate, /trail_dist): activate {fmt_pct(s.trail_activate_pct)} | dist {fmt_pct(s.trail_dist_pct)}\n\n"
...
def preset_keyboard(kind: str) -> InlineKeyboardMarkup:
    if kind == "mr_band":
        # EXTENDED (user asked >1%)
        opts = [0.20, 0.40, 0.60, 0.80, 1.00, 1.20, 1.50, 2.00, 2.50, 3.00]
        rows, row = [], []
        for v in opts:
    ...
    if kind == "mr_exit":
        # EXTENDED (user asked >0.15)
        opts = [0.00, 0.05, 0.10, 0.15, 0.20, 0.30, 0.50, 0.80, 1.00]
        rows, row = [], []
        for v in opts:
    ...
    if kind == "mr_trend":
        rows = [[
            InlineKeyboardButton("ON", callback_data="set_mr_trend:on"),
            InlineKeyboardButton("OFF", callback_data="set_mr_trend:off"),
        ]]
        return InlineKeyboardMarkup(rows)

    if kind == "trail_activate":
        opts = [0.30, 0.50, 0.80, 1.00, 1.50, 2.00]
        rows, row = [], []
        for v in opts:
    ...
    if kind == "trail_dist":
        opts = [0.20, 0.30, 0.40, 0.60, 0.80, 1.00]
        rows, row = [], []
        for v in opts:
    ...
    if kind == "tp":
        opts = [0.30, 0.60, 0.80, 1.00, 1.20, 1.50, 2.00]
        rows, row = [], []
        for v in opts:
    ...
    if kind == "sl":
        opts = [0.30, 0.35, 0.50, 0.70, 1.00, 1.50]
        rows, row = [], []
        for v in opts:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Exit: reclaim VWAP (exit-band) eller SL / trailing / TP.\n\n"
        "Tips:\n"
        "• /mr_band 1.80\n"
        "• /mr_exit 0.25\n"
        "• /trail_activate 1.50\n"
...
# -------------------------
# Settings commands: now support BOTH presets and manual values
# -------------------------
async def cmd_mr_band(update: Update, context: ContextTypes.DEFAULT_TYPE):
    v = parse_float_arg(context, default=None)
    if v is not None:
        ENGINE.settings.mr_band_pct = clamp(v, 0.01, 5.0)
        ENGINE.persist()
        await update.message.reply_text(f"✅ MR band satt till {ENGINE.settings.mr_band_pct:.2f}%", reply_markup=MAIN_KEYBOARD)
        return
    await update.message.reply_text(
        f"MR band nu: {fmt_pct(ENGINE.settings.mr_band_pct)} under VWAP\n"
        "Skriv t.ex: /mr_band 1.80  (0.01–5.00)\n"
        "Eller välj preset:",
...
async def cmd_mr_exit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if v is not None:
        ...
        ENGINE.persist()
        await update.message.reply_text(f"✅ MR exit band satt till {ENGINE.settings.mr_exit_pct:.2f}%", reply_markup=MAIN_KEYBOARD)
        return
    await update.message.reply_text(
        f"MR exit band nu: {fmt_pct(ENGINE.settings.mr_exit_pct)} under VWAP\n"
        "Skriv t.ex: /mr_exit 0.25  (0.00–5.00)\n"
        "Eller välj preset:",
...
async def cmd_mr_rsi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    v = parse_float_arg(context, default=None)
    if v is not None:
        ENGINE.settings.mr_rsi_max = int(clamp(int(v), 5, 95))
        ENGINE.persist()
        await update.message.reply_text(f"✅ MR RSI max satt till {ENGINE.settings.mr_rsi_max}", reply_markup=MAIN_KEYBOARD)
        return
    await update.message.reply_text(
        f"MR RSI max nu: {ENGINE.settings.mr_rsi_max}\n"
        "Skriv t.ex: /mr_rsi 30\n"
        "Eller välj preset:",
        reply_markup=preset_keyboard("mr_rsi"),
    )

async def cmd_mr_trend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    v = parse_on_off_arg(context)
    if v is not None:
        ENGINE.settings.mr_trend_filter = bool(v)
        ENGINE.persist()
        ...
        return
    await update.message.reply_text(
        f"MR Trend filter nu: {'ON' if ENGINE.settings.mr_trend_filter else 'OFF'}\n"
        "Skriv: /mr_trend on  eller  /mr_trend off\n"
        "Eller välj:",
        reply_markup=preset_keyboard("mr_trend"),
    )

async def cmd_trail_activate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    v = parse_float_arg(context, default=None)
    if v is not None:
        ENGINE.settings.trail_activate_pct = clamp(v, 0.01, 20.0)
        ENGINE.persist()
        await update.message.reply_text(f"✅ Trail activate satt till {ENGINE.settings.trail_activate_pct:.2f}%", reply_markup=MAIN_KEYBOARD)
        return
    await update.message.reply_text(
        f"Trail activate nu: {fmt_pct(ENGINE.settings.trail_activate_pct)}\n"
        "Skriv t.ex: /trail_activate 1.50\n"
        "Eller välj preset:",
...
async def cmd_trail_dist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    v = parse_float_arg(context, default=None)
    if v is not None:
        ENGINE.settings.trail_dist_pct = clamp(v, 0.01, 20.0)
        ENGINE.persist()
        await update.message.reply_text(f"✅ Trail dist satt till {ENGINE.settings.trail_dist_pct:.2f}%", reply_markup=MAIN_KEYBOARD)
        return
    await update.message.reply_text(
        f"Trail dist nu: {fmt_pct(ENGINE.settings.trail_dist_pct)}\n"
        "Skriv t.ex: /trail_dist 0.60\n"
        "Eller välj preset:",
...
async def cmd_tp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    v = parse_float_arg(context, default=None)
    if v is not None:
        ENGINE.settings.tp_pct = clamp(v, 0.05, 20.0)
        ENGINE.persist()
        await update.message.reply_text(f"✅ TP satt till {ENGINE.settings.tp_pct:.2f}%", reply_markup=MAIN_KEYBOARD)
        return
    await update.message.reply_text(
...
async def cmd_sl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    v = parse_float_arg(context, default=None)
    if v is not None:
        ENGINE.settings.sl_pct = clamp(v, 0.05, 20.0)
        ENGINE.persist()
        await update.message.reply_text(f"✅ SL satt till {ENGINE.settings.sl_pct:.2f}%", reply_markup=MAIN_KEYBOARD)
        return
    await update.message.reply_text(
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif data.startswith("set_mr_trend:"):
            v = data.split(":")[1].strip().lower()
            ENGINE.settings.mr_trend_filter = (v == "on")
            ENGINE.persist()
            await q.edit_message_text(f"✅ MR Trend filter: {'ON' if ENGINE.settings.mr_trend_filter else 'OFF'}")
