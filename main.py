# -*- coding: utf-8 -*-
# MP ORB/MOMO Hybrid Bot – LONG ONLY
#
# Features:
# - engine_loop:
#   • Hämtar KuCoin-klines (get_klines).
#   • Räknar EMA20, EMA50, RSI, momentum, ATR% osv (compute_features).
#   • Bestämmer regime: trend (momentum) eller range (mean reversion).
#   • Öppnar entries (endast LONG):
#       - Trend/Momentum (MOMO): om momentum_score > threshold → LONG.
#       - Mean Reversion (MR): om priset avviker från EMA20 mer än MR_DEV_PCT → LONG.
#   • TP / SL / Trailing:
#       - TP: TP_PCT.
#       - SL: SL_PCT.
#       - Trailing start: TRAIL_START_PCT.
#       - Trail: TRAIL_PCT.
#   • Hanterar både:
#       - MOCK-läge – egen fee-modell och kvantitet.
#       - LIVE-läge – riktiga market orders via KuCoin V3, med fills hämtade via /api/v1/fills.
#   • Loggar trades till:
#       - mock_trade_log.csv
#       - real_trade_log.csv
#   • Håller per-symbol-PnL i RAM (SymState.trades_log + realized_pnl)
#     och kan exportera CSV via /export_csv.
#
# Telegram:
#   • /start – välkomsttext + meny.
#   • /status – engine ON/OFF, symboler, risk, PnL, aktiva positioner.
#   • /engine_on / /engine_off
#   • /timeframe 1m,3m,...
#   • /threshold 0.55
#   • /risk + inline-knappar + avancerat /risk set key value.
#   • /pnl – summerar PnL per symbol + total.
#   • /export_csv – exporterar trades_log i minnet.
#   • /mode – välj MOCK/LIVE + bekräftelse för live, både via text och knappar.
#   • /close SYMBOL – stänger EN position (t.ex. /close LINK-USDT).
#   • /close_all – stänger alla positioner.
#   • /reset_pnl – nollar RAM-PnL (loggfilerna orörda).
#   • /test_buy SYMBOL [USDT] – öppnar LONG (Mock eller Live) med bekräftelseknapp.
#   • Inline-knappar:
#       - Risk-shortcuts (size, TP, SL, max_pos).
#       - Mode-väljar (mock / live med bekräftelse).
#       - Test-buy-bekräftelse.
#
# FastAPI / webhook:
#   • GET / – enkel statusrad (engine_on, mode, threshold, total PnL).
#   • GET /health – JSON med engine-status, tfs, symboler, pnl_total m.m.
#   • POST /webhook/{BOT_TOKEN} – Telegrams webhook-endpoint, skickar vidare uppdateringen till tg_app.process_update.
#   • on_startup:
#       - tg_app.initialize()
#       - Sätter webhook till WEBHOOK_BASE/webhook/{BOT_TOKEN} (om WEBHOOK_BASE finns).
#       - Startar Telegram-App + trading-engine som background task.
...
class EngineState:
    ...
    trade_mode: str = "mock"   # mock | live (båda LONG only)
...
# --------------------------------------------------
# POSITION MANAGEMENT (TP / SL / TRAILING + LOSS-GUARD)
# --------------------------------------------------
...
def close_position(symbol: str, st: SymState,
    ...
    if STATE.loss_guard_on:
        ...
        if STATE.loss_streak >= STATE.loss_guard_n:
            STATE.loss_streak = 0
            ...
            if STATE.chat_id:
                try:
                    human_time = STATE.paused_until.astimezone().strftime("%H:%M")
                    asyncio.create_task(
                        tg_app.bot.send_message(
                            STATE.chat_id,
                            f"⏸ Loss-guard: pausar nya entries till ca {human_time} "
                            f"(efter {STATE.loss_guard_n} förluster i rad)."
                        )
                    )
                except Exception:
                    pass
...
# --------------------------------------------------
# ENGINE LOOP (MAIN TRADING ENGINE, LONG ONLY)
# --------------------------------------------------
...
def status_text() -> str:
    ...
    mode_line = (
        ...
        if STATE.trade_mode == "live"
        else "Mode: MOCK (simulerat, LONG only)"
    )
    ...
    lines = [
        ...
        "Strategi: Hybrid Momentum + Mean Reversion (LONG ONLY)",
        regime_line,
        mr_line,
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "🤖 MP Bot – Hybrid Momentum + Mean Reversion (LONG ONLY)\n"
        ...
        try:
            if key == "size":
                ...
                await tg_app.bot.send_message(
                    ...
                    "mr_on, mr_dev, regime_auto, trend_slope_min, "
...
async def cmd_mode(update: Update, _):
    ...
    if parts[1].lower() == "mock":
        ...
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Läge satt till MOCK (simulerad handel, LONG ONLY).",
            reply_markup=reply_kb(),
        )
        return
    ...
    if parts[1].lower() == "live":
        ...
        if len(parts) < 3 or parts[2].upper() != "JA":
            ...
            await tg_app.bot.send_message(
                ...
                "Detta skickar riktiga market-ordrar (LONG ONLY) på KuCoin.",
                reply_markup=kb,
            )
            return
...
async def cmd_close_symbol(update: Update, _):
    """
    /close SYMBOL
    Exempel:
      /close LINK-USDT
      /close XRP-USDT
    Stänger EN öppen position i angiven symbol, mock eller live.
    """
    STATE.chat_id = update.effective_chat.id
    msg = update.message.text.strip().split()
    ...
    if symbol not in STATE.symbols:
        await tg_app.bot.send_message(
            STATE.chat_id,
            f"Symbol {symbol} är inte aktiv.\nAktuella symboler: {', '.join(STATE.symbols)}",
            reply_markup=reply_kb(),
        )
        return
    ...
    if not st.pos:
        await tg_app.bot.send_message(
            STATE.chat_id,
            f"Ingen öppen position i {symbol}.",
            reply_markup=reply_kb(),
        )
        return
    ...
    tf = STATE.tfs[0]
    try:
        kl = await get_klines(symbol, tf, limit=2)
        price = compute_features(kl)["close"]
    except:
        price = pos.entry_price

    # LIVE – sälj på riktigt
    if STATE.trade_mode == "live":
        ok, err = await kucoin_place_market_order(symbol, "sell", pos.qty)
        if not ok:
            await tg_app.bot.send_message(
                STATE.chat_id,
                f"⚠️ LIVE-close misslyckades för {symbol}.\n{err}",
                reply_markup=reply_kb(),
            )
            return

    net = close_position(symbol, st, reason="MANUAL_CLOSE_CMD", approx_price=price)

    await tg_app.bot.send_message(
        STATE.chat_id,
        f"🔻 Manuell stängning {symbol} @ {price:.4f}\nNetto: {net:+.4f} USDT",
        reply_markup=reply_kb(),
    )
...
async def on_button(update: Update, _):
    ...
    if data == "mode_mock":
        await query.answer()
        STATE.trade_mode = "mock"
        await query.edit_message_text("Läge satt till MOCK (LONG ONLY).")
        return
    ...
    if data == "mode_live_confirm":
        ...
        await query.edit_message_text(
            "⚠️ Vill du slå på LIVE?\nDetta skickar riktiga LONG-marketordrar på KuCoin.",
            reply_markup=kb,
        )
        return
...
tg_app.add_handler(CommandHandler("close", cmd_close_symbol))   # << NY
...
async def root():
    total = sum(STATE.per_sym[s].realized_pnl for s in STATE.symbols)
    return (
        "MP Bot Hybrid Momo LONG ONLY OK | "
