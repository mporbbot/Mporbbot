...
#  - Ingen JobQueue – egen asyncio-bakgrundsloop istället (funkar på DigitalOcean)
...
def generate_fake_candles(n: int, base_price: float = 100.0) -> List[Candle]:
    """
    En enkel faker för /mock_trade så att du kan se hur
...
# ---------------------------------------------------
# LIVE-HOOKS (KuCoin) – FÖRBEREDDA, EJ AKTIVA ÄN
# ---------------------------------------------------
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    txt = (
        ...
        "• /start_mock – starta mock-engine (med JA-bekräftelse)\n"
        "• /start_live – förbered live-engine (med JA-bekräftelse)\n"
        "• /stop – stoppa engine\n"
        "• /mock_trade BTCUSDT – kör ORB-simulering med fake-candles\n"
        "• /pnl – visa PnL\n"
        "• /export_csv – exportera mock_trade_log.csv\n"
        "• /cancel – avbryt bekräftelser\n"
    )
...
# ---- START MOCK / LIVE med JA-bekräftelse via ConversationHandler ----

async def cmd_start_mock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not update.message:
        return ConversationHandler.END
    STATE.mock_mode = True
    await update.message.reply_text(
        "Du är på väg att starta *mock-trading*.\n"
        "Skriv `JA` för att bekräfta eller /cancel för att avbryta.",
        parse_mode="Markdown",
    )
    return WAIT_MOCK_CONFIRM


async def confirm_start_mock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not update.message:
        return ConversationHandler.END
    if update.message.text.strip().upper() == "JA":
...
async def cmd_start_live(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not update.message:
        return ConversationHandler.END
    STATE.mock_mode = False
    await update.message.reply_text(
        "⚠️ Du är på väg att aktivera *live-läge*.\n"
        "I den här versionen skickas inga riktiga KuCoin-orders ännu,\n"
        "men motorn körs i live-läge.\n\n"
        "Skriv `JA` för att bekräfta eller /cancel för att avbryta.",
        parse_mode="Markdown",
    )
...
async def confirm_start_live(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not update.message:
        return ConversationHandler.END
    if update.message.text.strip().upper() == "JA":
        STATE.engine_running = True
        await update.message.reply_text(
            "▶️ Engine startad i *live-läge* (utan riktiga orders i denna version).",
            parse_mode="Markdown",
        )
    else:
...
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if update.message:
        await update.message.reply_text("Åtgärd avbruten.")
    return ConversationHandler.END
...
async def cmd_mock_trade(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # logga alla trades, notifiera senaste
    for t in closed:
...
# ---------------------------------------------------
# ENGINE-SCHEDULER (egen asyncio-loop, ingen JobQueue)
# ---------------------------------------------------

async def engine_loop(context: Optional[ContextTypes.DEFAULT_TYPE]) -> None:
    """
    ...
    Just nu gör den inget (för att inte fejka live-data).
    """
    ...
    #   candle = fetch_latest_candle_from_kucoin(...)
    #   trades = process_candle_for_symbol(...)
    #   för varje trade -> log_and_notify_trade(...)
    pass
...
async def main() -> None:
    ...
    # ConversationHandler för start_mock / start_live med JA-bekräftelse
    conv_handler = ConversationHandler(
        ...
        states={
            WAIT_MOCK_CONFIRM: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, confirm_start_mock)
            ],
            WAIT_LIVE_CONFIRM: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, confirm_start_live)
            ],
        },
    ...
    # EGEN BAKGRUNDSLOOP – ersätter JobQueue
    async def engine_background():
        while True:
            try:
                await engine_loop(None)
            except Exception as e:
                logging.error(f"Engine error: {e}")
            await asyncio.sleep(10)  # kör var 10:e sekund

    # starta background-task innan polling
