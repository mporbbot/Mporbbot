...
#   TELEGRAM_BOT_TOKEN  eller BOT_TOKEN  (annars används hårdkodad fallback)
#   WEBHOOK_BASE        (t.ex. https://urchin-app-fjigr.ondigitalocean.app)
#
#   KUCOIN_API_KEY
#   KUCOIN_API_SECRET
#   KUCOIN_API_PASSPHRASE
#   KUCOIN_API_KEY_VERSION  (2 eller 3 – matcha KuCoin)
...
# Hårdkodad fallback-token (din Mporbbot-token)
...
if not BOT_TOKEN:
    raise RuntimeError("Ingen Telegram-bot-token hittades.")
...
async def kucoin_place_market_order(symbol: str, side: str, amount: float):
    ...
    if side_l == "buy":
        # 'funds' i quote-valuta (USDT)
        body["funds"] = f"{amount:.2f}"
    else:
        # 'size' i bas-valuta (t.ex. BTC)
...
def compute_features(candles: List[List[str]]) -> Dict:
...
def decide_regime(feats: Dict) -> str:
    """
...
def _fee(amount_usdt: float) -> float:
    """Enkel avgiftsfunktion – använder STATE.fee_side."""
                    ...
                    for sym in STATE.symbols:
                        ...
                        if regime == "trend":
                            ...
                            if dev_pct <= -STATE.mr_dev_pct:
                                ...
                                # för långt över EMA20 => SHORT (mock)
                                side = "SHORT"
                                open_position(
                                    sym, side, price, st, regime="range", reason="MR"
                                )
                        ...
                        if (
                            ...
                            if STATE.chat_id:
                                ...
                                await app.bot.send_message(
                                    STATE.chat_id,
                                    (
                                        f"🛑 Loss-guard: pausar nya entries "
                                        f"till ca {pause_t}."
                                    ),
                                )
                        continue
                                ...
                                if (
                                    ...
                                    if STATE.chat_id:
                                        ...
                                        await app.bot.send_message(
                                            STATE.chat_id,
                                            (
                                                "🛑 Loss-guard: pausar nya entries "
                                                f"till ca {pause_t}."
                                            ),
                                        )
                                continue
                        else:  # SHORT (mock-only)
...
def status_text() -> str:
    ...
    lines = [
        ...
        (
            ...
            f"Fee per sida: {STATE.fee_side:.4%}"
        ),
        (
            "Risk: "
...
async def cmd_engine_on(update: Update, _: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id
...
async def cmd_engine_off(update: Update, _: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id
...
async def cmd_timeframe(update: Update, _: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id
...
async def cmd_threshold(update: Update, _: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id
...
async def cmd_pnl(update: Update, _: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id
    total = sum(STATE.per_sym[s].realized_pnl for s in STATE.symbols)
...
async def cmd_export_csv(update: Update, _: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id
    rows = [
        [
            "time",
...
async def cmd_mode(update: Update, _: ContextTypes.DEFAULT_TYPE):
    """
...
async def cmd_close_all(update: Update, _: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id
    closed = 0
    total_net = 0.0
    for sym in STATE.symbols:
...
async def cmd_reset_pnl(update: Update, _: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id
    for s in STATE.symbols:
        st = STATE.per_sym[s]
        st.realized_pnl = 0.0
...
async def cmd_test_buy(update: Update, _: ContextTypes.DEFAULT_TYPE):
    """
...
async def on_button(update: Update, _: ContextTypes.DEFAULT_TYPE):
...
async def on_startup():
    # Initiera Telegram-app innan webhook
