...
# Telegram parts kept stable (no job_queue). Adds:
#  - /coins (set coin list or count)
#  - /stake (set USDT per trade)
#  - /export_csv (send CSV logs in Telegram for Skatteverket)
#
# Env required:
#   TELEGRAM_TOKEN="xxxx"
#
...
EMA_FAST = 20
EMA_SLOW = 50
VWAP_LOOKBACK_MIN_DEFAULT = 180  # 3h rolling
TP_PCT_DEFAULT = 0.70
SL_PCT_DEFAULT = 0.35
PULLBACK_TOL_PCT_DEFAULT = 0.20  # /threshold controls this
COOLDOWN_SECONDS_DEFAULT = 180
MAX_POSITIONS_DEFAULT = 5  # allow up to 5 simultaneous positions (your request style)
STAKE_USDT_DEFAULT = 30.0

ENTRY_MODES = ["break", "retest"]  # kept for compatibility/UI only in this strategy
...
class BotConfig:
    ...
    entry_mode: str = "retest"  # UI only here
...
# -----------------------------
# Notifications + Logging (Skatteverket)
# -----------------------------
...
async def evaluate_symbol(app: Application, symbol: str):
    ...
    # Pullback near EMA20 using /threshold as tolerance
    tol = CFG.threshold_pct / 100.0
    pullback_ok = last_low <= ema20 * (1.0 + tol)

    # Bullish reclaim: bullish candle and close above EMA20
...
async def engine_background_loop(app: Application):
    while True:
        try:
            if CFG.engine_on:
                # Iterate a snapshot to avoid issues if list changes mid-loop
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Nytt:\n"
        "• /coins 5  (eller /coins BTC-USDT ETH-USDT ...)\n"
        "• /stake 30\n"
        "• /export_csv (Skatteverket-listor)\n",
...
async def cmd_close_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym in list(POS.keys()):
        try:
            ...
            # approximate net calc before close (close_long will log)
            p = POS.get(sym)
            if p:
                gross = (price - p.entry_price) * p.qty
                fee = calc_fees(p.entry_price, price, p.qty, CFG.fee_rate)
...
async def cmd_stake(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args if hasattr(context, "args") else []
    if not args:
        await update.message.reply_text(f"Stake per trade nu: {CFG.stake_usdt:.2f} USDT\nEx: /stake 30")
        return
    v = parse_float(args[0])
    if v is None:
...
async def cmd_coins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /coins 5                => set first 5 of DEFAULT_COINS
    /coins BTC-USDT ETH-USDT => set explicit list
    /coins                  => show current list
    """
    args = context.args if hasattr(context, "args") else []
    if not args:
        await update.message.reply_text(
    ...
    # If first arg is a number => take first N from DEFAULT_COINS
    n = None
    try:
        n = int(args[0])
    except Exception:
    ...
    # Otherwise treat args as symbols
    coins = [safe_symbol(a) for a in args]
    # basic sanity: ensure unique, and include only USDT pairs
    uniq = []
    for c in coins:
...
async def cmd_export_csv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Sends the CSV files as Telegram documents.
    These are the trade lists you can use for Skatteverket.
    """
    ...
    await update.message.reply_text(
        ...
        "Allt i USDT (kan konverteras till SEK vid deklaration)."
    )
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Kunde inte skicka mock_trade_log.csv (kontrollera filrättigheter).")
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Kunde inte skicka real_trade_log.csv (kontrollera filrättigheter).")
...
def build_app() -> Application:
    ...
    app.add_handler(CommandHandler("stake", cmd_stake))      # NEW
    app.add_handler(CommandHandler("coins", cmd_coins))      # NEW
    app.add_handler(CommandHandler("trade_mode", cmd_trade_mode))
    app.add_handler(CommandHandler("export_csv", cmd_export_csv))  # NEW
