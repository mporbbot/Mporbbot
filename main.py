...
# ------------------------------------------------------------
# MP Bot – v43
# - DD-stop sänkt (default 2.0%)
# - Trailing-TP + Break-even (går för större vinster)
# - Reply-keyboard-kommandon (inkl. /trend)
# - AI: lärning + auto-load/save (skapar katalog automatiskt)
# - Mock-handel (avgifter), Grid/DCA, Long/Short (shorts av som default)
...
GRID_STEP_MULT = float(os.getenv("GRID_STEP_MULT", "0.6"))               # multipel för nästa step
GRID_SIZE_MULT = float(os.getenv("GRID_SIZE_MULT", "1.5"))               # kvantitets-multipel
GRID_TP_PCT = float(os.getenv("GRID_TP_PCT", "0.25"))                    # fallback TP om trailing ej aktiv
TP_SAFETY_BONUS = float(os.getenv("TP_SAFETY_BONUS", "0.05"))

# Risk
DD_STOP_PCT = float(os.getenv("DD_STOP_PCT", "2.0"))                     # <-- sänkt default

# Trailing (NYTT)
TRAIL_ON = (os.getenv("TRAIL_ON", "on").lower() in ("1","on","true","yes"))
TRAIL_TRIGGER_PCT = float(os.getenv("TRAIL_TRIGGER_PCT", "0.35"))        # aktivera trailing vid +X% från avg
TRAIL_PCT = float(os.getenv("TRAIL_PCT", "0.25"))                        # avstånd under/över high/low-water
TRAIL_BE_PCT = float(os.getenv("TRAIL_BE_PCT", "0.20"))                  # be-arm vid +X% från avg
...
def _ensure_ai_dir():
    try:
        d = os.path.dirname(AI_MODEL_PATH)
...
def _enter_leg(sym: str, side: str, price: float, usd_size: float, st: SymState) -> TradeLeg:
    ...
    # reset trailing state vid ny leg
    st.pos.trail_active = False
    st.pos.high_water = 0.0
    st.pos.low_water = 0.0
        ...
        try:
            if STATE.engine_on:
                        ...
                        if score > 0.35:
                            ...
                            if STATE.chat_id:
                                await app.bot.send_message(
                                    ...
                                    f"DD {STATE.risk_cfg['dd']:.2f}% | QTY {st.pos.legs[-1].qty:.6f}"
                                )
                ...
                for sym in STATE.symbols:
                    ...
                    # 1) Break-even arm
                    if STATE.trend_cfg["on"] and not st.pos.be_armed:
                        be = STATE.trend_cfg["be_pct"]
                        if (st.pos.side == "LONG" and move_pct >= be) or \
                           (st.pos.side == "SHORT" and -move_pct >= be):
                            st.pos.be_armed = True
                            if STATE.chat_id:
                                await app.bot.send_message(STATE.chat_id, f"🟨 BE armed {sym} ({be:.2f}%)")

                    # 2) Aktivera trailing
                    if STATE.trend_cfg["on"] and not st.pos.trail_active:
                        trig = STATE.trend_cfg["tp_trigger"]
                        if (st.pos.side == "LONG" and move_pct >= trig) or \
                           (st.pos.side == "SHORT" and -move_pct >= trig):
                            st.pos.trail_active = True
                            st.pos.high_water = price
                            st.pos.low_water = price
                            if STATE.chat_id:
                                await app.bot.send_message(STATE.chat_id, f"🏃 Trail ON {sym} (trigger {trig:.2f}%)")

                    # 3) Trailing-läge
                    if st.pos.trail_active:
                        tpct = STATE.trend_cfg["trail_pct"] / 100.0
                        if st.pos.side == "LONG":
                            if price > st.pos.high_water:
                                st.pos.high_water = price
                            trail_stop = st.pos.high_water * (1 - tpct)
                            if st.pos.be_armed:
                                trail_stop = max(trail_stop, st.pos.avg_price)
                            if price <= trail_stop:
                                net = _exit_all(sym, price, st)
                                if STATE.chat_id:
                                    mark = "✅" if net >= 0 else "❌"
                                    await app.bot.send_message(STATE.chat_id, f"🏁 TRAIL EXIT {sym} @ {price:.4f} | Net {net:+.4f} {mark}")
                                feats = features_from_candles(kl)
                                if STATE.ai_on:
                                    ai_learn(sym, tf0, feats, net)
                                continue
                        else:  # SHORT
                            if price < st.pos.low_water:
                                st.pos.low_water = price
                            trail_stop = st.pos.low_water * (1 + tpct)
                            if st.pos.be_armed:
                                trail_stop = min(trail_stop, st.pos.avg_price)
                            if price >= trail_stop:
                                net = _exit_all(sym, price, st)
                                if STATE.chat_id:
                                    mark = "✅" if net >= 0 else "❌"
                    ...
                    # 4) Fallback: fast TP om trailing ej aktiv
                    if not st.pos.trail_active:
                    ...
                    if abs(move_pct) >= STATE.risk_cfg["dd"]:
                        ...
                        if STATE.chat_id:
                            mark = "✅" if net >= 0 else "❌"
                            await app.bot.send_message(STATE.chat_id, f"⛔ STOP {sym} @ {price:.4f} | Net {net:+.4f} {mark} (DD)")
            await asyncio.sleep(2)
        except Exception as e:
            if STATE.chat_id:
                try:
...
def status_text() -> str:
    ...
    for s in STATE.symbols:
        ...
        if st.pos:
            pos_lines.append(
                ...
                f"| legs {st.pos.safety_count} | trail={'ON' if st.pos.trail_active else 'OFF'}"
            )
    ...
    lines = [
        ...
        (f"Trend: on={t['on']} | trigger={t['tp_trigger']}% | trail={t['trail_pct']}% | be={t['be_pct']}%"),
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(STATE.chat_id, "MP Bot v43 – redo ✅", reply_markup=reply_kb())
...
async def cmd_grid(update: Update, _):
    ...
    await tg_app.bot.send_message(STATE.chat_id,
        ("Grid:\n"
         ...
         "Ex: /grid set step_mult 0.7"), reply_markup=reply_kb())
...
async def cmd_trend(update: Update, _):
    ...
    toks = txt.split()
    if len(toks) == 4 and toks[0] == "/trend" and toks[1] == "set":
        key, val = toks[2], toks[3]
        if key in STATE.trend_cfg:
            try:
                if key == "on":
                    STATE.trend_cfg[key] = (val.lower() in ("1","on","true","yes"))
                else:
                    STATE.trend_cfg[key] = float(val)
                await tg_app.bot.send_message(STATE.chat_id, "Trend-param uppdaterad.", reply_markup=reply_kb())
            except:
                await tg_app.bot.send_message(STATE.chat_id, "Felaktigt värde.", reply_markup=reply_kb())
            return
        else:
    ...
    t = STATE.trend_cfg
    await tg_app.bot.send_message(STATE.chat_id,
        (f"Trend (trailing): on={t['on']} | tp_trigger={t['tp_trigger']}% | "
         f"trail_pct={t['trail_pct']}% | be_pct={t['be_pct']}%\n"
         "Ex: /trend set tp_trigger 0.4\n"
         "Ex: /trend set trail_pct 0.25\n"
         "Ex: /trend set be_pct 0.2\n"
         "Ex: /trend set on on"),
        reply_markup=reply_kb())
...
async def cmd_export_k4(update: Update, _):
    # enklare CSV som underlag för K4 (datum, instrument, antal, á-pris, försäljningspris, resultat)
    STATE.chat_id = update.effective_chat.id
    rows = [["date","symbol","side","qty_total","avg_price","exit_price","net_usdt"]]
    for s in STATE.symbols:
        for r in STATE.per_sym[s].trades_log:
            # qty_total kan inte återskapas exakt efter exit – ta approx via avg_price & fee_out
            qty_est = 0.0
            # Vi lagrar inte qty_total i loggen; här förenklar vi (kan förbättras senare)
            rows.append([r["time"][:10], r["symbol"], r["side"], qty_est, r["avg_price"], r["exit_price"], r["net"]])
    if len(rows) == 1:
        await tg_app.bot.send_message(STATE.chat_id, "Inga affärer ännu.", reply_markup=reply_kb())
        return
    buf = io.StringIO()
    csv.writer(buf).writerows(rows)
    buf.seek(0)
    await tg_app.bot.send_document(STATE.chat_id, document=io.BytesIO(buf.getvalue().encode("utf-8")),
                                   filename="k4_export.csv", caption="K4-underlag (förenklad)")
...
async def on_startup():
    ...
    try:
        if os.path.exists(AI_MODEL_PATH):
            await tg_app.bot.send_message(STATE.chat_id, load_ai())
    except:
        pass
...
async def root():
    total = sum(STATE.per_sym[s].realized_pnl_net for s in STATE.symbols)
    return f"MP Bot v43 OK | engine_on={STATE.engine_on} | pnl_total={total:+.4f}"
