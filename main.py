...
#
# NEW in this version (ONLY change):
#   /entry command + buttons to switch entry_mode: BREAK / RETEST
#
# Commands:
...
def main_menu_keyboard() -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton("/entry"), KeyboardButton("/threshold")],  # <-- NEW: /entry in menu
        [KeyboardButton("/stake"), KeyboardButton("/tp")],
        [KeyboardButton("/sl"), KeyboardButton("/coins")],
...
async def open_long(app: Application, symbol: str, breakout_level: float, reason: str):
    ...
    sl = entry_price * (1.0 - CFG.sl_pct / 100.0)
    ...
    await notify_trade(
        ...
        f"| TF=5m | entry={CFG.entry_mode.upper()} | {reason}{extra}"
    )
...
async def evaluate_symbol(app: Application, symbol: str):
    ...
    if len(highs) < BREAKOUT_LOOKBACK + 2:
        return
    level = max(highs[-(BREAKOUT_LOOKBACK + 1):-1])
    last_close = closes[-1]
    ...
    if CFG.entry_mode == "break":
        ...
        now = int(time.time())
        PENDING_RETEST[symbol] = {"level": float(level), "set_ts": now, "expire_ts": now + RETEST_EXPIRE_SECONDS}
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• /entry = byt entry mode (BREAK/RETEST)\n"
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        f"Strategy: Trend Break & Hold (TF=5m)",
...
async def cmd_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # NEW: /entry [break|retest] or buttons
    args = context.args if hasattr(context, "args") else []
    if args:
...
def build_app() -> Application:
    ...
    app.add_handler(CommandHandler("entry", cmd_entry))  # <-- NEW
