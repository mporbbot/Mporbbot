# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# MP Bot – v53 (Hybrid Momentum + Mean Reversion, MOCK + LIVE)
# ------------------------------------------------------------
...
#
# Byt läge i Telegram:
#   /mode              -> visa nuvarande läge
#   /mode mock         -> tvinga MOCK
#   /mode live         -> fråga med JA/NEJ-knappar
#   /mode live JA      -> AKTIVERA LIVE direkt (om API-nycklar finns)
#
# Miljövariabler (Render):
#   TELEGRAM_BOT_TOKEN  eller BOT_TOKEN
#   WEBHOOK_BASE        (https://din-app.onrender.com)
#
#   KUCOIN_API_KEY
#   KUCOIN_API_SECRET
#   KUCOIN_API_PASSPHRASE
#   KUCOIN_API_KEY_VERSION  (för nya nycklar: VANLIGAST "3")
...
# Standardinställningar (matchar dina nuvarande värden)
...
# För nya KuCoin-konton är nyckeln oftast V3
...
class EngineState:
    ...
    # Pending test-buy (inline-knapp)
    pending_test_buy: Optional[Dict] = None
...
async def kucoin_private_request(method: str, path: str, body: Optional[dict] = None):
    ...
    Returnerar JSON eller None vid fel.
    """
    if not kucoin_creds_ok():
        print("KuCoin creds saknas.")
        return None
    ...
    # passphrase måste också hashas (gäller V2/V3-nycklar)
    ...
    async with httpx.AsyncClient(base_url=KUCOIN_BASE_URL, timeout=10) as client:
        ...
        try:
            ...
            print("KuCoin: kunde inte parsa JSON:", resp.text)
            return None

        if resp.status_code != 200:
            print("KuCoin HTTP-fel:", resp.status_code, data)
            return None

        if data.get("code") != "200000":
            # Viktigt för felsökning
            print("KuCoin API-fel:", data)
            return None

        return data

async def kucoin_place_market_order(symbol: str, side: str, size_base: float) -> bool:
    """
    Skicka en enkel market-order på spot.
    side: "buy" eller "sell"
    size_base: mängd bas-valuta (t.ex. BTC) att köpa/sälja
    """
    body = {
            ...
            if STATE.engine_on:
                ...
                if len(open_syms) < STATE.max_pos:
                    for sym in STATE.symbols:
                        ...
                        try:
                            ...
                            print("get_klines fel (entry):", sym, e)
                            continue
                        ...
                        if regime == "trend":
                            ...
                            if score > STATE.threshold:
                                ...
                                qty = STATE.mock_size / price if price > 0 else 0.0
                                if STATE.trade_mode == "live":
                                    ok = await kucoin_place_market_order(
                                        sym, "buy", qty
                                    )
                                    if not ok:
                                        if STATE.chat_id:
                ...
                for sym in STATE.symbols:
                    ...
                    try:
                        ...
                        print("get_klines fel (manage):", sym, e)
                        continue
                    feats = compute_features(kl)
                    price = feats["close"]
                    pos = st.pos
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "🤖 MP Bot v53 – Hybrid Momentum + Mean Reversion\n"
...
async def cmd_risk(update: Update, _):
    ...
    # /risk utan argument: visa status + storleks-knappar
    await tg_app.bot.send_message(
        STATE.chat_id, status_text(), reply_markup=reply_kb()
    )
...
async def cmd_mode(update: Update, _):
    ...
    /mode           -> visa läge
    /mode mock      -> tvinga MOCK
    /mode live      -> fråga JA/NEJ inline
    /mode live JA   -> aktivera LIVE direkt
    """
    STATE.chat_id = update.effective_chat.id
    toks = update.message.text.strip().split()
    if len(toks) == 1:
    ...
    if target == "live":
        # Om användaren skriver "JA" direkt
        if len(toks) >= 3 and toks[2].upper() == "JA":
            if not kucoin_creds_ok():
                await tg_app.bot.send_message(
                    STATE.chat_id,
                    "❌ Kan inte aktivera LIVE: saknar KUCOIN_API_KEY/SECRET/PASSPHRASE "
                    "eller fel värde på KUCOIN_API_KEY_VERSION i Render.",
                    reply_markup=reply_kb(),
                )
                return
        ...
        await tg_app.bot.send_message(
            STATE.chat_id,
            "⚠️ Vill du verkligen aktivera LIVE-spotläge?\n"
            "Detta skickar RIKTIGA ordrar på KuCoin (endast LONG på spot).",
            reply_markup=kb,
        )
        return
...
async def cmd_close_all(update: Update, _):
    """Stäng alla öppna positioner (mock/live). I LIVE-läge skickas riktiga säljordrar."""
    STATE.chat_id = update.effective_chat.id
    msgs = []
    for sym in STATE.symbols:
        st = STATE.per_sym[sym]
        ...
        try:
            ...
            msgs.append(f"{sym}: kunde inte hämta pris ({e})")
            continue
        feats = compute_features(kl)
        price = feats["close"]
        pos = st.pos
        if STATE.trade_mode == "live" and pos.side == "LONG":
            ok = await kucoin_place_market_order(sym, "sell", pos.qty)
            if not ok:
                msgs.append(f"{sym}: LIVE-sälj misslyckades, lämnar öppen.")
                continue
        net = close_position(sym, price, st, "MANUAL_CLOSE")
        mark = "✅" if net >= 0 else "❌"
        msgs.append(f"{sym}: stängd @ {price:.4f} | Net {net:+.4f} USDT {mark}")
    if not msgs:
        await tg_app.bot.send_message(
            STATE.chat_id, "Inga öppna positioner att stänga.", reply_markup=reply_kb()
        )
    else:
        await tg_app.bot.send_message(
            STATE.chat_id,
...
async def cmd_reset_pnl(update: Update, _):
    """Nollställ PnL (endast minnet i boten, inte loggfiler)."""
    STATE.chat_id = update.effective_chat.id
    for s in STATE.symbols:
        STATE.per_sym[s].realized_pnl = 0.0
    await tg_app.bot.send_message(
        STATE.chat_id,
        "PnL återställd till 0.0 USDT (loggarna på disk ändras inte).",
        reply_markup=reply_kb(),
    )

async def cmd_test_buy(update: Update, _):
    """
    /test_buy              -> BTC-USDT, 5 USDT (LIVE, fråga JA/NEJ)
    /test_buy SYMBOL SIZE  -> valfritt (t.ex. /test_buy BTC-USDT 10)
    """
    STATE.chat_id = update.effective_chat.id
    ...
    if STATE.trade_mode != "live":
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Testköp använder LIVE-läge (riktiga ordrar på KuCoin).\n"
            "Byt först till LIVE med /mode live.",
            reply_markup=reply_kb(),
        )
        return
    ...
    STATE.pending_test_buy = {"symbol": symbol, "size_usdt": size_usdt}

    kb = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("✅ Ja", callback_data="testbuy_yes"),
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        f"Testköp i LIVE-läge:\n\n"
        f"BUY {symbol} för ca {size_usdt:.2f} USDT (market-order på KuCoin).\n\n"
        "Vill du fortsätta?",
        reply_markup=kb,
    )

# -----------------------------
# Inline-knappar (callback)
# -----------------------------
async def cb_inline(update: Update, _):
    ...
    # Storleksval från /risk
    if data.startswith("size_"):
        try:
            size = float(data.split("_", 1)[1])
        except Exception:
            size = STATE.mock_size
        STATE.mock_size = size
        await query.edit_message_text(
            f"Mock-size uppdaterad till {size:.0f} USDT.",
            reply_markup=reply_kb(),
        )
        return
    ...
    if data == "mode_live_no":
        await query.edit_message_text(
            "LIVE-läge aktiverades inte.", reply_markup=reply_kb()
        )
        return

    # Test-buy JA/NEJ
    if data == "testbuy_no":
        STATE.pending_test_buy = None
        await query.edit_message_text(
            "❎ Testköp avbrutet.", reply_markup=reply_kb()
        )
        return

    if data == "testbuy_yes":
        payload = STATE.pending_test_buy
        STATE.pending_test_buy = None
        if not payload:
            await query.edit_message_text(
                "Ingen aktiv testköpsförfrågan.", reply_markup=reply_kb()
            )
            return
        symbol = payload["symbol"]
        size_usdt = payload["size_usdt"]
        # Hämta pris och räkna om USDT -> bas-mängd
        tf = STATE.tfs[0] if STATE.tfs else "1m"
        try:
            ...
            await query.edit_message_text(
                f"❌ Kunde inte hämta pris för {symbol}: {e}",
                reply_markup=reply_kb(),
            )
            return
        ...
        if price <= 0:
            await query.edit_message_text(
                f"❌ Ogiltigt pris för {symbol}.", reply_markup=reply_kb()
            )
            return

        qty = size_usdt / price
        ok = await kucoin_place_market_order(symbol, "buy", qty)
        if ok:
            await query.edit_message_text(
                f"✅ Testköp skickat: BUY {symbol} för ca {size_usdt:.2f} USDT.",
                reply_markup=reply_kb(),
            )
        else:
            await query.edit_message_text(
                f"❌ Testköp misslyckades för {symbol}.\n"
                "Kolla Render-loggarna för detaljer (KuCoin API-fel).",
                reply_markup=reply_kb(),
            )
        return
...
tg_app.add_handler(CallbackQueryHandler(cb_inline))
