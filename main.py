...
BACKTEST_RESULT = "backtest_results.csv"
...
class TradingEngine:
    ...
    def entry_data(self, symbol):
        ...
        opens = [safe_float(x[1]) for x in candles]
        closes = [safe_float(x[2]) for x in candles]
        highs = [safe_float(x[3]) for x in candles]
        lows = [safe_float(x[4]) for x in candles]
        volumes = [safe_float(x[5]) for x in candles]
        candle_times = [int(x[0]) for x in candles]
    ...
    def volume_ok(self, volumes, index):
    ...
    def detect_signal(self, symbol):
        ...
        # Senaste helt stängda 1m-candle.
        i = len(closes) - 2
        ...
        # VIKTIG FIX:
        # Den gamla versionen hittade setup men returnerade den aldrig.
    ...
    def close_position(
        ...
        # Bara för rapportering.
        # Slippage finns redan inräknad i entry och exit_price.
        slip_cost = (
            entry_value
            * self.settings.slippage_side
            + exit_value
        ...
        # FIX: dra INTE av slippage en gång till.
        net = (
            gross
            - fees
        )
        ...
        for symbol in list(
            ...
            try:
                ...
                if pos.trail_active:
                    ...
                    if pos.trail_stop > pos.stop:
                        pos.stop = pos.trail_stop
...
async def cmd_backtest(
    ...
    if (
        ...
        and BACKTEST_PROCESS.poll() is None
    ):

        await update.message.reply_text(
            "Backtest kör redan.\n"
            "Använd /backtest_status.",
            reply_markup=KEYBOARD
        )
    ...
    if not os.path.exists(
        ...
        await update.message.reply_text(
            "Hittar inte backtest.py.\n"
            "Kontrollera att backtest.py ligger bredvid main.py.",
            reply_markup=KEYBOARD
        )
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "30 dagar\n"
            "108 konfigurationer\n\n"
            "Tradingmotorn bör vara OFF medan testet kör.\n\n"
            "Kolla med /backtest_status",
            reply_markup=KEYBOARD
        )
        ...
        await update.message.reply_text(
            f"Kunde inte starta backtest:\n{e}",
            reply_markup=KEYBOARD
        )


async def cmd_backtest_status(
    update,
    context
):
    ...
    if (
        BACKTEST_PROCESS is None
        and not os.path.exists(BACKTEST_LOG)
    ):

        await update.message.reply_text(
            "Inget backtest har startats.",
            reply_markup=KEYBOARD
        )
    ...
    if running:
        ...
        header = "BACKTEST KLART\n\n"

    if not text:
        text = "Väntar på output..."
...
async def cmd_backtest_result(
    ...
    if not os.path.exists(
        ...
        await update.message.reply_text(
            "Det finns inget färdigt resultat ännu.",
            reply_markup=KEYBOARD
        )
    ...
    try:
        ...
        for row in rows[:5]:
            ...
            parts.append(
                f"#{row['rank']}\n"
                f"Trades: {row['trades']}\n"
                f"Winrate: {safe_float(row['winrate']):.1f}%\n"
                f"Net: {safe_float(row['net_usdt']):+.2f} USDT\n"
                f"PF: {safe_float(row['profit_factor']):.2f}\n"
                f"Max DD: {safe_float(row['max_drawdown']):.2f} USDT\n"
                f"TP: {row['tp_pct']}%\n"
                f"SL ATR: {row['sl_atr']}\n"
                f"Volume: {row['volume_mult']}\n"
...
def main():
    ...
    handlers = [
        ...
        ("backtest_status", cmd_backtest_status),
        ("backtest_result", cmd_backtest_result),
    ]
