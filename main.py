...
#   KUCOIN_API_KEY_VERSION  (standard "2")
...
def reply_kb() -> ReplyKeyboardMarkup:
    rows = [
        ...
        [
            KeyboardButton("/mode"),
            KeyboardButton("/mode mock"),
            KeyboardButton("/mode live"),
        ],
        [KeyboardButton("/test_buy")],
    ]
...
async def cmd_mode(update: Update, _):
    ...
    if target == "live":
        if len(toks) < 3 or toks[2].upper() != "JA":
            await tg_app.bot.send_message(
                STATE.chat_id,
                "⚠️ För att slå på LIVE-spot, skriv:\n\n"
                "/mode live JA\n\n"
                "Se till att du har:\n"
                "• API-key på KuCoin med enbart 'Spot Trading'\n"
                "• KUCOIN_API_KEY / SECRET / PASSPHRASE i Render env\n"
                "• KUCOIN_API_KEY_VERSION = 2\n"
                "• Tillräckligt USDT-saldo.\n\n"
                "OBS: Endast LONG på spot, inga shorts.",
                reply_markup=reply_kb(),
            )
            return
...
async def cmd_close_all(update: Update, _):
    """Stänger alla öppna positioner (mock eller live)."""
    STATE.chat_id = update.effective_chat.id
    if not any(STATE.per_sym[s].pos for s in STATE.symbols):
        await tg_app.bot.send_message(
            STATE.chat_id, "Inga öppna positioner att stänga.", reply_markup=reply_kb()
        )
        return

    msg_lines = ["Stänger alla öppna positioner:"]
    for sym in STATE.symbols:
        st = STATE.per_sym[sym]
        if not st.pos:
            continue
        ...
        try:
            ...
            msg_lines.append(f"- {sym}: kunde inte hämta pris, hoppar över.")
            continue
        feats = compute_features(kl)
        price = feats["close"]
        pos = st.pos
        ...
        if STATE.trade_mode == "live" and pos.side == "LONG":
            ...
            if not ok:
                msg_lines.append(f"- {sym}: LIVE-sälj misslyckades, lämnas öppen.")
                continue

        net = close_position(sym, price, st, "MANUAL_CLOSE")
        mark = "✅" if net >= 0 else "❌"
        msg_lines.append(f"- {sym}: stängd @ {price:.4f} | Net {net:+.4f} USDT {mark}")

    await tg_app.bot.send_message(
        STATE.chat_id, "\n".join(msg_lines), reply_markup=reply_kb()
    )

async def cmd_reset_pnl(update: Update, _):
    """Nollställer PnL i minnet (påverkar inte CSV-loggar)."""
    STATE.chat_id = update.effective_chat.id
    for s in STATE.symbols:
        st = STATE.per_sym[s]
        st.realized_pnl = 0.0
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "PnL återställd till 0 (endast i minnet, CSV-loggar är kvar).",
        reply_markup=reply_kb(),
    )

async def cmd_test_buy(update: Update, _):
    """
    /test_buy SYMBOL SIZEUSDT
    Ex: /test_buy BTC-USDT 5
    Gör ett litet LIVE-köp för att testa KuCoin-API:t.
    """
    STATE.chat_id = update.effective_chat.id
    toks = update.message.text.strip().split()
    if len(toks) < 2:
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Använd: /test_buy SYMBOL [SIZEUSDT]\n"
            "Ex: /test_buy BTC-USDT 5",
            reply_markup=reply_kb(),
        )
        return
    ...
    if STATE.trade_mode != "live":
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Testköp använder riktiga LIVE-ordrar.\n"
            "Aktivera först live-läget med /mode live JA.",
            reply_markup=reply_kb(),
        )
        return
    ...
    if not kucoin_creds_ok():
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Saknar eller felaktiga KuCoin-API-nycklar (KUCOIN_API_*).",
            reply_markup=reply_kb(),
        )
        return

    sym = toks[1].upper().replace("/", "-")
    if len(toks) >= 3:
        try:
            ...
            await tg_app.bot.send_message(
                STATE.chat_id,
                "Fel SIZEUSDT. Ex: /test_buy BTC-USDT 5",
                reply_markup=reply_kb(),
            )
            return
    else:
        size_usdt = 5.0  # default
    ...
    try:
        ...
        await tg_app.bot.send_message(
            STATE.chat_id,
            f"Kunde inte hämta pris för {sym}: {e}",
            reply_markup=reply_kb(),
        )
        return
    ...
    if price <= 0:
        await tg_app.bot.send_message(
            STATE.chat_id,
            f"Ogiltigt pris för {sym}.",
            reply_markup=reply_kb(),
        )
        return

    qty = size_usdt / price
    ok = await kucoin_place_market_order(sym, "buy", qty)
    if ok:
        await tg_app.bot.send_message(
            STATE.chat_id,
            f"✅ Testköp skickat: BUY {sym} size≈{qty:.8f} (≈{size_usdt:.2f} USDT).",
            reply_markup=reply_kb(),
        )
    else:
        await tg_app.bot.send_message(
            STATE.chat_id,
            f"❌ Testköp misslyckades för {sym}. Kolla API-key, version=2 och saldo.",
            reply_markup=reply_kb(),
        )
