...
#
# Extra kommandon:
#   /reset_pnl         -> nollställer PnL i minnet
#   /close_all         -> stänger alla öppna positioner (mock + live)
#   /pnl reset         -> samma som /reset_pnl
#
                            ...
                            if score > STATE.threshold:
                                ...
                                if STATE.trade_mode == "live":
                                    ok = await kucoin_place_market_order(
                                        sym, "sell" if side == "SHORT" else "buy", qty
                                    )
                                    if not ok:
                                        if STATE.chat_id:
...
def reply_kb() -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton("/mode"), KeyboardButton("/close_all"), KeyboardButton("/reset_pnl")],
    ]
...
async def cmd_pnl(update: Update, _):
    ...
    # /pnl reset -> nollställ PnL
    if len(toks) >= 2 and toks[1].lower() == "reset":
        for s in STATE.symbols:
            st = STATE.per_sym[s]
            st.realized_pnl = 0.0
            st.trades_log.clear()
        await tg_app.bot.send_message(
            STATE.chat_id,
            "🔄 PnL och intern trade-logg nollställd (mock/live). CSV-filerna rörs inte.",
            reply_markup=reply_kb(),
        )
        return
...
async def cmd_close_all(update: Update, _):
    """
    Stänger alla öppna positioner (mock + live).
    I LIVE-läge skickas market-sell för alla LONG-positioner.
    """
    STATE.chat_id = update.effective_chat.id
    closed_lines = []
    total_net = 0.0
    ...
    for sym in STATE.symbols:
        ...
        try:
            ...
            # om priset inte kan hämtas hoppa över
            continue
        ...
        if STATE.trade_mode == "live" and pos.side == "LONG":
            ...
            if not ok:
                await tg_app.bot.send_message(
                    STATE.chat_id,
                    f"⚠️ LIVE sälj {sym} misslyckades, positionen är kvar.",
                    reply_markup=reply_kb(),
                )
                continue
        ...
        total_net += net
        closed_lines.append(
            f"{sym}: {pos.side} stängd @ {price:.4f} | Net {net:+.4f} USDT"
        )
    ...
    msg = "🔻 Alla möjliga positioner stängda:\n" + "\n".join(closed_lines)
    msg += f"\n\nSumma Net: {total_net:+.4f} USDT"
