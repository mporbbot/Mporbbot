...
#
# TRADING:
#   /engine_on, /engine_off   -> start/stop engine
#   /mode                     -> mock/live
#   /risk                     -> risk-inställningar (size mm)
#
# TEST LIVE (nytt):
#   /test_buy                 -> välj coin i inline-knappar, bot gör LIVE
#                                market-köp OCH market-sälj (round-trip)
#   /test_buy ADA-USDT        -> samma, men direkt för angiven coin
#
# Miljövariabler:
...
#   KUCOIN_API_KEY_VERSION  (2 eller 3 – matcha det som står i KuCoin)
...
#   MAX_POS            (default 4)
#   TEST_BUY_USDT      (default 5.0 – storlek för testköp)
#
# ------------------------------------------------------------
...
TEST_BUY_USDT = float(os.getenv("TEST_BUY_USDT", "5.0"))
...
async def kucoin_private_request(method: str, path: str, body: Optional[dict] = None):
    ...
    # KuCoin vill ha tom body-string för GET
    if method_u == "GET":
        body_str = ""
    else:
    ...
    prehash = ts + method_u + path + body_str
    ...
    # Liten debug för längder (hjälper felsökning)
    print(
        "[KUCOIN DEBUG]\n"
        f"key_len={len(KUCOIN_API_KEY)},\n"
        f"secret_len={len(KUCOIN_API_SECRET)},\n"
        f"passphrase_len={len(KUCOIN_API_PASSPHRASE)},\n"
        f"version='{KUCOIN_API_KEY_VERSION}'"
    )
...
async def kucoin_get_trade_balance(currency: str) -> Optional[float]:
    """
    Hämta tillgänglig trade-balans för given valuta (t.ex. 'BTC', 'ADA').
    """
    data = await kucoin_private_request("GET", "/api/v1/accounts?type=trade")
    if not data:
        return None
    status = data.get("_http_status")
    code = data.get("code")
    ...
    for acc in accounts:
        if acc.get("currency") == currency:
            # 'available' är det vi kan använda direkt
            try:
                return float(acc.get("available") or acc.get("balance") or 0.0)
            except Exception:
                return None
    return None
                    ...
                    if move_pct >= STATE.tp_pct and not pos.trailing:
                            ...
                            if not ok:
                                if STATE.chat_id:
                                    await app.bot.send_message(
                                        STATE.chat_id,
                                        "⚠️ LIVE TP-sälj "
                                        f"{sym} misslyckades, försöker igen senare.\n"
                                        f"{err}",
                                    )
                                continue
                        ...
                        if (
                            ...
                            if STATE.chat_id:
                                ...
                                await app.bot.send_message(
                                    STATE.chat_id,
                                    "🛑 Loss-guard: pausar nya entries till ca "
                                    f"{pause_t}.",
                                )
                        continue
                        ...
                        if pos.side == "LONG":
                            ...
                            if price <= trail_stop:
                                    ...
                                    if not ok:
                                        if STATE.chat_id:
                                            await app.bot.send_message(
                                                STATE.chat_id,
                                                "⚠️ LIVE TRAIL-sälj "
                                                f"{sym} misslyckades, försöker igen "
                                                "senare.\n"
                                                f"{err}",
                                            )
                                        continue
                                ...
                                if (
                                    ...
                                    if STATE.chat_id:
                                        pause_t = (
                                            STATE.paused_until.astimezone()
                                            .strftime("%H:%M")
                                        )
                                        await app.bot.send_message(
                                            STATE.chat_id,
                                            "🛑 Loss-guard: pausar nya entries till "
                                            f"ca {pause_t}.",
                                        )
                                continue
                        else:
...
# -------- Ny test-buy logik: välj coin & gör köp + sälj --------
async def do_test_buy_roundtrip(chat_id: int, symbol: str):
    """
    Gör ett litet LIVE-test:
      1) Market BUY för TEST_BUY_USDT i vald symbol
      2) Hämtar trade-balans i basvalutan
      3) Market SELL av ~99.9% av balansen
    """
    if not kucoin_creds_ok():
        await tg_app.bot.send_message(
            chat_id,
    ...
    amount = TEST_BUY_USDT
    await tg_app.bot.send_message(
        chat_id,
        f"🔄 Kör LIVE test-köp + sälj för {symbol} ({amount:.2f} USDT)...",
    )

    ok, err = await kucoin_place_market_order(symbol, "buy", amount)
    if not ok:
        await tg_app.bot.send_message(
            chat_id,
            "❌ Testköp misslyckades.\n"
            f"KuCoin fel: {err}",
        )
        return
    ...
    await tg_app.bot.send_message(
        chat_id,
        f"✅ LIVE test-köp skickat: {symbol} för {amount:.2f} USDT.\n"
    ...
    bal = await kucoin_get_trade_balance(base)
    if not bal or bal <= 0:
        await tg_app.bot.send_message(
            chat_id,
            "⚠️ Hittade ingen trade-balans att sälja efter test-köpet.\n"
            "Kolla KuCoin-kontot manuellt.",
        )
        return

    sell_size = bal * 0.999  # lämna lite “dust” för att undvika exakt balans-problem
    ok2, err2 = await kucoin_place_market_order(symbol, "sell", sell_size)
    if ok2:
        await tg_app.bot.send_message(
            chat_id,
            f"✅ LIVE test-sälj klart: {symbol}, ca {sell_size:.8f} {base} sålt.\n"
...
async def cmd_test_buy(update: Update, _):
    """
    /test_buy             -> visar knappar för alla coins, köp + sälj 5 USDT
    /test_buy ADA-USDT    -> direkt test på given coin
    """
    STATE.chat_id = update.effective_chat.id
    ...
    toks = update.message.text.strip().split()
    if len(toks) >= 2:
        symbol = toks[1].upper().replace("/", "-")
        await do_test_buy_roundtrip(STATE.chat_id, symbol)
        return

    # Inga argument -> visa knappar för varje symbol
    buttons = [
        [InlineKeyboardButton(sym, callback_data=f"testbuycoin_{sym}")]
        for sym in STATE.symbols
    ]
    kb = InlineKeyboardMarkup(buttons)
    await tg_app.bot.send_message(
        STATE.chat_id,
        f"Välj coin för LIVE test-köp + sälj ({TEST_BUY_USDT:.2f} USDT, market):",
        reply_markup=kb,
    )
...
async def on_button(update: Update, _):
    ...
    # Ny: test-buy per coin via knapp
    if data.startswith("testbuycoin_"):
        await query.answer()
        symbol = data.split("testbuycoin_", 1)[1]
        await query.edit_message_text(
            f"🔄 Kör LIVE test-köp + sälj för {symbol}..."
        )
        await do_test_buy_roundtrip(chat_id, symbol)
        return
