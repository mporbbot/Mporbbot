...
# main_v47.py
# ------------------------------------------------------------
# MP Bot – v47 (max-SL, early-cut & max DD per trade + mode)
# - Reply-keyboard only
...
# - Trailing & HYBRID (startar efter vinst, låser min-vinst)
# - Live ändring av trade-belopp: /risk set size <USDT>
# - /mode <scalp|neutral|swing|safe-trail|show|set>
# - NYTT: /risk set sl_max … | early_cut on/off | early_cut_thr … | early_cut_window_m … | max_dd_per_trade …
...
POSITION_SIZE_USDT = float(os.getenv("POSITION_SIZE_USDT", "30"))   # default USDT per leg
...
# NYTT: hård stop i %, early-cut, max DD (USDT)
SL_MAX_PCT = float(os.getenv("SL_MAX_PCT", "0"))                # 0 = av
EARLY_CUT_ON = (os.getenv("EARLY_CUT_ON", "false").lower() in ("1", "true", "on", "yes"))
EARLY_CUT_THR = float(os.getenv("EARLY_CUT_THR", "0.30"))       # % mot dig inom fönster
EARLY_CUT_WIN_M = int(os.getenv("EARLY_CUT_WINDOW_M", "2"))     # minuter
MAX_DD_TRADE_USDT = float(os.getenv("MAX_DD_TRADE_USDT", "0"))  # 0 = av
...
HYB_START_PCT = float(os.getenv("HYB_START_PCT", "0.30"))      # aktivera trailing efter +0.30%
HYB_LOCK_PCT = float(os.getenv("HYB_LOCK_PCT", "0.10"))        # lås minst +0.10%
...
class EngineState:
    ...
    position_size: float = POSITION_SIZE_USDT   # legacy default
    ...
    risk_cfg: Dict[str, float] = field(default_factory=lambda: dict(
        ...
        size=POSITION_SIZE_USDT,   # live-positionsstorlek (USDT per leg)
        sl_max=SL_MAX_PCT,         # NYTT
        early_cut=EARLY_CUT_ON,    # NYTT (bool)
        early_cut_thr=EARLY_CUT_THR,
        early_cut_window_m=EARLY_CUT_WIN_M,
        ...
        try:
            if STATE.engine_on:
                ...
                for sym in STATE.symbols:
                    ...
                    # HYBRID: aktivera trailing efter vinst
                    if hyb_on and trail_on and not st.pos.trailing_active:
                        ...
                        if (st.pos.side == "LONG" and move_pct >= start) or (st.pos.side == "SHORT" and -move_pct >= start):
                            ...
                            st.pos.lock_price = (
                                st.pos.avg_price * (1.0 + lock/100.0) if st.pos.side == "LONG"
                    ...
                    # --- NYTT: Early-cut (inom tidsfönster från entry) ---
                    try:
                        if STATE.risk_cfg.get("early_cut", False):
                            win_m = int(STATE.risk_cfg.get("early_cut_window_m", 2))
                            thr = float(STATE.risk_cfg.get("early_cut_thr", 0.3))
                            age = datetime.now(timezone.utc) - st.pos.opened_at
                            if age <= timedelta(minutes=win_m):
                                hit = (st.pos.side == "LONG" and move_pct <= -thr) or \
                                      (st.pos.side == "SHORT" and move_pct >= thr)
                                if hit:
                    ...
                    # --- NYTT: Hård MAX-SL i % från avg_price ---
                    sl = float(STATE.risk_cfg.get("sl_max", 0.0) or 0.0)
                    if sl > 0:
                        hit = (st.pos.side == "LONG" and move_pct <= -sl) or \
                              (st.pos.side == "SHORT" and move_pct >= sl)
                        if hit:
                            net = _exit_all(sym, price, st)
                            if STATE.chat_id:
                                mark = "✅" if net >= 0 else "❌"
                    ...
                    # --- NYTT: Max DD i USDT (failsafe) ---
                    max_dd_usdt = float(STATE.risk_cfg.get("max_dd_per_trade", 0.0) or 0.0)
                    if max_dd_usdt > 0:
                        if st.pos.side == "LONG":
                            unreal = qty * max(0.0, st.pos.avg_price - price)
                        else:
                            unreal = qty * max(0.0, price - st.pos.avg_price)
                        if unreal >= max_dd_usdt:
                            net = _exit_all(sym, price, st)
                            if STATE.chat_id:
                                mark = "✅" if net >= 0 else "❌"
                                await app.bot.send_message(
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(STATE.chat_id, "MP Bot v47 – redo ✅", reply_markup=reply_kb())
...
async def cmd_risk(update: Update, _):
    ...
    msg = (
        ...
        f"  trail={'ON' if r.get('trail_on') else 'OFF'} (min={r.get('trail_min',0):.2f}% x{r.get('trail_mult',1):.2f})\n"
        f"  hybrid={'ON' if r.get('hyb_on') else 'OFF'} (start={r.get('hyb_start',0):.2f}% lock={r.get('hyb_lock',0):.2f}%)\n"
        f"  sl_max={r.get('sl_max',0):.2f}% (0=av)\n"
        f"  early_cut={'ON' if r.get('early_cut') else 'OFF'} (thr={r.get('early_cut_thr',0):.2f}% / {r.get('early_cut_window_m',0)}m)\n"
        f"  max_dd_per_trade={r.get('max_dd_per_trade',0):.2f} USDT (0=av)\n"
        "Ex: /risk set size 50   | /risk set dd 2.5   | /risk set max_pos 3\n"
        "    /risk set sl_max 0.40   | /risk set early_cut on   | /risk set early_cut_thr 0.30\n"
        "    /risk set early_cut_window_m 2   | /risk set max_dd_per_trade 3"
    )
    await tg_app.bot.send_message(STATE.chat_id, msg, reply_markup=reply_kb())
...
def _yesno(s: str) -> bool:
    return s.lower() in ("1", "true", "on", "yes", "y")
...
async def cmd_mode(update: Update, _):
    ...
    if len(toks) == 1:
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Använd: /mode <scalp|neutral|swing|safe-trail|show|set>\n"
            "Ex:\n"
            "  /mode scalp\n  /mode swing\n  /mode show\n"
            "  /mode set threshold 0.85\n  /mode set size 60\n  /mode set tfs 5m,15m,30m",
            reply_markup=reply_kb()
        )
        return
    ...
    if sub == "show":
        ...
        txt = (
            "Mode/params:\n"
            f"  threshold={STATE.entry_threshold:.2f}\n"
            f"  size={r.get('size', STATE.position_size):.2f} USDT\n"
            f"  max_pos={r['max_pos']}\n"
            f"  shorts={'ON' if r['allow_shorts'] else 'OFF'}\n"
            f"  trail_on={'ON' if r.get('trail_on') else 'OFF'}  "
            f"(trail_min={r.get('trail_min',0):.2f}%  trail_mult={r.get('trail_mult',1):.2f})\n"
            f"  hybrid={'ON' if r.get('hyb_on') else 'OFF'}  "
            f"(hyb_start={r.get('hyb_start',0):.2f}%  hyb_lock={r.get('hyb_lock',0):.2f}%)\n"
            f"  sl_max={r.get('sl_max',0):.2f}% | early_cut={'ON' if r.get('early_cut') else 'OFF'} "
            f"(thr={r.get('early_cut_thr',0):.2f}%/{r.get('early_cut_window_m',0)}m) | "
            f"max_dd_per_trade={r.get('max_dd_per_trade',0):.2f} USDT\n"
    ...
    if sub == "set":
        if len(toks) < 4:
            await tg_app.bot.send_message(
                ...
                "Nycklar: threshold, size, max_pos, shorts, trail_on, trail_min, trail_mult, "
                "hyb_on, hyb_start, hyb_lock, sl_max, early_cut, early_cut_thr, early_cut_window_m, "
                "max_dd_per_trade, tfs",
                reply_markup=reply_kb()
            )
            return

        key = toks[2].lower()
        val = " ".join(toks[3:]).strip()

        try:
            if key == "threshold":
                STATE.entry_threshold = float(val)
            elif key == "size":
                STATE.risk_cfg["size"] = float(val)
            elif key == "max_pos":
                STATE.risk_cfg["max_pos"] = int(float(val))
            elif key == "shorts":
                STATE.risk_cfg["allow_shorts"] = _yesno(val)
            elif key == "trail_on":
                STATE.risk_cfg["trail_on"] = _yesno(val)
            elif key == "trail_min":
                STATE.risk_cfg["trail_min"] = float(val)
            elif key == "trail_mult":
                STATE.risk_cfg["trail_mult"] = float(val)
            elif key == "hyb_on":
                STATE.risk_cfg["hyb_on"] = _yesno(val)
            elif key == "hyb_start":
                STATE.risk_cfg["hyb_start"] = float(val)
            elif key == "hyb_lock":
                STATE.risk_cfg["hyb_lock"] = float(val)
            elif key == "sl_max":
                STATE.risk_cfg["sl_max"] = float(val)
            elif key == "early_cut":
                STATE.risk_cfg["early_cut"] = _yesno(val)
            elif key == "early_cut_thr":
                STATE.risk_cfg["early_cut_thr"] = float(val)
            elif key == "early_cut_window_m":
                STATE.risk_cfg["early_cut_window_m"] = int(float(val))
            elif key == "max_dd_per_trade":
                STATE.risk_cfg["max_dd_per_trade"] = float(val)
            elif key == "tfs":
                tfs = [x.strip() for x in val.split(",") if x.strip()]
                STATE.tfs = tfs
            else:
                await tg_app.bot.send_message(STATE.chat_id, f"Okänd nyckel: {key}", reply_markup=reply_kb())
                return

            await tg_app.bot.send_message(STATE.chat_id, "Mode/param uppdaterad ✅", reply_markup=reply_kb())
            # snabb sammanfattning
            await cmd_mode(Update.de_json({"message": {"text": "/mode show"}}, tg_app.bot), _)
        except Exception:
            await tg_app.bot.send_message(STATE.chat_id, f"Fel värde ({key}): {val}", reply_markup=reply_kb())
        return

    # PRESETS
    if sub not in ("scalp", "neutral", "swing", "safe-trail"):
        await tg_app.bot.send_message(STATE.chat_id, f"Okänt mode: {sub}", reply_markup=reply_kb())
        return

    if sub == "scalp":
        STATE.entry_threshold = 0.55
        STATE.risk_cfg["trail_on"] = True
        STATE.risk_cfg["hyb_on"] = False
        STATE.risk_cfg["trail_min"] = 0.35
        STATE.risk_cfg["trail_mult"] = 2.0
        STATE.risk_cfg["max_pos"] = 3
        STATE.tfs = ["1m", "3m", "5m"]

    elif sub == "neutral":
        STATE.entry_threshold = 0.75
        STATE.risk_cfg["trail_on"] = True
        STATE.risk_cfg["hyb_on"] = True
        STATE.risk_cfg["trail_min"] = 0.60
        STATE.risk_cfg["trail_mult"] = 2.5
        STATE.risk_cfg["hyb_start"] = 0.40
        STATE.risk_cfg["hyb_lock"] = 0.15
        STATE.risk_cfg["max_pos"] = 2
        STATE.tfs = ["3m", "5m", "15m"]

    elif sub == "swing":
        STATE.entry_threshold = 0.90
        STATE.risk_cfg["trail_on"] = True
        STATE.risk_cfg["hyb_on"] = True
        STATE.risk_cfg["trail_min"] = 1.00
        STATE.risk_cfg["trail_mult"] = 3.5
        STATE.risk_cfg["hyb_start"] = 0.60
        STATE.risk_cfg["hyb_lock"] = 0.20
        STATE.risk_cfg["max_pos"] = 2
        STATE.tfs = ["5m", "15m", "30m"]

    elif sub == "safe-trail":
        STATE.entry_threshold = 0.85
        STATE.risk_cfg["trail_on"] = True
        STATE.risk_cfg["hyb_on"] = True
        STATE.risk_cfg["trail_min"] = 0.80
        STATE.risk_cfg["trail_mult"] = 3.0
        STATE.risk_cfg["hyb_start"] = 0.60
        STATE.risk_cfg["hyb_lock"] = 0.20
        STATE.risk_cfg["max_pos"] = 2
        STATE.tfs = ["5m", "15m", "30m"]
...
async def root():
    total = sum(STATE.per_sym[s].realized_pnl_net for s in STATE.symbols)
    return f"MP Bot v47 OK | engine_on={STATE.engine_on} | thr={STATE.entry_threshold:.2f} | pnl_total={total:+.4f}"
