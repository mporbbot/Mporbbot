...
async def kucoin_place_market_order(symbol: str, side: str, amount: float) -> bool:
    """
...
async def on_button(update: Update, _):
    ...
    if data == "testbuy_yes":
        ...
        if not kucoin_creds_ok():
            ...
            return
        ok = await kucoin_place_market_order("BTC-USDT", "buy", 5.0)
        if ok:
            await query.edit_message_text(
            ...
            await query.edit_message_text(
                ...
                "Kolla API-key, version och saldo."
            )
        return
