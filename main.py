                        ...
                        if regime == "trend":
                            ...
                            if score > STATE.threshold:
                                ...
                                if STATE.chat_id:
                                    await app.bot.send_message(
                                        STATE.chat_id,
                                        f"🟢 MOMO ENTRY {sym} LONG @ {price:.4f} | score={score:.2f} | thr={STATE.threshold:.2f}"
                                    )
                                if len(open_syms) >= STATE.max_pos:
                                    break
                                ...
                                if STATE.chat_id:
                                    await app.bot.send_message(
                                        STATE.chat_id,
                                        f"🔻 MOMO ENTRY {sym} SHORT @ {price:.4f} | score={score:.2f} | thr={STATE.threshold:.2f}"
                                    )
                                if len(open_syms) >= STATE.max_pos:
                                    break
                            ...
                            if dev_pct <= -STATE.mr_dev_pct:
                                ...
                                if STATE.chat_id:
                                    await app.bot.send_message(
                                        STATE.chat_id,
                                        f"🟢 MR ENTRY {sym} LONG @ {price:.4f} | dev={dev_pct:.2f}%"
                                    )
                                if len(open_syms) >= STATE.max_pos:
                                    break
                            # För långt över EMA20 => SHORT (om tillåtet)
                                ...
                                if STATE.chat_id:
                                    await app.bot.send_message(
                                        STATE.chat_id,
                                        f"🔻 MR ENTRY {sym} SHORT @ {price:.4f} | dev={dev_pct:.2f}%"
                                    )
                                if len(open_syms) >= STATE.max_pos:
                                    break
                ...
                for sym in STATE.symbols:
                    ...
                    if (not pos.trailing) and move_pct >= STATE.trail_start_pct:
                        ...
                        if STATE.chat_id:
                            await app.bot.send_message(
                                STATE.chat_id,
                                f"🔒 TRAIL ON {sym} | move≈{move_pct:.2f}% (regime={pos.regime},src={pos.reason})"
                            )
    ...
    for s in STATE.symbols:
        ...
        if st.pos:
            ...
            pos_lines.append(
                ...
                f"max_move≈{move_pct:.2f}% | regime={p.regime},{p.reason}"
            )
            ...
            if key == "size":
                ...
                await tg_app.bot.send_message(
                    ...
                    ("Stödjer: size, tp, sl, trail_start, trail, max_pos, "
                     "allow_shorts, mr_on, mr_dev, regime_auto, trend_slope_min, range_atr_max"),
                    reply_markup=reply_kb()
                )
                return
