# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# MP Bot – v60 (Hybrid Momentum + Mean Reversion, MOCK + LIVE)
# DigitalOcean + KuCoin + Telegram
# ------------------------------------------------------------
...
ALLOW_SHORTS_DEFAULT = (
    os.getenv("ALLOW_SHORTS", "false").lower() in ("1", "true", "yes", "on")
)
...
class Position:
    ...
    low_water: float
    trailing: bool = False
    regime: str = "trend"   # "trend" / "range"
    reason: str = "MOMO"    # "MOMO" / "MR"
...
async def kucoin_place_market_order(symbol: str, side: str, amount: float):
    """
    Market-order på spot.
    side: "buy" / "sell"
    amount:
      BUY  -> USDT-belopp (funds)
      SELL -> mängd bas-valuta (size)
    """
    side_l = side.lower()
    body = {
    ...
    if not data:
        return False, "Inget svar från KuCoin."
    ...
    if status != 200 or code != "200000":
        return False, f"HTTP={status}, code={code}, msg={msg}"

    return True, ""


async def kucoin_get_available_base(symbol: str) -> Optional[float]:
    """
    Hämta available balans i bas-valutan för en symbol, t.ex.
    BTC-USDT -> BTC i spot-wallet.
    """
    if not kucoin_creds_ok():
        return None

    base = symbol.split("-")[0]
    path = f"/api/v1/accounts?currency={base}"
...
async def kucoin_live_safe_sell(symbol: str, requested_qty: float):
    """
    Sälj "säkert" i LIVE:
      - hämtar available från KuCoin
      - säljer min(requested_qty, available) * 0.999
    för att undvika Balance insufficient.
    """
    qty = max(requested_qty, 0.0)
    avail = await kucoin_get_available_base(symbol)
    if avail is not None:
        qty = min(qty, avail)
        qty = qty * 0.999  # lämna lite dust för fees
    if qty <= 0:
        return False, "No available balance to sell."
    return await kucoin_place_market_order(symbol, "sell", qty)
...
# ============================================================
# TRADING HELPERS (MOCK + LIVE)
# ============================================================

def open_position(sym: str, side: str, price: float,
    ...
    while True:
        try:
                ...
                for sym in STATE.symbols:
                    ...
                    if move_pct >= STATE.tp_pct and not pos.trailing:
                        if STATE.trade_mode == "live" and pos.side == "LONG":
                            ok, err = await kucoin_live_safe_sell(sym, pos.qty)
                            if not ok:
                                if STATE.chat_id:
                                    await app.bot.send_message(
                                        STATE.chat_id,
            ...
            if STATE.chat_id:
                try:
                    await app.bot.send_message(
                        STATE.chat_id, f"⚠️ Engine-fel: {e}"
                    )
                except Exception:
                    pass
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "🤖 MP Bot v60 – Hybrid Momentum + Mean Reversion\n"
...
async def cmd_risk(update: Update, _):
    ...
    if len(toks) == 4 and toks[1] == "set":
        ...
        try:
            ...
            await tg_app.bot.send_message(
                STATE.chat_id,
                "Risk-parametrar uppdaterade.",
                reply_markup=reply_kb(),
            )
        except Exception:
            await tg_app.bot.send_message(
                STATE.chat_id,
                "Fel /risk set. Stödjer: size,tp,sl,trail_start,trail,max_pos,"
                "allow_shorts,loss_guard_on,loss_guard_n,loss_guard_pause",
                reply_markup=reply_kb(),
            )
        return
    ...
    kb = InlineKeyboardMarkup(
        [
            ...
            [
                ...
                InlineKeyboardButton("MaxPos 4", callback_data="risk_maxpos_4"),
                InlineKeyboardButton("MaxPos 6", callback_data="risk_maxpos_6"),
            ],
        ]
    )
    txt = (
        ...
        f"max_pos: {STATE.max_pos}\n"
        "Välj snabbknappar eller använd /risk set key value."
    )
    await tg_app.bot.send_message(STATE.chat_id, txt, reply_markup=kb)
...
async def cmd_mode(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "Använd: /mode, /mode mock eller /mode live",
        reply_markup=reply_kb(),
    )
    ...
    for sym in STATE.symbols:
        ...
        if STATE.trade_mode == "live" and pos.side == "LONG":
            ...
            if not ok:
                await tg_app.bot.send_message(
                    STATE.chat_id,
                    f"⚠️ LIVE close-all SELL {sym} misslyckades.\n{err}",
                )
                continue
...
async def cmd_reset_pnl(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "PnL återställd i minnet (CSV-loggarna finns kvar).",
        reply_markup=reply_kb(),
    )

# -------- TEST-BUY PER COIN (LIVE) --------

async def cmd_test_buy(update: Update, _):
    """
    /test_buy -> knappar för symboler.
    Gör LIVE market BUY (5 USDT) + försöker direkt SELL.
    """
    STATE.chat_id = update.effective_chat.id
    if not kucoin_creds_ok():
        await tg_app.bot.send_message(
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "Välj coin för LIVE test-köp + sälj (5 USDT):",
        reply_markup=kb,
    )
...
async def on_button(update: Update, _):
    ...
    if data.startswith("testbuy_"):
        ...
        if not ok:
            await tg_app.bot.send_message(
                chat_id,
                f"❌ Test-köp misslyckades för {symbol}.\nKuCoin fel: {err}",
                reply_markup=reply_kb(),
            )
            return
        await tg_app.bot.send_message(
            chat_id,
            f"✅ LIVE test-köp skickat: {symbol} för 5.00 USDT.\nFörsöker nu sälja (market).",
        )
        await asyncio.sleep(1.5)
        avail = await kucoin_get_available_base(symbol)
        qty_req = avail if avail is not None else 0.0
        ok2, err2 = await kucoin_live_safe_sell(symbol, qty_req)
        if ok2:
            await tg_app.bot.send_message(
                chat_id,
                f"✅ LIVE test-sälj klart: {symbol}, ca {qty_req:.8f} sålt.\n"
