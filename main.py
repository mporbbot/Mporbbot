...
# - Telegram + FastAPI (DigitalOcean-kompatibel via webhook)
#
# LÄGEN:
...
#   /risk              -> visa / ändra size, TP, SL, max_pos via knappar
#
...
class Position:
    ...
    reason: str = "MOMO"    # "MOMO" eller "MR" eller "TEST_BUY"
...
def kucoin_creds_ok() -> bool:
    ok = bool(KUCOIN_API_KEY and KUCOIN_API_SECRET and KUCOIN_API_PASSPHRASE)
    if ok:
        logger.info(
            "[KUCOIN DEBUG] key_len=%d, secret_len=%d, passphrase_len=%d, version='%s'",
            len(KUCOIN_API_KEY),
...
def open_position(
    ...
    """Öppna position i STATE + logga ENTRY."""
    size = size_usdt if size_usdt is not None else STATE.mock_size
    ...
    _log_trade({
        ...
        "fee_out": "",
        "net": "",
        "info": f"size_usdt={size};regime={regime};reason={reason};mode={STATE.trade_mode}",
    })
...
async def engine_loop(app: Application):
    ...
    while True:
        try:
            ...
            logger.exception("Fel i engine_loop: %s", e)
            if STATE.chat_id:
                try:
...
async def cmd_risk(update: Update, _):
    """
    /risk                -> visar knappar för size, TP, SL, max_pos
    /risk set key value  -> avancerade inställningar (size, tp, sl, trail, osv)
    """
    STATE.chat_id = update.effective_chat.id
    toks = update.message.text.strip().split()
    ...
    # Annars: visa snabbknappar för size + TP/SL + max_pos
    ...
    await tg_app.bot.send_message(
        ...
        (
            ...
            f"• TP: {STATE.tp_pct:.2f}%\n"
            f"• SL: {STATE.sl_pct:.2f}%\n"
            f"• max_pos: {STATE.max_pos}\n\n"
            "Ändra snabbt med knapparna, eller använd t.ex.:\n"
            "/risk set size 10\n"
            "/risk set tp 0.3\n"
            "/risk set sl 0.5\n"
            "/risk set max_pos 2"
...
async def cmd_test_buy(update: Update, _):
    ...
    Positionen registreras i STATE så att TP/SL/close_all fungerar.
    """
    STATE.chat_id = update.effective_chat.id
    if not kucoin_creds_ok():
        await tg_app.bot.send_message(
...
async def on_button(update: Update, _):
    ...
    # Snabb-TP
    if data in ("risk_tp_0_3", "risk_tp_0_5"):
        await query.answer()
        if data == "risk_tp_0_3":
            STATE.tp_pct = 0.30
            ...
            STATE.tp_pct = 0.50
        await query.edit_message_text(
            f"TP uppdaterad till {STATE.tp_pct:.2f}% (SL {STATE.sl_pct:.2f}%)."
        )
        return

    # Snabb-SL
    if data in ("risk_sl_0_5", "risk_sl_1_0"):
        await query.answer()
        if data == "risk_sl_0_5":
            STATE.sl_pct = 0.50
            ...
            STATE.sl_pct = 1.00
        await query.edit_message_text(
            f"SL uppdaterad till {STATE.sl_pct:.2f}% (TP {STATE.tp_pct:.2f}%)."
        )
        return
    ...
    if data.startswith("risk_maxpos_"):
        ...
        try:
            ...
            mp = STATE.max_pos
        STATE.max_pos = mp
        await query.edit_message_text(f"max_pos uppdaterad till {mp}.")
        return
    ...
    if data == "testbuy_yes":
        ...
        # 1) Skicka LIVE market BUY på BTC-USDT för 5 USDT
        ...
        # 2) Hämta ungefärligt pris för att skapa en position i STATE
        sym = "BTC-USDT"
        tf = STATE.tfs[0] if STATE.tfs else "1m"
        try:
            kl = await get_klines(sym, tf, limit=2)
        ...
        if sym not in STATE.per_sym:
            STATE.per_sym[sym] = SymState()

        # 3) Skapa position (regime=trend, reason=TEST_BUY)
        open_position(
            sym,
            "LONG",
            price,
        ...
        await query.edit_message_text(
            ...
            "Boten har nu registrerat en LONG-position i BTC-USDT.\n"
            "• /close_all stänger den\n"
            "• TP/SL/trailing/risk gäller som vanligt om engine är ON."
        )
        return
...
async def on_startup():
    ...
    if WEBHOOK_BASE:
        ...
        logger.warning("WEBHOOK_BASE saknas – boten förväntar sig ändå webhook-calls.")
