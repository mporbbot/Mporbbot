...
# Token env var: TELEGRAM_TOKEN  (primary)
# Fallback: TELEGRAM_BOT_TOKEN   (optional)
#
# Requirements:
...
# Mock positions: symbol -> (entry_price, qty, entry_ts)
...
def close_position(symbol: str, exit_price: float, cfg: BotConfig, reason: str) -> float:
    ...
    log_trade(
        is_mock=True if cfg.mode == "mock" else False,
        symbol=symbol,
        side="LONG",
        qty=qty,
        entry_price=entry,
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Mp ORBbot är igång.\n\n"
        "• /threshold 0.6 (fri input funkar)\n"
        "• /mode (AI-läge)\n"
...
def build_app() -> Application:
    # ✅ PRIMARY: TELEGRAM_TOKEN (matches your DigitalOcean env var)
    # Optional fallback: TELEGRAM_BOT_TOKEN
