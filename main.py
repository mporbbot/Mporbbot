# main.py – Mp ORBbot (Web Service / Free plan)
# Start command (Render Web Service): uvicorn main:app --host 0.0.0.0 --port $PORT
...
AI_MODE = os.getenv("AI_MODE", "neutral")
...
# ----- starta Telegram + engine i bakgrundstrådar -----
_app: Application | None = None
bot_started = False
...
def start_bot_threaded():
    ...
    # kör polling i separat daemontråd
...
@app.get("/")
def root():
    return {"status": "ok", "bot": BOT_NAME, "mode": "web+polling"}
