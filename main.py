# -*- coding: utf-8 -*-
# MP ORB/MOMO Hybrid Bot – CLEAN v1
# Features:
# - Momentum + Mean Reversion
# - MOCK + LIVE trading (endast LONG i LIVE)
# - TestBuy
# - TP/SL/Trail
# - Close All
# - Full Loss-Guard (STOP-GUARD)
# - PnL tracking
# - CSV-export
...
logger = logging.getLogger("mp_orb_clean")
...
if not BOT_TOKEN:
    raise RuntimeError("Missing TELEGRAM_BOT_TOKEN / BOT_TOKEN variable")
...
async def kucoin_private_request(method: str, path: str, body: Optional[dict] = None):
    ...
    body = body or {}
    body_str = json.dumps(body, separators=(",", ":")) if method != "GET" else ""

    ts = str(int(time.time() * 1000))
    prehash = ts + method.upper() + path + body_str

    sign = base64.b64encode(
        hmac.new(KUCOIN_API_SECRET.encode(), prehash.encode(), hashlib.sha256).digest()
    ).decode()
    ...
    try:
        async with httpx.AsyncClient(base_url=KUCOIN_BASE_URL, timeout=10) as client:
            r = await client.request(method, path, headers=headers, content=body_str or None)
...
# KUCOIN GET FILLS
# --------------------------------------------------
async def kucoin_get_fills_for_order(order_id: str):
    if not kucoin_creds_ok():
        return None
    ...
    for _ in range(6):
        ...
        if not data or data.get("_http_status") != 200 or data.get("code") != "200000":
    ...
    total_funds = sum(float(f.get("funds", 0)) for f in fills)
...
async def kucoin_place_market_order(symbol: str, side: str, amount: float):
    ...
    if not data or data.get("_http_status") != 200 or data.get("code") != "200000":
        return False, data.get("msg", "KuCoin error")

    order_id = (data.get("data") or {}).get("orderId")
    if not order_id:
        return False, "orderId missing"

    fills = await kucoin_get_fills_for_order(order_id)
    if fills:
        if side.lower() == "buy":
            LAST_LIVE_ENTRY_INFO[symbol] = fills
        else:
            LAST_LIVE_EXIT_INFO[symbol] = fills
...
async def get_klines(symbol: str, tf: str, limit: int = 100):
    ...
    params = {"symbol": symbol, "type": tf_api, "limit": limit}
    ...
    return data[::-1][:limit]  # oldest first
...
# --------------------------------------------------
# POSITION MGMT (TP / SL / TRAIL / LOSS-GUARD)
# --------------------------------------------------
...
def close_position(symbol: str, st: SymState,
    ...
    """
    Stänger en position & uppdaterar Loss-Guard.
    """
    if not st.pos:
        return 0.0
                ...
                try:
                    asyncio.create_task(
                        tg_app.bot.send_message(
                            STATE.chat_id,
                            f"⏸ Loss-guard aktiv – engine pausad i "
                            f"{STATE.loss_guard_pause_min} minuter."
                        )
                    )
                except Exception:
                    pass
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        ...
        "Justera momentum-känslighet med /threshold 0.60",
...
# --------------------------------------------------
# RISK / TP / SL / SIZE / MAX_POS / LOSS-GUARD
# --------------------------------------------------
...
async def cmd_threshold(update: Update, _):
    ...
    try:
        ...
        await tg_app.bot.send_message(
            STATE.chat_id,
            "Fel värde. Exempel: /threshold 0.60",
            reply_markup=reply_kb(),
        )
...
async def cmd_export_csv(update: Update, _):
    ...
    rows = [
        ["time", "symbol", "side", "entry", "exit", "gross",
         "fee_in", "fee_out", "net", "reason"]
    ]
        ...
        if len(parts) < 3 or parts[2].upper() != "JA":
            kb = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "✅ Ja, aktivera LIVE", callback_data="mode_live_yes"
                        ),
...
async def cmd_reset_pnl(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "PnL återställd (RAM) och Loss-Guard streak nollställd.",
        reply_markup=reply_kb(),
    )
...
async def cmd_test_buy(update: Update, _):
    ...
    Öppnar LONG-position i MOCK eller LIVE och låter engine sköta TP/SL/Trail.
    """
    STATE.chat_id = update.effective_chat.id
    msg = update.message.text.strip().split()
...
async def on_button(update: Update, _):
    ...
    if data.startswith("testbuy_confirm|"):
        ...
        open_position(symbol, "LONG", price, st, regime="trend", reason="TESTBUY")
