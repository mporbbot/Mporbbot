# main.py
# Mp ORBbot - Strategy: Micro Pullback Trend Scalper (1m entry + 5m trend)
# LONG only, more active + higher winrate focus.
#
# Keeps Telegram structure, partial + BE-after-partial, CSV logging, mock/live, fees/slippage.
#
# Key meanings:
#   /threshold = pullback tolerance (%) around EMA21 on 1m (how close price must touch)
#   /entry break  = first green close after touch -> enter
#   /entry retest = require reclaim above EMA21 after being below (prev close < ema21, cur close > ema21)
#
# pip:
...
# /threshold now = pullback tolerance (%) around EMA21 (1m)
PULLBACK_TOL_PCT_DEFAULT = 0.20

TP_PCT_DEFAULT = 0.70
SL_PCT_DEFAULT = 0.60

COOLDOWN_SECONDS_DEFAULT = 10
MAX_POSITIONS_DEFAULT = 5
STAKE_USDT_DEFAULT = 30.0

# Trailing runner (after partial)
TRAIL_ACTIVATE_PCT_DEFAULT = 0.35
TRAIL_DISTANCE_PCT_DEFAULT = 0.30

# Partial defaults
PARTIAL_ENABLED_DEFAULT = True
PARTIAL_TP_PCT_DEFAULT = 0.35
PARTIAL_FRACTION_DEFAULT = 0.30

# Breakeven buffer after partial
PARTIAL_BE_BUFFER_PCT_DEFAULT = 0.15

# Mock realism
MOCK_USE_BID_ASK_DEFAULT = True
MOCK_SLIPPAGE_PCT_DEFAULT = 0.02  # 0.02%
...
class BotConfig:
    ...
    # threshold = pullback tolerance (%)
    threshold_pct: float = PULLBACK_TOL_PCT_DEFAULT
    ...
    def from_json(d: Dict[str, Any]) -> "BotConfig":
        ...
        cfg.threshold_pct = float(clamp(float(cfg.threshold_pct), 0.05, 1.00))
...
# Pullback state: armed after touch EMA21
PULLBACK_ARMED: Dict[str, Dict[str, Any]] = {}
...
def threshold_buttons(current: float) -> InlineKeyboardMarkup:
    options = [0.10, 0.15, 0.20, 0.25, 0.30, 0.40]
    row = []
    for x in options:
...
async def open_long(app: Application, symbol: str, reason: str):
    ...
    PULLBACK_ARMED.pop(symbol, None)
    ...
    await notify_trade(
        ...
        f"| pullback_tol={CFG.threshold_pct:.2f}% | entry={CFG.entry_mode.upper()} | {reason}{extra}"
    )
...
async def manage_position(app: Application, symbol: str):
    ...
    # Partial + BE-after-partial + runner
        ...
        if px >= p.partial_price:
            ...
            if qty_part > 0:
                ...
                if symbol in POS:
                    ...
                    if be > p2.sl_price:
                        p2.sl_price = be
                        await notify_trade(app, f"🛡️ {symbol} SL -> BE+{CFG.partial_be_buffer_pct:.2f}% = {p2.sl_price:.6f}")
...
async def trend_ok_5m(symbol: str) -> bool:
    ...
    start = end - 60 * 60 * 15  # ~15h of 5m candles
    candles = await asyncio.to_thread(KC.get_klines, symbol, TF_TREND, start, end)
    if len(candles) < (TREND_EMA_SLOW + 20):
        TREND_CACHE_OK[symbol] = False
        TREND_CACHE_T[symbol] = now
        return False

    closes = [c["c"] for c in candles]
    ema_fast = ema_series(closes, TREND_EMA_FAST)
    ema_slow = ema_series(closes, TREND_EMA_SLOW)
    ...
    # mild slope (last 3 candles ~15m)
    ef_prev = ema_fast[-4]
    es_prev = ema_slow[-4]

    ok = (ef > es) and (ef >= ef_prev) and (es >= es_prev)
    TREND_CACHE_OK[symbol] = bool(ok)
    TREND_CACHE_T[symbol] = now
    return bool(ok)

# -----------------------------
# Entry logic: Micro pullback (1m)
# -----------------------------
async def evaluate_symbol(app: Application, symbol: str):
    ...
    # get enough 1m candles
    end = int(time.time())
    start = end - 60 * 600  # 10h of 1m
    candles = await asyncio.to_thread(KC.get_klines, symbol, TF_SIGNAL, start, end)
    if len(candles) < (ENTRY_EMA_SLOW + 30):
        return
    ...
    closes = [c["c"] for c in candles]
    ema9 = ema_series(closes, ENTRY_EMA_FAST)[-1]
    ema21_series = ema_series(closes, ENTRY_EMA_SLOW)
    ...
    # Require microtrend on 1m too: ema9 above ema21 and ema21 not falling
    if not (ema9 > ema21 and ema21 >= ema21_prev):
        PULLBACK_ARMED.pop(symbol, None)
        return

    tol = CFG.threshold_pct / 100.0  # how close low must get to ema21
    touch_level = ema21 * (1.0 + tol)

    # Arm when low touches EMA21 (within tolerance)
    if cur["l"] <= touch_level:
        PULLBACK_ARMED[symbol] = {"armed_ts": int(time.time())}

    if symbol not in PULLBACK_ARMED:
        return

    # Confirmation candle must be green and close above EMA9 (momentum back)
    ...
    if CFG.entry_mode == "break":
        # first green confirmation after touch
        await open_long(app, symbol, reason=f"PULLBACK touch EMA21 + green close>EMA9 (tol {CFG.threshold_pct:.2f}%)")
    else:
        # retest: require reclaim above EMA21 (prev close below, cur close above)
        if prev["c"] < ema21 and cur["c"] > ema21:
            await open_long(app, symbol, reason=f"PULLBACK RETEST reclaim EMA21 (tol {CFG.threshold_pct:.2f}%)")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Mp ORBbot är igång.\n"
        "Strategi: Micro Pullback Trend Scalper (1m entry + 5m trend EMA20>EMA50).\n"
        "LONG only.\n"
        "• /threshold = pullback-tolerans (%) mot EMA21 (1m)\n"
        "• /entry = BREAK (snabb) eller RETEST (strikt)\n"
        "• /partial = partial + BE + runner trail\n",
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        "Strategy: Micro Pullback Trend Scalper (1m entry + 5m trend EMA20>EMA50)",
        ...
        f"Pullback armed: {list(PULLBACK_ARMED.keys()) if PULLBACK_ARMED else 'none'}",
...
async def cmd_threshold(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if v is None:
            ...
            return
        CFG.threshold_pct = float(round(clamp(v, 0.05, 1.00), 4))
        save_config()
        await update.message.reply_text(f"✅ Pullback tolerans satt till: {CFG.threshold_pct:.2f}%")
        return
    await update.message.reply_text(
        f"Pullback tolerans nu: {CFG.threshold_pct:.2f}%\nVälj:",
...
def main():
    ...
    print("Mp ORBbot running (Micro Pullback Trend Scalper)...")
