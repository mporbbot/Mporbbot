# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# MP Bot – v47 (Break-Only Strategy)
# ------------------------------------------------------------
# Aktivare strategi:
#  - ENTRY direkt på break över lokalt HIGH (EMA20>EMA50) (shorts valfritt)
#  - Inga retests krävs => fler trades
#  - ATR-filter (lågt) och trendfilter kan stängas av
#  - Hybrid TP→Trail + SL + Early-cut + Max DD
#  - Mock-handel, CSV-export, Telegram UI, Render-vänlig
# ------------------------------------------------------------
...
DEFAULT_SYMBOLS = (os.getenv("SYMBOLS", "BTC-USDT,ETH-USDT,ADA-USDT,LINK-USDT,XRP-USDT")
                   .replace(" ", "")).split(",")
DEFAULT_TFS = (os.getenv("TIMEFRAMES", "3m,5m,15m").replace(" ", "")).split(",")

POSITION_SIZE_USDT = float(os.getenv("POSITION_SIZE_USDT", "60"))
...
# Grid/TP (vi håller DCA lågt i break-läge)
...
# ENTRY-threshold för break-score (högre = färre entries)
ENTRY_THRESHOLD = float(os.getenv("ENTRY_THRESHOLD", "1.10"))

# Trail/Hybrid
TRAIL_ON = (os.getenv("TRAIL_ON", "true").lower() in ("1","true","on","yes"))
TRAIL_ATR_MULT = float(os.getenv("TRAIL_ATR_MULT", "6.0"))
TRAIL_MIN_PCT = float(os.getenv("TRAIL_MIN_PCT", "2.5"))
HYB_ON = (os.getenv("HYB_ON", "true").lower() in ("1","true","on","yes"))
HYB_START_PCT = float(os.getenv("HYB_START_PCT", "2.0"))
HYB_LOCK_PCT = float(os.getenv("HYB_LOCK_PCT", "0.20"))

# Failsafes
SL_MAX_PCT = float(os.getenv("SL_MAX_PCT", "0.50"))
EARLY_CUT_ON = (os.getenv("EARLY_CUT_ON", "true").lower() in ("1","true","on","yes"))
EARLY_CUT_THR = float(os.getenv("EARLY_CUT_THR", "0.40"))
EARLY_CUT_WIN_M = int(os.getenv("EARLY_CUT_WINDOW_M", "3"))
MAX_DD_TRADE_USDT = float(os.getenv("MAX_DD_TRADE_USDT", "2.50"))

# Filter
BR_BREAK_LOOKBACK = int(os.getenv("BR_BREAK_LOOKBACK", "12"))       # lokalt high/low
TREND_FILTER = (os.getenv("TREND_FILTER", "false").lower() in ("1","true","on","yes"))  # OFF default
TREND_LEN = int(os.getenv("TREND_LEN", "35"))
ATR_MIN = float(os.getenv("ATR_MIN", "0.15"))  # låg vol tillåts (aktiv)
...
class EngineState:
    ...
    risk_cfg: Dict[str, float] = field(default_factory=lambda: dict(
        ...
        br_break_lookback=BR_BREAK_LOOKBACK, trend_filter=TREND_FILTER,
        trend_len=TREND_LEN, atr_min=ATR_MIN
    ))
...
def features_from_candles(candles: List[List[str]]) -> Dict[str, float]:
    ...
    if len(closes)<60:
        return {"ema20":0,"ema50":0,"atrp":0.25,"close":closes[-1],"hi":max(highs), "lo":min(lows)}
    ...
    hi = max(highs[-int(STATE.risk_cfg["br_break_lookback"]):])
    lo = min(lows [-int(STATE.risk_cfg["br_break_lookback"]):])
    return {"ema20":ema20,"ema50":ema50,"atrp":max(0.05,min(5.0,atrp)),
            "close":closes[-1],"hi":hi,"lo":lo,"high":highs[-1],"low":lows[-1]}
...
def _enter_leg(sym: str, side: str, price: float, usd_size: float, st: SymState):
    ...
    if not st.pos:
        st.pos = Position(side=side, legs=[leg], avg_price=price)
...
# -----------------------------
# Break-Only ENTRY score
# -----------------------------
def break_score(feats: Dict[str,float]) -> float:
    c=feats["close"]; ema20=feats["ema20"]; ema50=feats["ema50"]; hi=feats["hi"]; lo=feats["lo"]
    score=0.0
    # LONG: EMA20>EMA50 och close > lokalt high
    if ema20>ema50 and c>hi:
        overshoot = (c-hi)/(hi or 1)*100.0
        ema_gap = (ema20-ema50)/(ema50 or 1)*100.0
        score = overshoot + 0.4*ema_gap
    # SHORT: (om påslaget)
    elif ema50>ema20 and c<lo:
        undershoot = (lo-c)/(lo or 1)*100.0
        ema_gap = (ema50-ema20)/(ema20 or 1)*100.0
        score = undershoot + 0.4*ema_gap
        score = -score
    return score
            ...
            if STATE.engine_on:
                ...
                if len(open_syms) < int(STATE.risk_cfg["max_pos"]):
                    for sym in STATE.symbols:
                        ...
                        if st.pos: continue
                        tf0 = STATE.tfs[0] if STATE.tfs else "3m"
                        try:
                        ...
                        # Filter
                        if feats["atrp"] < float(STATE.risk_cfg.get("atr_min",0.15)):
                            continue
                        if bool(STATE.risk_cfg.get("trend_filter", False)):
                            # enkel trendfilter: kräver EMA20>EMA50 för longs när shorts OFF
                            if not STATE.risk_cfg.get("allow_shorts", False):
                                if feats["ema20"] <= feats["ema50"]:
                                    continue
                        score = break_score(feats)
                        if score > STATE.entry_threshold:
                            side="LONG"
                            price=feats["close"]
                            usd=float(STATE.risk_cfg.get("size", STATE.position_size))
                            _enter_leg(sym, side, price, usd, st)
                            if STATE.chat_id:
                                await app.bot.send_message(STATE.chat_id,
                                    f"🟢 ENTRY {sym} {side} @ {price:.4f} | score={score:.2f} | thr={STATE.entry_threshold:.2f}")
                            open_syms.append(sym)
                            if len(open_syms) >= int(STATE.risk_cfg["max_pos"]):
                                break
                        elif STATE.risk_cfg.get("allow_shorts", False) and (-score) > STATE.entry_threshold:
                            side="SHORT"
                            price=feats["close"]
                            usd=float(STATE.risk_cfg.get("size", STATE.position_size))
                            _enter_leg(sym, side, price, usd, st)
                ...
                for sym in STATE.symbols:
                    ...
                    # trail width
                    trail_w = max(float(STATE.risk_cfg.get("trail_min",2.5)), atrp*float(STATE.risk_cfg.get("trail_mult",6.0)))
                    ...
                    if bool(STATE.risk_cfg.get("hyb_on", True)) and not st.pos.trailing_active:
                        start=float(STATE.risk_cfg.get("hyb_start",2.0))
                        lock=float(STATE.risk_cfg.get("hyb_lock",0.20))
                    ...
                    if bool(STATE.risk_cfg.get("early_cut", True)):
                        if datetime.now(timezone.utc) - st.pos.opened_at <= timedelta(minutes=int(STATE.risk_cfg.get("early_cut_window_m",3))):
                            thr = float(STATE.risk_cfg.get("early_cut_thr",0.40))
                    ...
                    # Max SL
                    sl=float(STATE.risk_cfg.get("sl_max",0.5))
                    ...
                    if max_dd>0 and st.pos.qty_total()>0:
                        if st.pos.side=="LONG":
                            unreal = st.pos.qty_total()*max(0.0, st.pos.avg_price - price)
                        else:
                            unreal = st.pos.qty_total()*max(0.0, price - st.pos.avg_price)
                        if unreal >= max_dd:
                            net=_exit_all(sym, price, st)
                            if STATE.chat_id:
                                mark="✅" if net>=0 else "❌"
...
def reply_kb() -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton("/status"), KeyboardButton("/mode break")],
...
def status_text() -> str:
    ...
    lines=[
        ...
        "Strategi: Break-Only (direkt på break)",
        ...
        (f"Risk: dd={r['dd']}% | max_pos={r['max_pos']} | shorts={'ON' if r['allow_shorts'] else 'OFF'}\n"
         ...
         f"  Break: lookback={r['br_break_lookback']} | trend_filter={'ON' if r['trend_filter'] else 'OFF'} "
         f"| atr_min={r['atr_min']}%"),
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "🤖 Mp Bot v47 – Break-Only (mock)\nTips: /mode break, /engine_on",
...
async def cmd_timeframe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts)==2:
        ...
        await tg_app.bot.send_message(STATE.chat_id, "Använd: /timeframe 3m,5m,15m", reply_markup=reply_kb())
...
async def cmd_threshold(update: Update, _):
    ...
    try:
        ...
        await tg_app.bot.send_message(STATE.chat_id, "Fel värde. Ex: /threshold 1.10", reply_markup=reply_kb())
    ...
    if len(toks)==4 and toks[0]=="/risk" and toks[1]=="set":
        ...
        if key in STATE.risk_cfg:
            try:
                ...
                elif key in ("early_cut_window_m","br_break_lookback","trend_len"):
...
async def cmd_export_csv(update: Update, _):
    ...
    await tg_app.bot.send_document(STATE.chat_id, document=io.BytesIO(buf.getvalue().encode("utf-8")),
                                   filename="trades_v47_break.csv", caption="Export CSV")

async def cmd_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id=update.effective_chat.id
    sub=(context.args[0].lower() if context.args else "")
    if sub=="break":
        STATE.entry_threshold=1.10
        STATE.risk_cfg.update(dict(
            max_pos=4, size=60.0, allow_shorts=False,
            trail_on=True, trail_mult=6.0, trail_min=2.5,
            hyb_on=True, hyb_start=2.0, hyb_lock=0.20,
            sl_max=0.50, early_cut=True, early_cut_thr=0.40, early_cut_window_m=3,
            br_break_lookback=12, trend_filter=False, trend_len=35, atr_min=0.15
        ))
        await tg_app.bot.send_message(STATE.chat_id, "✅ Mode satt: break (Break-Only)", reply_markup=reply_kb()); return
    await tg_app.bot.send_message(STATE.chat_id, "Använd: /mode break", reply_markup=reply_kb())
...
async def root():
    total=sum(STATE.per_sym[s].realized_pnl_net for s in STATE.symbols)
    return f"MP Bot v47 Break-Only OK | engine_on={STATE.engine_on} | thr={STATE.entry_threshold:.2f} | pnl_total={total:+.4f}"
