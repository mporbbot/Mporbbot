# main.py
# Mp ORBbot – Render-vänlig.
# Kör som Web Service (uvicorn main:app ...) ELLER som Background Worker (python main.py).
# Mini-ORB per candle: entry direkt på break, initial stop under/över break-candle,
# trailing till föregående candle. Telegram inline-knappar JA/NEJ för start_mock/start_live.
...
CHAT_ID = int(os.getenv("CHAT_ID", "0"))  # sätt privat chat-id om du vill låsa
...
def update_orb_after_close(sym: SymState, closed: Candle):
    """Ny mini-ORB efter varje stängd candle (grön/röd)."""
...
def start_bot_once():
    """Starta Telegram + motortrådar exakt en gång (funkar för både worker & web)."""
    global _updater, _threads_started
    if _threads_started:
        return
    _threads_started = True

    _updater = Updater(TELEGRAM_TOKEN, use_context=True)
    ...
    # Trådar
    th_send = threading.Thread(target=sender_thread, args=(_updater,), daemon=True)
    th_send.start()
    th_engine = threading.Thread(target=engine_loop, args=(_updater,), daemon=True)
    th_engine.start()
    th_keep = threading.Thread(target=keepalive_thread, args=(_updater,), daemon=True)
    th_keep.start()
...
# ======================= ASGI-APP (för Render Web Service) =======================
class _SimpleASGIApp:
    """
    Minimal ASGI-app (utan externa webbramverk).
    Startar boten första gången den får en HTTP-förfrågan eller vid import i __main__.
    Endpoints:
      GET /        -> "Mp ORBbot OK"
      GET /health  -> {"status": "ok"}
    """
    def __init__(self):
        start_bot_once()  # se till att boten rullar även som web
    async def __call__(self, scope, receive, send):
        ...
        if path == "/" or path == "":
            body = f"{BOT_NAME} OK".encode("utf-8")
...
# Detta objekt används av `uvicorn main:app`
app = _SimpleASGIApp()

# ======================= KÖR SOM SCRIPT =======================
if __name__ == "__main__":
    # Kör som Background Worker: starta boten och blockera tråden.
    start_bot_once()
    print(f"{BOT_NAME} startad som background worker. Polling aktiv.")
    # Blockera huvudtråden utan att spinna CPU:
    try:
        while not stop_event.is_set():
            time.sleep(60)
