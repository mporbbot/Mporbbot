...
# main_v46.py
# ------------------------------------------------------------
# MP Bot – v46 (mode-presets + manuell tweak)
# - Reply-keyboard only
...
# - NYTT: /mode <scalp|neutral|swing|safe-trail|show|set>
        ...
        try:
            if STATE.engine_on:
                ...
                for sym in STATE.symbols:
                    ...
                    # HYBRID: aktivera trailing först efter vinst
    ...
    if sub == "set":
        if len(toks) < 4:
            await tg_app.bot.send_message(
                ...
                "hyb_on, hyb_start, hyb_lock, tfs\n"
                "Ex: /mode set threshold 0.85\n"
                "    /mode set size 60\n"
                "    /mode set tfs 5m,15m,30m",
                reply_markup=reply_kb()
            )
        ...
        try:
            if key == "threshold":
                ...
                STATE.entry_threshold = v
            elif key == "size":
                v = float(val);  assert v > 0
                STATE.risk_cfg["size"] = v
            elif key == "max_pos":
                v = int(float(val));  assert v >= 1
                STATE.risk_cfg["max_pos"] = v
            elif key == "shorts":
                ...
                v = float(val);  assert v >= 0
                STATE.risk_cfg["trail_min"] = v
            elif key == "trail_mult":
                v = float(val);  assert v > 0
                STATE.risk_cfg["trail_mult"] = v
            elif key == "hyb_on":
                STATE.risk_cfg["hyb_on"] = _yesno(val)
            elif key == "hyb_start":
                v = float(val);  assert v >= 0
                STATE.risk_cfg["hyb_start"] = v
            elif key == "hyb_lock":
                v = float(val);  assert v >= 0
                STATE.risk_cfg["hyb_lock"] = v
            elif key == "tfs":
            ...
            await tg_app.bot.send_message(STATE.chat_id, awaitable=False)
            # Call show:
            r = STATE.risk_cfg
            txt = (
                "Aktuella params:\n"
...
async def root():
    total = sum(STATE.per_sym[s].realized_pnl_net for s in STATE.symbols)
    return f"MP Bot v46 OK | engine_on={STATE.engine_on} | thr={STATE.entry_threshold:.2f} | pnl_total={total:+.4f}"
