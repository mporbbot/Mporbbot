...
#  - MOCK now simulates spread (ask/bid) + slippage (config default)
#
# Env required:
#   TELEGRAM_TOKEN="xxxx"
...
class KuCoinClient:
    ...
    def get_level1(self, symbol: str) -> Dict[str, float]:
        """
        Returns: {"price": last, "bid": bestBid, "ask": bestAsk}
        KuCoin returns strings; we cast to float. If bid/ask missing, fall back to last.
        """
        r = requests.get(
...
def mock_entry_price(level1: Dict[str, float]) -> float:
    """
    MOCK LONG entry: buy on ASK + slippage.
    """
...
def mock_exit_price(level1: Dict[str, float]) -> float:
    """
    MOCK LONG exit: sell on BID - slippage.
    """
    slip = CFG.mock_slippage_pct / 100.0
    if CFG.mock_use_bid_ask:
...
async def open_long(app: Application, symbol: str, reason: str):
    ...
    if CFG.trade_mode == "mock":
        ...
        # live: still use last as approximate for our internal tracking
...
async def close_long(app: Application, symbol: str, reason: str):
    ...
    # remove position after we have exit price
    POS.pop(symbol, None)
