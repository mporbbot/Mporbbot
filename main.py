# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# MP Bot – v52 (Hybrid Momentum + Mean Reversion, MOCK + LIVE)
# ------------------------------------------------------------
# - Default: endast mock-trades (ingen riktig handel)
...
# - Loss-guard: stoppa efter N förlusttrades i rad, pausa X minuter
# - Stöd för KuCoin spot LIVE-marketorders (mode = LIVE)
# - Telegram + FastAPI (Render-kompatibel)
# ------------------------------------------------------------

import os, io, csv, asyncio, time, json, base64, hmac, hashlib, uuid
...
# Risk-defaults (anpassade efter dina senaste inställningar)
TP_PCT = float(os.getenv("TP_PCT", "0.3"))        # take-profit (%)
SL_PCT = float(os.getenv("SL_PCT", "0.5"))        # stop-loss (%)
...
ALLOW_SHORTS_DEFAULT = (os.getenv("ALLOW_SHORTS", "false").lower()
                        in ("1", "true", "on", "yes"))
...
RANGE_ATR_MAX = float(os.getenv("RANGE_ATR_MAX", "0.8"))   # max ATR% för range

# Loss-guard defaults
LOSS_GUARD_ON = (os.getenv("LOSS_GUARD_ON", "true").lower()
                 in ("1", "true", "on", "yes"))
LOSS_GUARD_N = int(os.getenv("LOSS_GUARD_N", "2"))          # antal förluster i rad
LOSS_GUARD_PAUSE_MIN = int(os.getenv("LOSS_GUARD_PAUSE_MIN", "15"))

# Trade mode: MOCK (default) eller LIVE
TRADE_MODE = os.getenv("TRADE_MODE", "MOCK").upper().strip()

# KuCoin API för LIVE (spot)
...
KUCOIN_KLINES_URL = f"{KUCOIN_BASE}/api/v1/market/candles"
...
class EngineState:
    ...
    loss_guard_pause_min: int = LOSS_GUARD_PAUSE_MIN
    loss_guard_count: int = 0
    loss_guard_until: Optional[datetime] = None
    # Mode / chat
    mode: str = TRADE_MODE  # "MOCK" eller "LIVE"
...
def live_creds_ok() -> bool:
    return bool(KUCOIN_API_KEY and KUCOIN_API_SECRET and KUCOIN_API_PASSPHRASE)

# -----------------------------
# KuCoin LIVE helpers
# -----------------------------
async def kucoin_private_request(method: str, path: str, data: Optional[dict] = None) -> dict:
    """
    Signerad privat-förfrågan mot KuCoin (spot).
    """
    ...
    str_to_sign = f"{now}{method}{path}{data_json if method != 'GET' else ''}"
    signature = base64.b64encode(
        hmac.new(KUCOIN_API_SECRET.encode("utf-8"),
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        r = await client.request(method, url, headers=headers,
                                 data=(data_json if method != "GET" else None))
        r.raise_for_status()
        return r.json()

async def kucoin_place_spot_market(symbol: str, side: str, size: float) -> bool:
    """
    Skapar en spot-marketorder på KuCoin.
    side: "buy" eller "sell"
    size: mängd bas-coin (ex BTC) att köpa/sälja
    """
    if STATE.mode != "LIVE":
        return True  # inget att göra i mock-läge

    if not live_creds_ok():
        if STATE.chat_id:
            # Endast en varning, fortsätter i mock-läge
            await tg_app.bot.send_message(
                STATE.chat_id,
                "⚠️ TRADE_MODE=LIVE men KuCoin-API-nycklar saknas. Order EJ skickad."
            )
        return False
    ...
    try:
        res = await kucoin_private_request("POST", "/api/v1/orders", data)
        if res.get("code") == "200000":
            ...
            if STATE.chat_id:
                await tg_app.bot.send_message(
                    STATE.chat_id,
                    f"⚠️ KuCoin-order misslyckades ({symbol} {side}): {res}"
                )
            return False
        ...
        if STATE.chat_id:
            await tg_app.bot.send_message(
                STATE.chat_id,
                f"⚠️ KuCoin-orderfel ({symbol} {side}): {e}"
            )
        return False
...
# -----------------------------
# Trading helpers (mock + live bookkeeping)
# -----------------------------
...
def open_position(sym: str, side: str, price: float, st: SymState,
                  regime: str, reason: str, qty: Optional[float] = None):
    if qty is None:
    ...
    # Logg till mock-fil även i LIVE-läge (för historik)
    log_mock({
        ...
        "fee_out": "",
        "net": "",
        "info": f"mode={STATE.mode};size_usdt={STATE.mock_size};regime={regime};reason={reason}"
    })
...
def close_position(sym: str, price: float, st: SymState, reason: str) -> float:
    ...
    st.trades_log.append({
        ...
        "net": round(net, 6),
        "reason": f"{reason};regime={pos.regime};src={pos.reason};mode={STATE.mode}"
    })
    log_mock({
        "time": datetime.now(timezone.utc).isoformat(),
        "symbol": sym,
        ...
        "fee_out": round(fee_out, 6),
        "net": round(net, 6),
        "info": f"{reason};regime={pos.regime};src={pos.reason};mode={STATE.mode}"
    })
    st.pos = None
    return net

async def exit_with_live(sym: str, price: float, st: SymState, reason: str, app: Application) -> float:
    if not st.pos:
        return 0.0
    pos = st.pos

    # Skicka KuCoin-order i LIVE-läge
    if STATE.mode == "LIVE":
        side = "sell" if pos.side == "LONG" else "buy"
        ok = await kucoin_place_spot_market(sym, side, pos.qty)
        if not ok:
            # Avbryt exit om ordern inte gick igenom
            return 0.0

    net = close_position(sym, price, st, reason)

    # Loss-guard uppdatering
    if STATE.loss_guard_on:
        if net < 0:
            STATE.loss_guard_count += 1
            if STATE.loss_guard_count >= STATE.loss_guard_n:
                STATE.loss_guard_count = 0
                STATE.loss_guard_until = datetime.now(timezone.utc) + timedelta(
                    minutes=STATE.loss_guard_pause_min
                )
                if STATE.chat_id:
                    await app.bot.send_message(
                        STATE.chat_id,
                        f"🛑 Loss-guard aktiv – pausar nya entries i {STATE.loss_guard_pause_min} min."
                    )
        else:
            STATE.loss_guard_count = 0
    ...
    while True:
        try:
            if STATE.engine_on:
                ...
                # Loss-guard: blockera nya entries?
                blocked_by_loss_guard = False
                if STATE.loss_guard_on and STATE.loss_guard_until is not None:
                    if now < STATE.loss_guard_until:
                        blocked_by_loss_guard = True
                    else:
                        STATE.loss_guard_until = None  # pausa över

                if len(open_syms) < STATE.max_pos and not blocked_by_loss_guard:
                    for sym in STATE.symbols:
                        ...
                        # qty som används både för mock & live
                        qty = STATE.mock_size / price if price > 0 else 0.0
                        ...
                        if regime == "trend":
                            ...
                            if score > STATE.threshold:
                                # LONG
                                if STATE.mode == "LIVE":
                                    ok = await kucoin_place_spot_market(sym, "buy", qty)
                                    if not ok:
                                        continue
                                open_position(sym, "LONG", price, st,
                                              regime="trend", reason="MOMO", qty=qty)
                                open_syms.append(sym)
                                if STATE.chat_id:
                                    await app.bot.send_message(
                                        STATE.chat_id,
                                        f"🟢 MOMO ENTRY {sym} LONG @ {price:.4f} | "
                                        f"score={score:.2f} | thr={STATE.threshold:.2f} | regime=trend"
                                    )
                                if len(open_syms) >= STATE.max_pos:
                                    break
                            elif STATE.allow_shorts and score < -STATE.threshold:
                                # SHORT (rekommenderas EJ på spot)
                                if STATE.mode == "LIVE":
                                    # Om man någon gång kör futures kan detta ändras.
                                    # För nu: skip om LIVE + shorts.
                                    continue
                            ...
                            if dev_pct <= -STATE.mr_dev_pct:
                                if STATE.mode == "LIVE":
                                    ...
                                    if not ok:
                                        continue
                                open_position(sym, "LONG", price, st,
                                              regime="range", reason="MR", qty=qty)
                                open_syms.append(sym)
                                if STATE.chat_id:
                                    await app.bot.send_message(
                                        STATE.chat_id,
                            ...
                            # För långt över EMA20 => SHORT (om tillåtet)
                                ...
                                if STATE.mode == "LIVE":
                                    # igen: undvik shorts i LIVE/spot
                                    continue
                                open_position(sym, "SHORT", price, st,
                                              regime="range", reason="MR", qty=qty)
                                open_syms.append(sym)
                ...
                for sym in STATE.symbols:
                    ...
                    if move_pct >= STATE.tp_pct and not pos.trailing:
                        net = await exit_with_live(sym, price, st, "TP", app)
                        if net != 0.0 or not st.pos:
                            if STATE.chat_id:
                                mark = "✅" if net >= 0 else "❌"
                                await app.bot.send_message(
                                    STATE.chat_id,
                    ...
                    if pos.trailing:
                        if pos.side == "LONG":
                            ...
                            if price <= trail_stop:
                                net = await exit_with_live(sym, price, st, "TRAIL", app)
                                if net != 0.0 or not st.pos:
                                    if STATE.chat_id:
                                        mark = "✅" if net >= 0 else "❌"
                                        await app.bot.send_message(
                    ...
                    if move_pct <= -STATE.sl_pct:
                        net = await exit_with_live(sym, price, st, "SL", app)
                        if net != 0.0 or not st.pos:
                            if STATE.chat_id:
                                mark = "✅" if net >= 0 else "❌"
                                await app.bot.send_message(
...
def status_text() -> str:
    ...
    regime_line = "Regime: AUTO (trend + mean reversion)" if STATE.regime_auto else "Regime: TREND only"
    mr_line = (f"MR: {'ON' if STATE.mr_on else 'OFF'} | dev={STATE.mr_dev_pct:.2f}% | "
               f"range_atr_max={STATE.range_atr_max:.2f}%")
    trend_line = f"Trend-slope min: {STATE.trend_slope_min:.2f}%"

    # Loss-guard-info
    if STATE.loss_guard_on:
        if STATE.loss_guard_until and datetime.now(timezone.utc) < STATE.loss_guard_until:
            lg = (f"Loss-guard: ON | N={STATE.loss_guard_n} | pause={STATE.loss_guard_pause_min}m "
                  f"| ACTIVE till {STATE.loss_guard_until.astimezone().strftime('%H:%M')}")
        else:
            lg = f"Loss-guard: ON | N={STATE.loss_guard_n} | pause={STATE.loss_guard_pause_min}m"
    else:
        lg = "Loss-guard: OFF"

    mode_line = "Läge: LIVE-SPOT" if STATE.mode == "LIVE" else "Läge: MOCK (simulerad)"
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "🤖 MP Bot v52 – Hybrid Momentum + Mean Reversion\n"
        "Läge: MOCK (simulerad) som standard.\n"
...
async def cmd_engine_on(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        f"Engine: ON ✅ ({'LIVE' if STATE.mode == 'LIVE' else 'mock'})",
        reply_markup=reply_kb()
    )
...
async def root():
    ...
    return (
        ...
        f"engine_on={STATE.engine_on} | mode={STATE.mode} | "
