...
# Nu med PnL baserad på riktiga fills från KuCoin (funds + fee)
...
async def kucoin_get_fills_for_order(order_id: str) -> Optional[dict]:
    ...
    fills: List[dict] = []
    # För säkerhets skull: försök några gånger om fills inte hunnit in än
    for _ in range(6):
...
async def kucoin_place_market_order(symbol: str, side: str, amount: float):
    ...
    if not fills_info:
        # Order gick igenom men vi hittade inga fills (borde vara ovanligt för market)
        return True, "", {
            "orderId": order_id,
...
async def on_button(update: Update, _):
    ...
    if data == "testbuy_yes":
        ...
        if not kucoin_creds_ok():
            ...
            return
        ok, err, info = await kucoin_place_market_order("BTC-USDT", "buy", 5.0)
        if ok:
            avg = info.get("avg_price") if info else None
            txt = "✅ LIVE test-köp skickat: BTC-USDT för 5 USDT."
            if avg:
                txt += f"\nAvg pris≈ {avg:.4f}"
