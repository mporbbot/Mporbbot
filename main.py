# -*- coding: utf-8 -*-
# MP Hybrid ORB/MOMO Bot – Optimized v4
# - NO SHORT
# - Buttons (risk, mode, test_buy)
# - /stats
# - Max daily loss
# - Trade alerts (ENTRY/EXIT)
# - Loss-guard reset on /engine_on
...
class BotState:
    ...
    # max daglig förlust
    max_daily_loss: float = MAX_DAILY_LOSS_USDT
...
async def close_all_positions(app: Application, reason: str) -> (int, float):
    closed = 0
    total_net = 0.0
    ...
    for sym in STATE.symbols:
        ...
        net = close_pos(sym, st, reason, exit_price)
        closed += 1
        total_net += net

    return closed, total_net
        ...
        if STATE.max_daily_loss > 0:
            ...
            for s in STATE.symbols:
                for r in STATE.sym[s].log:
                    ...
                    if t.date() == today:
                        try:
                            daily_pnl += float(r["net"])
                        except Exception:
                            pass

            if daily_pnl <= -STATE.max_daily_loss:
                closed, total_net = await close_all_positions(app, "DAILY_MAX_LOSS")
                STATE.engine = False
                if STATE.chat_id:
                    try:
                        await app.bot.send_message(
                            STATE.chat_id,
                            f"⛔️ Max daglig förlust nådd ({daily_pnl:.2f} USDT).\n"
                            f"Stängde {closed} positioner (netto {total_net:+.4f}).\n"
                            f"Engine är nu STOPPAD."
                        )
                    except Exception:
                        pass
                await asyncio.sleep(2)
                ...
                if regime == "trend":
                    ...
                    if score > STATE.threshold:
                        if STATE.mode == "live":
                            ...
                            qty = info.get("size", STATE.mock_size / entry_price if entry_price else 0.0)
                            ...
                            qty = STATE.mock_size / entry_price if entry_price else 0.0
                        ...
                        if STATE.chat_id:
                            try:
                                await app.bot.send_message(
                                    STATE.chat_id,
                                    f"🟢 MOMO ENTRY {sym} LONG @ {entry_price:.4f} "
                                    f"(mode={STATE.mode.upper()}, score={score:.2f})"
                                )
                            except Exception:
                                pass
                    ...
                    if dev <= -STATE.mr_dev:
                        ...
                        if STATE.chat_id:
                            try:
                                await app.bot.send_message(
                                    STATE.chat_id,
                                    f"🟢 MR ENTRY {sym} LONG @ {entry_price:.4f} "
                                    f"(dev={dev:.2f}%, mode={STATE.mode.upper()})"
                                )
                            except Exception:
                                pass
            ...
            if move >= STATE.tp and not p.trailing:
                ...
                if STATE.chat_id:
                    try:
                        await app.bot.send_message(
                            STATE.chat_id,
                            f"🎯 TP EXIT {sym} @ {exit_price:.4f} | Net {net:+.4f} USDT"
                        )
                    except Exception:
                        pass
                ...
                if price <= stop_lvl:
                    ...
                    if STATE.chat_id:
                        try:
                            await app.bot.send_message(
                                STATE.chat_id,
                                f"🏁 TRAIL EXIT {sym} @ {exit_price:.4f} | Net {net:+.4f} USDT"
                            )
                        except Exception:
                            pass
            ...
            if move <= -STATE.sl:
                ...
                if STATE.chat_id:
                    try:
                        await app.bot.send_message(
                            STATE.chat_id,
                            f"⛔ SL EXIT {sym} @ {exit_price:.4f} | Net {net:+.4f} USDT"
                        )
                    except Exception:
                        pass
...
def reply_kb() -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton("/status"), KeyboardButton("/pnl"), KeyboardButton("/stats")],
...
def status_text() -> str:
    ...
    loss_line = (
        f"Loss-guard (streak): N={STATE.loss_n}, pause={STATE.loss_pause}m, "
        f"streak={STATE.loss_streak}"
    )
    if STATE.paused_until:
        loss_line += f" | paus till {STATE.paused_until.astimezone().strftime('%H:%M')}"

    daily_info = f"Max daily loss: {STATE.max_daily_loss:.2f} USDT"
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "🤖 MP ORB/MOMO Bot – Optimized v4\n"
        "Standardläge: MOCK (simulerad).\n"
        "Nyheter:\n"
        "• /stats för trade-statistik\n"
        "• Max daily loss + loss-guard\n"
        "• Trade alerts för alla entries/exits.",
...
async def cmd_engine_on(update: Update, _):
    ...
    # Reset loss-guard när du startar engine
    STATE.loss_streak = 0
    STATE.paused_until = None

    STATE.engine = True
    await tg_app.bot.send_message(
        STATE.chat_id, "Engine ON ✅ (loss-guard reset)", reply_markup=reply_kb()
    )
...
# /stats – trade-statistik
async def cmd_stats(update: Update, _):
    STATE.chat_id = update.effective_chat.id

    trades: List[dict] = []
    for s in STATE.symbols:
        trades.extend(STATE.sym[s].log)

    if not trades:
        await tg_app.bot.send_message(
            STATE.chat_id, "Inga avslutade trades ännu – ingen statistik.",
            reply_markup=reply_kb()
        )
        return
    ...
    sum_win = sum(t.get("net", 0) for t in wins) if wins else 0.0
    sum_loss = sum(t.get("net", 0) for t in losses) if losses else 0.0
    ...
    pf = None
    if sum_loss < 0:
        pf = sum_win / abs(sum_loss)

    per_reason: Dict[str, float] = {}
    for t in trades:
        r = str(t.get("reason", "UNK"))
        per_reason.setdefault(r, 0.0)
        try:
            per_reason[r] += float(t.get("net", 0))
        except Exception:
            pass

    lines = []
    lines.append(f"📊 Trades totalt: {n}")
    lines.append(f"✅ Vinster: {len(wins)} | ❌ Förluster: {len(losses)}")
    lines.append(f"Winrate: {winrate:.1f}%")
    lines.append(f"Snittvinst: {avg_win:+.4f} USDT")
    lines.append(f"Snittförlust: {avg_loss:+.4f} USDT")
    if pf is not None:
    ...
    lines.append("")
    lines.append("Per strategi (reason):")
    for r, v in per_reason.items():
        lines.append(f"• {r}: {v:+.4f} USDT")
...
async def cmd_risk(update: Update, _):
    ...
    if len(toks) == 4 and toks[1] == "set":
        ...
        try:
            if key == "size":
                ...
                STATE.max_daily_loss = float(val)
            else:
                raise ValueError()
            await tg_app.bot.send_message(
    ...
    text = (
        ...
        f"Trail start: {STATE.trail_on:.2f}% | Trail: {STATE.trail_pct:.2f}%\n"
        f"Max Pos: {STATE.max_pos}\n"
        f"Max daily loss: {STATE.max_daily_loss:.2f} USDT\n\n"
        "Välj knapp nedan eller använd:\n"
        "`/risk set key value`\n"
        "Ex: /risk set max_daily_loss 25"
    )
...
async def cmd_close_all(update: Update, _):
    STATE.chat_id = update.effective_chat.id
    closed, total_net = await close_all_positions(tg_app, "MANUAL_CLOSE")
    await tg_app.bot.send_message(
        STATE.chat_id,
...
async def root():
    ...
    return (
        f"MP Bot Optimized v4 OK | engine={STATE.engine} | "
