# -*- coding: utf-8 -*-
# MP ORB/MOMO Hybrid Bot – v57 (LONG ONLY)
#
# Features:
# - Kör en engine_loop som:
...
#   • /timeframe 1m,3m,...
#   • /threshold 0.55
#   • /risk (+ inline-knappar, /risk set key value)
...
#   • /close_all – stänger alla positioner (inkl. riktiga sells i live).
...
def open_position(symbol: str, side: str, price: float,
    ...
    # I vår design är side alltid "LONG" – ingen short
    if STATE.trade_mode == "live":
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(
        STATE.chat_id,
        "🤖 MP Bot v57 – Hybrid Momentum + Mean Reversion (LONG only)\n"
    ...
    if parts[1].lower() == "live":
        ...
        if len(parts) < 3 or parts[2].upper() != "JA":
            ...
            await tg_app.bot.send_message(
                ...
                "Detta skickar riktiga market-ordrar på KuCoin (LONG only).",
                reply_markup=kb,
            )
            return
...
async def on_button(update: Update, _):
    ...
    if data == "mode_live_confirm":
        ...
        await query.edit_message_text(
            "⚠️ Vill du slå på LIVE?\nDetta skickar riktiga ordrar på KuCoin (LONG only).",
            reply_markup=kb,
        )
        return
