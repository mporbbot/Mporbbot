# main_20.py — Mp ORBbot (PTB v20, FastAPI, valbar TF 1/3/5/15)
# - Entry: ORB-long + CloseBreak (valbara)
# - Stop: sätts på ORB-low vid entry, flyttas UPP till föreg. candle-low
# - TF: /tf 1|3|5|15 (KuCoin '1min','3min','5min','15min')
# - Två layouter: 1) BUY-kort (entry/stop/size/reason)  2) Kompakt statuslista
# - Knappar/kommandon kvar (setMyCommands)
#
# Start: uvicorn main_20:app --host 0.0.0.0 --port $PORT
# Env: TELEGRAM_BOT_TOKEN, KUCOIN_BASE_URL (valfritt)
...
async def notify_buy(ctx: ContextTypes.DEFAULT_TYPE, chat_id: int, symbol: str, entry: float, stop: Optional[float], qty: float, reason: str):
    pct_str = "-"
    ...
    text = (
        ...
        f"entry={entry:.4f}  stop={stop:.4f if stop is not None else '-'} {pct_str}\n"
        f"size={qty:.6f}\n"
        f"reason={reason}"
    )
    try:
        await ctx.bot.send_message(chat_id=chat_id, text=text, parse_mode=ParseMode.HTML)
    except Exception:  # fortsätt ändå
        traceback.print_exc()
...
def update_stop_trailing(symbol: str, prev_closed_low: Optional[float], curr: Candle):
    """Höj hård stop till föregående candles low (eller ORB-low initialt). Sälj om träffad av aktuell candle."""
...
def fetch_kucoin(symbol: str, tf_min: int, limit: int = 50)->List[Candle]:
    ...
    for it in reversed(arr[-limit:]):   # äldst → nyast
        ...
        # öppning ≈ slut - tf_min*60
        ts=(t_end - tf_min*60)*1000
        out.append(Candle(ts,o,h,l,c,v))
    return out

def poll_loop(send_buy_cb):
    """send_buy_cb(context, symbol, entry, stop, qty, reason) for BUY-notiser"""
    while True:
        try:
            tf = STATE.tf_min
            for s in SYMBOLS:
                ...
                for c in candles:
                    if last is None or c.ts>last:
                        pre_pos = STATE.positions.get(s)
                        res = None
                        # process
                        process_candle(s, prev, c)
                        # om köp skedde: pre_pos None → nu finns pos
                        post_pos = STATE.positions.get(s)
                        if pre_pos is None and post_pos is not None:
                            res = ("buy", post_pos.qty, post_pos.stop, post_pos.entry, "ENTRY")
                        if last is None or c.ts>last:
                            STATE.last_candle_ts[s]=c.ts
                    prev=c
            # polling-interval beroende på TF
            time.sleep(10 if tf==1 else 20)
...
def fmt_status_compact() -> str:
    lines=[f"Mode: {STATE.mode}   Engine: {'ON' if STATE.trading_enabled else 'OFF'}",
           ...
           f"Trail: {'ON' if STATE.trailing_enabled else 'OFF'}",
           f"DayPnL: {current_pnl_with_offset():.4f} USDT",
           f"ORB master: {'ON' if STATE.use_orb else 'OFF'}"]
    for s in SYMBOLS:
        ...
        orb_str = f"ORB: {'ON' if orb else '-'}"
        lines.append(f"{s}: {pos_str} | {orb_str}")
    return "\n".join(lines)
...
async def h_entry_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args:
        ...
        if arg=="orb": STATE.use_orb=True; STATE.use_closebreak=False
            ...
            await update.message.reply_text("Använd: /entry_mode [orb|closebreak|both]"); return
...
async def h_tf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text(f"Aktuell TF: {STATE.tf_min}m. Använd /tf 1|3|5|15")
        return
    try:
...
# (valfri text-handler)
async def h_echo(update: Update, context: ContextTypes.DEFAULT_TYPE): pass

# ====================== Körning (PTB + poll) ======================
_application = None
def _run_bot_and_poll():
    # separat event loop för PTB v20 i egen tråd
    async def _amain():
        global _application
        ...
        # starta polling parallellt
        def buy_cb(ctx, symbol, entry, stop, qty, reason): pass  # köp-notiser sköts i place_order+notify vid behov
...
@app.get("/")
def root():
    return {"name":"Mp ORBbot","msg":"Service up (v20)","note":"TF 1/3/5/15, ORB stop trail ↑ prev low"}
