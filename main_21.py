...
# - Stop: sätts på ORB-low vid entry; höjs till föregående candle-low (aldrig nedåt)
# - BUY-kort skickas vid köp; SELL-kort vid stängning (STOP/PANIC)
# - /tf 1|3|5|15, /status, övriga knappar/kommandon
# - REST-polling av KuCoin (stabilt för Render)
# - Ingen PTB JobQueue krävs (egen async outbox-worker)
...
def fetch_kucoin(symbol: str, tf_min: int, limit: int = 60)->List[Candle]:
    ...
    for it in reversed(arr[-limit:]):   # äldst→nyast
        ...
        ts=(t_end - tf_min*60)*1000     # approx open-tid
...
# --- Egen outbox-worker: skickar meddelanden utan JobQueue ---
...
async def h_echo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    STATE.chat_id = update.effective_chat.id  # säkerställ chat-id
...
def _run_bot_and_poll():
    async def _amain():
        ...
        # 🧹 Ta bort ev. webhook så polling funkar
        ...
        # Starta outbox-worker (ingen JobQueue krävs)
