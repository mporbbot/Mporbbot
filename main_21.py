...
# - REST-polling av KuCoin-candles; stabil på Render
...
def fetch_kucoin(symbol: str, tf_min: int, limit: int = 60)->List[Candle]:
    ...
    for it in reversed(arr[-limit:]):   # äldst→nyast
        ...
        ts=(t_end - tf_min*60)*1000     # approximativ open-tid
...
# Töm OUTBOX och skicka till chat
...
def _run_bot_and_poll():
    async def _amain():
        ...
        # 🧹 Viktigt: ta bort ev. gammal webhook så polling funkar
        ...
        # Skicka kort från OUTBOX var sekund
...
@app.get("/")
def root():
    return {"name":"Mp ORBbot","version":"v21","note":"TF 1/3/5/15, BUY/SELL-kort, webhook-kill on start"}
