# main_22_2.py — PTB v20 + BUY/SELL-kort, TF 1/3/5/15
# - Robust Telegram-tokenhantering: startar bara TG om giltig token finns
# - Rensar ev. webhook; polling i bakgrundstråd med stop_signals=None (fixar add_signal_handler-felet)
...
def _valid_token(t: str) -> bool:
    # Vanligt PTB-format: <digits>:<tecken>
...
async def h_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Mp ORBbot v22.2 igång.\n" + HELP)
...
def _run_bot_and_poll():
    async def _amain():
        ...
        # VIKTIGT: stop_signals=None för att tillåta körning i bakgrundstråd
        await _application.run_polling(drop_pending_updates=True, stop_signals=None)
...
@app.get("/")
def root():
    return {"name":"Mp ORBbot","version":"v22.2","note":"stop_signals=None + robust TG token"}
...
@app.get("/tg_test")
def tg_test(chat_id: int, text: str = "hello from v22.2"):
    api = _tg_api()
    if not api: return {"error":"no valid token"}
    try:
