...
STATE = {
    ...
    "chat_id": None,             # sätts när user gör /start
}
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Hej! ORB-bot v25 uppe.\nDin chat_id = {chat_id}\n"
        "Använd knapparna eller kommandon.\n/menu /pnl /panic etc.",
        reply_markup=build_menu_kb(),
    )


async def cmd_whoami(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"Din chat_id = {update.effective_chat.id}")
...
async def tg_outbox_pumper(app: Application):
    while True:
        try:
            ...
            continue
        try:
            if item["type"] == "text" and STATE["chat_id"]:
