...
# Symbols (KuCoin-format med bindestreck)
SYMBOLS = ["BTC-USDT", "ETH-USDT", "ADA-USDT", "LINK-USDT", "XRP-USDT"]
...
STATE: Dict = {
    ...
    "realized_pnl_usdt": 0.0,    # total PnL
    "outbox": queue.Queue(),     # telegram utgående meddelanden
    "chat_id": None,             # sätts vid /start
}
...
def tg_send_text(text: str, parse_mode: Optional[str] = "HTML"):
    msg = {
        "type": "text",
        "text": text,
...
def _map_tf_to_kucoin(tf: str) -> str:
    return {"1m": "1min", "3m": "3min", "5m": "5min", "15m": "15min"}.get(tf, "1min")
...
async def kucoin_klines(symbol: str, tf: str, limit: int = 60) -> List[List]:
    ...
    Datapunkt: [time, open, close, high, low, volume, turnover] (allt str)
    """
    k_tf = _map_tf_to_kucoin(tf)
    ...
    # sortera äldst->nyast med tid som int
...
def compute_orb(candles: List[List]) -> Optional[Tuple[float, float]]:
    """
    Enkel ORB: använd första candle i listan (äldsta) som bas.
...
def candle_trailing_stop_long(current_stop: float, prev_low: float) -> float:
    return max(current_stop, prev_low)


def candle_trailing_stop_short(current_stop: float, prev_high: float) -> float:
    return min(current_stop, prev_high)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Spara chat_id för utgående notiser
    chat_id = update.effective_chat.id if update.effective_chat else None
    if chat_id:
        STATE["chat_id"] = chat_id
        log.info(f"[TG] chat_id set: {chat_id}")

    await update.message.reply_text(
        "Hej! ORB-bot v26 uppe. Använd knapparna eller kommandon.\n"
...
async def cmd_panic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for symbol, pos in list(STATE["open_positions"].items()):
        ...
        STATE["realized_pnl_usdt"] += trade_pnl
        del STATE["open_positions"][symbol]
        tg_send_text(make_sell_card(symbol, price, qty, trade_pnl, "PANIC"))
        closed += 1
...
async def tg_outbox_pumper(app: Application):
    """Skickar alla meddelanden som ligger i outbox-kön till STATE['chat_id']."""
    while True:
        try:
        ...
        try:
            if item["type"] == "text":
                ...
                if not chat_id:
                    # Ingen chat_id ännu: användaren måste skriva /start en gång
                    continue
