...
log = logging.getLogger("mp_orbbot_v38")
...
CHAT_ID = os.getenv("CHAT_ID", "").strip()  # valfri; om satt så pushas loggar dit
...
TIMEFRAME = "3min"              # KuCoin kline typ
TRADE_SIZE_USDT = 30            # mock trade storlek
MIN_GREEN_BODY_PCT = 0.05       # minsta gröna kropp (%) för ORB-candle
ENGINE_TICK_SEC = 10            # loop-sleep

# --------------------------
# APPs
# --------------------------
app = FastAPI(title="MP ORB BOT v38")

# KuCoin marknadsklient
...
# per symbol: aktiv trade
# {"entry": float, "stop": float, "entry_time": dt, "last_i": int}
open_trades: Dict[str, Dict] = {}

# pnl historik
# {"symbol": str, "pnl": float, "exit_price": float, "entry": float, "time": dt}
pnl_history: List[Dict] = []

# cache för sista behandlade index så vi inte retriggar
last_index_cache: Dict[str, int] = {s: -1 for s in SYMBOLS}

# --------------------------
# HJÄLPFUNKTIONER – DATA
# --------------------------
def _df_from_klines(klines: List[List]) -> pd.DataFrame:
    """
    KuCoin get_kline returnerar listor:
...
def detect_orb(df: pd.DataFrame) -> Optional[Dict]:
    ...
    Returnerar dict {"high": float, "low": float, "index": int} eller None.
    """
    if len(df) < 2:
        return None
...
def find_entry_after_orb(df: pd.DataFrame, orb: Dict, after_index: int) -> Optional[Dict]:
    ...
    after_index används för att inte duplicera entries om vi redan agerat.
    Returnerar {"i": int, "price": float} eller None.
    """
...
# --------------------------
# HJÄLPFUNKTIONER – TRADES
# --------------------------
def _size_from_entry(entry_price: float) -> float:
    # USDT-kvantitet approx i underliggande
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("MP ORBbot v38 – redo ✅", reply_markup=main_keyboard())
...
async def engine_loop():
    await asyncio.sleep(1)  # ge tg-app tid att starta
            ...
            if engine_running:
                for symbol in SYMBOLS:
                    try:
                        ...
                        # 1) hitta ORB (första gröna efter röd)
                        orb = detect_orb(df)

                        # 2) ENTRY: vänta på candle (2,3,4...) som stänger över ORB-high
                        if orb:
                        ...
                        # 3) TRAILING STOP – följer med candlelow uppåt
                        if symbol in open_trades:
                            last_c = df.iloc[-1]  # använd senaste stängda candle
                            ...
                            if last_price < new_stop:
                                ...
                                # valfritt debugmeddelande om stop höjdes
