...
CHAT_ID = os.getenv("CHAT_ID", "").strip()  # valfri (för push)
...
# pnl historik
# {"symbol": str, "pnl": float, "exit_price": float, "entry": float, "time": dt, "reason": str}
pnl_history: List[Dict] = []

# cache för senaste entry-index per symbol (för att inte dubbel-trigga)
...
def _df_from_klines(klines: List[List]) -> pd.DataFrame:
    """
    KuCoin get_kline returnerar listor: [time, open, close, high, low, volume, turnover]
    """
...
def detect_orb(df: pd.DataFrame) -> Optional[Dict]:
    ...
    Returnerar {"high": float, "low": float, "index": int} eller None.
    """
    if len(df) < 2:
        return None
...
def main_keyboard() -> InlineKeyboardMarkup:
    # Endast knapparna nedtill (som i v36)
    row = [
...
async def engine_loop():
    await asyncio.sleep(1)  # ge tg-app tid att initialiseras
            ...
            if engine_running:
                for symbol in SYMBOLS:
                    try:
                        ...
                        # 1) ORB (första gröna efter röd, min kropp)
                        orb = detect_orb(df)
                        ...
                        # 3) TRAILING STOP – följer candle low uppåt
                        if symbol in open_trades:
                            last_c = df.iloc[-1]  # senaste STÄNGDA candle
...
async def on_startup():
    # Initiera Telegram (webhook-läge, ingen polling)
