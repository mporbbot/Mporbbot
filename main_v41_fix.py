...
# EN panel så knappar alltid är nederst
...
async def tg_webhook(token: str, request: Request):
    ...
    log.info("Webhook hit ✅")  # <- syns i Render logs vid varje inkommande
    try:
...
# Sänd en testnotis till CHAT_ID
@app.get("/poke")
async def poke():
    if not CHAT_ID:
