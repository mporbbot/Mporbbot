...
def ku_price(symbol:str)->Optional[float]:
    try:
        r = requests.get(f"{KU_PUB}/market/orderbook/level1", params={"symbol":symbol}, timeout=10)
        j = r.json()
...
def ku_klines(symbol:str, tf:str, limit:int=3)->List[dict]:
    ...
    try:
        r = requests.get(f"{KU_PUB}/market/candles", params={"type":t, "symbol":symbol}, timeout=10)
        j = r.json()
        if j.get("code")=="200000":
            rows = []
...
def write_trade(symbol, side, qty, price, pnl=0.0, mode="MOCK"):
    ...
    with open(TRADES_CSV, "a", newline="") as f:
        ...
        w.writerow(row)
    if side.lower()=="sell":
        # Förenklad K4-rad (1 rad per avyttring, i USDT – omräkning till SEK görs i efterhand)
...
def try_entry(sym:str, price:float, cur:dict):
    ...
    if p.qty>0 or not o.active or o.used:
        return
    do_buy = False
    if ENTRY_MODE == "close":
...
def maybe_exit(sym:str, price:float):
    ...
    if p.qty>0 and p.stop:
        if price <= p.stop:
            ...
            # Nästa ORB armeras när ny röd→grön uppstår (maybe_arm_orb)
...
# =========================
def trader_loop():
    # separat loop som håller igång Render (keepalive) – oberoende av engine_on
    last_ping = 0
    while STATE.running:
        try:
            # handel
            ...
            # -------- KEEPALIVE --------
            now = time.time()
            if STATE.keepalive and PING_URL and now - last_ping >= 120:
                try:
