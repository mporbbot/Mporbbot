...
# NY: minsta grön kropp i procent för att en grön candle ska få skapa ORB
MIN_GREEN_BODY_PCT = float(os.getenv("MIN_GREEN_BODY_PCT", "0.05"))  # t.ex. 0.05 = 0.05%
...
class SymState:
    ...
    orb_candle_time: Optional[int] = None  # ms (candle som skapade ORB)
...
def detect_orb(prev: Tuple[int,float,float,float,float],
    ...
    """
    NY ORB:
      • Den FÖRSTA gröna candlen som kommer DIREKT efter en röd.
      • Den gröna måste ha minst MIN_GREEN_BODY_PCT i kropp.
      • ORB high/low = den gröna candlens high/low.
    Returnerar (ORB_high, ORB_low, orb_time_ms) om cur kvalar.
    """
    _, po, ph, pl, pc = prev
    _, co, ch, cl, cc = cur
    if is_red(po,ph,pl,pc) and is_green(co,ch,cl,cc) and body_pct(co, cc) >= MIN_GREEN_BODY_PCT:
        return (ch, cl, cur[0])
    return None

def entry_close_rule(orb_high: float, closed_candle: Tuple[int,float,float,float,float], orb_time_ms: int) -> bool:
    """
    NY ENTRY (close-läge):
      • Köp FÖRST när en SENARE candle (2,3,4,...) stänger över ORB_high.
      • Dvs closed_candle måste komma efter den candle som skapade ORB.
    """
    t, o, h, l, c = closed_candle
    return (t > orb_time_ms) and (c > orb_high)

def entry_tick_rule(orb_high: float, current_candle: Tuple[int,float,float,float,float], orb_time_ms: int) -> bool:
    """
    Tick-läge (behåller som option):
      • Köp om en senare candle når över ORB_high intrabar.
    """
    t, o, h, l, c = current_candle
        ...
        try:
            if STATE.engine_on and STATE.orb_on:
                for sym in STATE.symbols:
                    ...
                    # 1) ORB (första gröna efter röd med min-kropp)
                    orb = detect_orb(last_closed, current)
                    if orb:
                        st.orb_high, st.orb_low, st.orb_candle_time = orb

                    # 2) ENTRY – bara om vi inte redan är inne och ORB finns
                    if st.pos is None and st.orb_high and st.orb_low and st.orb_candle_time:
                        if STATE.entry_mode == "close" and entry_close_rule(st.orb_high, last_closed, st.orb_candle_time):
                        ...
                        elif STATE.entry_mode == "tick" and entry_tick_rule(st.orb_high, current, st.orb_candle_time):
                    ...
                    if st.pos:
                        ...
                        if exited:
                            ...
                            if len(st.trades) > 50:  # cap historik
...
async def send_status(chat_id: int):
    ...
    lines = [
        ...
        f"ORB: {'PÅ' if STATE.orb_on else 'AV'} (första gröna efter röd, min kropp {MIN_GREEN_BODY_PCT:.3f}%)",
...
async def cmd_start(update: Update, _):
    ...
    await tg_app.bot.send_message(STATE.chat_id, "Hej! ORB-bot v36 (ORB/entry uppdaterad) ✅", reply_markup=reply_kb())
...
async def root():
    ...
    return {
        ...
        "min_green_body_pct": MIN_GREEN_BODY_PCT,
    }
